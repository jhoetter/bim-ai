...
(async () => {
  ...
  const mirrorBtn = page
    .locator('[data-testid="tool-mirror"], button[aria-label="Mirror"], button:has-text("Mirror")')
    .first();
  ...
  await shot(
    page,
    '21-wp47-inspector-boundary-card.png',
    'WP-47 inspector with Edit Boundary card (roof/ceiling)',
  );
  ...
  const threeDTab = page
    .locator('[data-testid="view-3d"], [aria-label="3D"], button:has-text("3D")')
    .first();
  ...
  await shot(
    page,
    '24-wp49-repair-panel.png',
    'WP-49 plan Modify tab with Repair panel (Delete Duplicate + Detach Orphan)',
  );
  ...
  if (hasBanner) {
    await shot(
      page,
      '25-wp49-boundary-error-banner.png',
      'WP-49 boundary-validation-error dismissable banner (live)',
    );
  } else {
    // Capture advisor violations panel as secondary proof
    const advisorBtn = page
      .locator(
        '[data-testid="advisor-panel-toggle"], button:has-text("Issues"), button:has-text("Advisor")',
      )
      .first();
    ...
    await shot(
      page,
      '25-wp49-advisor-structural-violations.png',
      'WP-49 advisor panel with structural violations (boundary-error gate)',
    );
  }
  ...
  const splitBtn = page
    .locator(
      '[data-testid="split-view"], [data-testid="split-pane"], button:has-text("Split"), [aria-label*="Split"]',
    )
    .first();
  ...
  const palette = page
    .locator(
      '[data-testid="cmd-palette"], [role="dialog"][aria-label*="command"], [role="combobox"]',
    )
    .first();
  if (await palette.isVisible().catch(() => false)) {
    ...
    await shot(
      page,
      '27-wp50-cmdk-roof-from-walls.png',
      'WP-50 Cmd+K palette showing roof-from-walls and structural commands',
    );
  } else {
  ...
  await shot(
    page,
    '28-wp45-plan-structural-stack-clean.png',
    'WP-45 plan view full structural model (no orphaned constraints)',
  );

  // ── 29: WP-50 final clean state screenshot ────────────────────────────────
  await shot(
    page,
    '29-wp50-session-final.png',
    'WP-50 final session state — clean structural authoring',
  );
