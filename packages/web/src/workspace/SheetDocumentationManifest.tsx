  ...
  return (
    ...
    >
      ...
      {evidence.status === 'ready' && deterministicRow ? (
        <div className="mt-3 space-y-1 border-t border-border pt-2">
          <div className="text-[10px] font-semibold uppercase text-muted">
            Section-on-sheet integration (v1)
          </div>
          <p className="text-[9px] text-muted">
            One row per section/elevation viewport — cut-line digest, profile token, listing-segment
            digest, and revision/issue cross-reference (WP-E03/E04/E05/E06/V01).
          </p>
          <div
            className="overflow-x-auto"
            data-testid="sheet-manifest-section-on-sheet-integration-readout"
          >
            <table className="min-w-[1100px] border-collapse border border-border font-mono text-[10px]">
              <thead>
                <tr className="bg-muted/30">
              ...
              <tbody>
                {sectionOnSheetIntegrationRows.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="border border-border px-1 py-0.5 text-muted">
                      (no section/elevation viewports on this sheet)
                    </td>
                  </tr>
                ) : (
                  sectionOnSheetIntegrationRows.map((r, i) => {
                    const vid = String(r.viewportId ?? '');
                    const cutDigest = String(r.cutLineDigestSha256 ?? '');
                    const listDigest = String(r.listingSegmentDigestSha256 ?? '');
                    return (
                      <tr key={`sec-on-sheet-${vid || i}`}>
                        ...
                        <td className="border border-border px-1 py-0.5 font-mono">
                          {cutDigest ? cutDigest.slice(0, 12) : '—'}
                        </td>
                        <td className="border border-border px-1 py-0.5">
                          {String(r.sectionProfileToken ?? '') || '—'}
                        </td>
                        <td className="border border-border px-1 py-0.5 break-all">
                          {String(r.listingSegment ?? '') || '—'}
                        </td>
                        <td className="border border-border px-1 py-0.5 font-mono">
                          {listDigest ? listDigest.slice(0, 12) : '—'}
                        </td>
                        <td className="border border-border px-1 py-0.5 break-all">
                          {String(r.sheetRevIssDocCrossRef ?? '') || '—'}
                        </td>
                      </tr>
                    );
                  })
