...
const VISIBILITY_CAPABILITIES: CommandCapabilityDraft[] = [
  ...
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§2.4.3: JoinGeometryCmd/UnjoinGeometryCmd store sorted element ID pairs in joinedPairs[]; plan symbology renders a join indicator at the midpoint between joined elements.',
  },
  {
...
const MASS_CAPABILITIES: CommandCapabilityDraft[] = [
  ...
  },
  {
    ...
    scope: 'selection',
    intendedModes: ['plan', '3d'],
    surfaces: ['element-sidebar', 'cmd-k'],
    executionSurface: 'store',
    preconditions: ['selected-type-element'],
    status: 'implemented',
    usabilityScore: 8,
    notes:
      '§1.6.2: SaveFamilyToLibraryCmd stores element type as family_definition in store; inspector button on wall_type/floor_type/roof_type; DB-backed, no file required.',
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§1.6.2: AppSettingsPanel with defaultUnits (mm/cm/m) + uiDensity (normal/compact); appSettings in store; Settings… button in ProjectMenu.',
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§1.6.2: cloud-native version history panel listing milestones from milestoneStore; Restore + Delete per row; RestoreMilestoneCmd in core; Version History… button in ProjectMenu.',
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§9.5.3: steel_connection element (end_plate/bolted_flange/shear_tab) with boltRows/boltCols/boltDiameterMm; CreateSteelConnectionCmd; plan symbol; steel-connection tool in registry; inspector case in InspectorContent.',
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§9.5.4: beam_section_profile element type with profilePoints[]; SetBeamSectionProfileCmd patches sectionProfileId on beam; parametric cross-section for structural detail.',
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§1.6.5 Wave 33 completion marker: plan ribbon exposes Systems/MEP, Insert, Annotate, Analyze/Review, Collaborate links, View/visibility controls, Manage controls, contextual Modify, Steel, Precast, and Massing/Site tab coverage. 3D has its own implemented ribbon subset and is not claimed by this marker.',
  },
  {
    ...
    usabilityScore: 8,
    notes:
      '§1.6.5: steel tab (connections/framing tools), precast tab (column/beam/slab), massing-site tab (in-place mass, toposolid, building pad) added to plan RibbonBar tabs.',
  },
];
