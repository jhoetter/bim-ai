...
describe('default Cmd+K commands', () => {
  ...
  it('routes lens discipline commands through the palette host context', () => {
    ...
    expect(setLensMode).toHaveBeenNthCalledWith(4, 'energy');
    expect(setLensMode).toHaveBeenNthCalledWith(5, 'construction');
    expect(setLensMode).toHaveBeenNthCalledWith(6, 'sustainability');
  });
