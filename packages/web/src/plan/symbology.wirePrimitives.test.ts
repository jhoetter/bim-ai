...
function countFamilySemanticLines(root: THREE.Object3D, semanticKind: string): number {
  let n = 0;
  root.traverse((o) => {
    if (o instanceof THREE.Line && o.userData.familySymbolicSemanticKind === semanticKind) n += 1;
  });
  return n;
}
...
describe('PlanCanvas server wire primitives path (WP-C03)', () => {
  ...
  it('honors VV hidden categories for loaded family symbolic subcategories', () => {
    const familyType: Extract<Element, { kind: 'family_type' }> = {
      kind: 'family_type',
      id: 'ft-chair',
      name: 'Chair Type',
    ...
    rebuildPlanMeshes(
      ...
      {
        activeLevelId: 'lvl',
        hiddenSemanticKinds: new Set(['family_hidden_cut']),
      },
    );

    expect(countFamilySemanticLines(root, 'family_symbolic_line')).toBe(1);
    expect(countFamilySemanticLines(root, 'family_hidden_cut')).toBe(0);
  });
});
