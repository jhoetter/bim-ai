...
/** Mock PNG bytes for the browser-free `sheet-print-raster` endpoint. */
...
const MOCK_SHEET_SVG_SHA = '38780a26160d611bdc16776e682d852bbf4eecd83f93f9d4fef1e3d0b3e2f4cd';

const MOCK_SHEET_PDF_SHA = 'f'.repeat(64);

const MOCK_SHEET_PRINT_RASTER_PNG_SHA =
  '01a1bd732c454a43298e4888bce24a483808695147e28c3ab5e6a0ecab12e349';

const MOCK_SHEET_RASTER_METADATA_SHA =
  'ce452b73d2ef6b39621da34f8227b7ca75aa6685825b34fdda6bcfb836576ce3';
...
async function sharedRoutes(page: Page, layoutPreset: string) {
  ...
  await page.route(`**/api/models/*/exports/sheet-print-raster.png**`, async (route) => {
    await route.fulfill({
      ...
      headers: {
        'X-Bim-Ai-Sheet-Print-Raster-Contract': 'sheetPrintRasterPrintContract_v3',
        'X-Bim-Ai-Sheet-Svg-Sha256': MOCK_SHEET_SVG_SHA,
        'X-Bim-Ai-Sheet-Pdf-Sha256': MOCK_SHEET_PDF_SHA,
        'X-Bim-Ai-Sheet-Print-Raster-Metadata-Sha256': MOCK_SHEET_RASTER_METADATA_SHA,
        'X-Bim-Ai-Sheet-Print-Raster-Width': '256',
        'X-Bim-Ai-Sheet-Print-Raster-Height': '224',
      },
    });
  });
          ...
          deterministicSheetEvidence: [
            {
              ...
              sheetPrintRasterIngest_v1: {
                ...
                contract: 'sheetPrintRasterPrintContract_v3',
                svgContentSha256: MOCK_SHEET_SVG_SHA,
                pdfContentSha256: MOCK_SHEET_PDF_SHA,
                printRasterPngSha256: MOCK_SHEET_PRINT_RASTER_PNG_SHA,
                placeholderPngSha256: MOCK_SHEET_PRINT_RASTER_PNG_SHA,
                metadataSha256: MOCK_SHEET_RASTER_METADATA_SHA,
                metadata: {
                ...
                diffCorrelation: {
                  ...
                  notes:
                    'mock ingest — v3 print-contract hash; correlate vs Playwright baselines in CI only.',
                },
              },
...
test.describe('evidence PNG baselines', () => {
  ...
  test('evidence-package exposes closure review inventory', async ({ page }) => {
    ...
    expect(rasterIngest?.contract).toBe('sheetPrintRasterPrintContract_v3');
    expect(rasterIngest?.printRasterPngSha256).toBe(MOCK_SHEET_PRINT_RASTER_PNG_SHA);
    expect(rasterIngest?.pdfContentSha256).toBe(MOCK_SHEET_PDF_SHA);
    expect(rasterIngest?.metadataSha256).toBe(MOCK_SHEET_RASTER_METADATA_SHA);
    const rasterValidation = rasterIngest?.serverValidation as Record<string, unknown> | undefined;
    expect(rasterValidation?.format).toBe('sheetPrintRasterContractValidation_v3');
    expect(rasterValidation?.ok).toBe(true);
  });

  test('sheet-print-raster.png: print-contract v3 response headers', async ({ page }) => {
    await sharedRoutes(page, 'coordination');
    await page.goto('/');
    await expect(page.getByText('Ready', { exact: false })).toBeVisible({ timeout: 30_000 });
    const result = await page.evaluate(async (mid: string) => {
      ...
      return {
        ...
        pdfSha: r.headers.get('X-Bim-Ai-Sheet-Pdf-Sha256'),
        metadataSha: r.headers.get('X-Bim-Ai-Sheet-Print-Raster-Metadata-Sha256'),
    ...
    expect(result.contract).toBe('sheetPrintRasterPrintContract_v3');
    expect(result.svgSha).toBe(MOCK_SHEET_SVG_SHA);
    expect(result.pdfSha).toBe(MOCK_SHEET_PDF_SHA);
    expect(result.metadataSha).toBe(MOCK_SHEET_RASTER_METADATA_SHA);
