...
async function advisorSummary(modelId, { severity = null } = {}) {
  ...
  for (const v of violations) {
    ...
    for (const recommendation of actionability.recommendation ? [actionability.recommendation] : []) {
...
async function cmdIntegrity(modelId, userId, args) {
...
async function cmdProfileComparison(modelId, args) {
...
async function cmdDocumentation(modelId, userId, sub, args) {
...
async function cmdDocumentationPresentationPack(modelId, userId, args) {
  const opts = authorOptions(args);
  const sheetId = flagValue(args, '--sheet-id');
...
function withOptionalNumber(command, key, value) {
  if (value != null) command[key] = parseNumber(value, undefined);
}

function withOptionalString(command, key, value) {
  if (value != null && value !== '') command[key] = value;
}

function siteBundle(toolId, commands, opts) {
...
function commandForSiteGradedRegion(sub, args) {
  if (sub === 'create') {
...
function commandForSitePropertyLine(sub, args) {
  if (sub === 'create') {
    const line = parsePoint2List(flagValue(args, ['--line', '--points']), '--line');
    if (line.length !== 2) {
...
function commandForSiteToposolidExcavation(sub, args) {
  if (sub === 'create') {
    const hostToposolidId = flagValue(args, ['--host-toposolid', '--hostToposolidId']);
...
function commandForSiteSubdivision(sub, args) {
  if (sub === 'update') {
    const id = flagValue(args, '--id') ?? args.find((a) => !String(a).startsWith('-'));
    if (!id) {
...
function commandForSiteBasePoint(sub, args) {
  if (sub === 'create') {
    const command = {
...
function commandForSiteSurveyPoint(sub, args) {
  if (sub === 'create') {
    const command = {
...
function commandForSiteSunSettings(sub, args) {
...
function commandsForSiteSetup(args) {
...
async function cmdSite(modelId, userId, domain, sub, args) {
  const opts = authorOptions(args);
  let command;
  let toolId;
  if (domain === 'setup') {
