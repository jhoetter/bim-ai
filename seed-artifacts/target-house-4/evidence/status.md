...
- `sketch_bim.py compile --seed target-house-4`: pass, 172 commands.
...
- The source recipe now includes front glass panels, basement openings,
  roof-tile material, kitchen clearance correction, and attic-wall
  construction-readiness metadata.
- Renderer code now places dormer bodies on the sampled roof plane and adds
  dormer glazing.
- Constructability geometry now has a physical collision proxy for `dormer`.
...
- Browser evidence was recaptured in `phase-final/browser-evidence.json` with
  the saved long-elevation 3D view active and the tour dismissed.
