...
| `BIR-C08` | `app/bim_ai/model_integrity_hosting.py`; `packages/web/src/viewport/collectRendererDiagnostics.ts`                     | `app/tests/test_model_integrity_hosting.py`; `packages/web/src/viewport/collectRendererDiagnostics.test.ts`                                                                                                           | Backend `hosted_render_proxy_orphan` integrity findings, renderer `detached_or_proxy_render_risk` diagnostics, and W17-C helper-family render/export leakage fixtures carry `BIR-C08` evidence. | Wave 14-B local commit; Wave 17-C local commit                   | Renderer-side coverage is wall-hosted cut diagnostics; broader renderer/export pipeline rejection for every non-wall helper proxy remains partial.                                  |
