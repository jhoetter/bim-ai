...
| `BIR-N01` | `packages/cli/lib/target-house-geometry-diagnostics.mjs`; `packages/cli/generate-target-house-geometry-diagnostic.mjs` | `packages/cli/targetHouseGeometryDiagnostics.test.mjs`; `pnpm --filter -ai/cli test` | `seed-artifacts/target-house-1/evidence/live-run-current/target-house-geometry-diagnostic.json`; `seed-artifacts/target-house-1/evidence/live-run-current/target-house-geometry-diagnostic.md` | Wave 8 Worker A local commit | Current report is diagnostic-only and intentionally does not mutate seed source; it records 155 current findings for downstream correction workers. |
