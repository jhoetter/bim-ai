...
| `BIR-N02` | P0       | Partial | Fix seed source, not only live state.                | Wave 8 Worker B corrected `seed-artifacts/target-house-1/evidence/target-house-1.recipe.json` and the authoritative `bundle.json`: removed physical access-helper walls/doors, removed the overlapping full-width loggia wall opening, and made the front wrapper split reproducible from recipe semantics. `make seed name=target-house-1` now replays the corrected bundle; final live screenshot/evidence acceptance remains stale and must be refreshed before this can move to Done.                                                                                         |
