...
| ID        | Priority | Status  | Item                                         | Acceptance                                                                                                                                                                                                                                                                                                                     |
| --------- | -------- | ------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `BIR-P01` | P0       | Partial | Define document invariant contract.          | `modelIntegrityInvariantContract_v1` now states role/kind classification, references, unit and coordinate contracts, type-instance relations, schema compatibility policy, and tracked P01-P08 coverage. Evidence: `app/bim_ai/model_integrity.py`, `app/tests/test_model_integrity_invariants.py`.                            |
...
| `BIR-P08` | P1       | Partial | Add invariant smoke command.                 | `modelIntegritySmokeCommandEvidence_v1` now bundles smoke findings, units/coordinate normalization, type-instance resolution, schema compatibility, a stable digest, and CLI/API command evidence. Evidence: `app/bim_ai/model_integrity.py`, `app/tests/test_model_integrity_invariants.py`.                                  |

### Q. Transactions, Collaboration, Undo/Redo, And Agent Safety

| ID        | Priority | Status  | Item                                     | Acceptance                                                                                                                                                                                                                                                           |
| --------- | -------- | ------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BIR-Q01` | P0       | Partial | Protect transaction boundaries.          | Added pure `transactionSafety_v1` checks requiring parent revisions across dry-run, commit, bundle, UI command, and MCP mutation surfaces. Evidence: `app/bim_ai/transaction_safety.py`, `app/tests/test_transaction_safety.py`.                                     |
| `BIR-Q02` | P0       | Partial | Preserve undo/redo semantics.            | Added `undoRedoContract_v1` validation for inspectable forward commands, inverse commands, changed ids, undo availability metadata, and advancing revisions. Evidence: `app/bim_ai/transaction_safety.py`, `app/tests/test_transaction_safety.py`.                   |
| `BIR-Q03` | P0       | Partial | Add collaboration conflict safety.       | Stale parent revisions now have a deterministic `revision_conflict` decision with current/parent revision, retry-safe flag, and rebase guidance in the shared safety contract. Evidence: `app/bim_ai/transaction_safety.py`, `app/tests/test_transaction_safety.py`. |
...
| `BIR-Q05` | P0       | Partial | Require dry-run for agent remediation.   | Agent/MCP commit safety now blocks without successful dry-run evidence matching the current parent revision and exact command digest. Evidence: `app/bim_ai/transaction_safety.py`, `app/tests/test_transaction_safety.py`.                                          |
| `BIR-Q06` | P1       | Partial | Add audit provenance for fixes.          | Added `agentRemediationProposal_v1` provenance rows requiring source finding id, affected elements, before/after summary, actor identity, and evidence path. Evidence: `app/bim_ai/transaction_safety.py`, `app/tests/test_transaction_safety.py`.                   |
| `BIR-Q07` | P1       | Partial | Add permission/scope metadata.           | Added permission-scope inference for mutation, export, external-service, and destructive command bundles so MCP clients can request explicit approval. Evidence: `app/bim_ai/transaction_safety.py`, `app/tests/test_transaction_safety.py`.                         |
...
| ID        | Priority | Status  | Item                                           | Acceptance                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------- | -------- | ------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BIR-S01` | P0       | Partial | Validate project/site coordinate systems.      | `siteGeoreferencingIntegrityReport_v1` now emits deterministic project coordinate-system summaries/findings for project base point, survey point, internal origin, true north, georeference presence, and level datum exportability. Evidence: `app/bim_ai/site_georeferencing_integrity.py`, `app/tests/test_site_georeferencing_integrity.py::test_project_coordinate_system_summary_is_exportable_when_datums_are_explicit`. |
| `BIR-S02` | P0       | Partial | Validate linked model transforms.              | Link/import transform diagnostics now check `link_model`, `link_dxf`, and `link_external` rows for origin alignment mode, translation/rotation, units, source metadata, stale/unloaded state, and missing host base/survey anchors for project-origin/shared-coordinate links. Evidence: `app/tests/test_site_georeferencing_integrity.py::test_missing_coordinate_datums_and_shared_coord_link_are_reported`.                  |
| `BIR-S03` | P1       | Partial | Add import diagnostic contract.                | Added `importDiagnosticContract_v1` normalizer/report for unsupported products, lost geometry, category mapping fallback, transform drift, material/type fallback, and unit normalization with deterministic severity/category counts and BIR tracker tags. Evidence: `app/tests/test_site_georeferencing_integrity.py::test_import_diagnostic_contract_normalizes_categories_and_tracker_items`.                               |
| `BIR-S04` | P1       | Partial | Add roundtrip drift checks.                    | Added `roundtripDriftReport_v1` for source/readback element count, placement, category, type, material, and geometry point-count drift with configurable placement tolerance. Evidence: `app/tests/test_site_georeferencing_integrity.py::test_roundtrip_drift_report_detects_counts_placement_category_and_material`.                                                                                                          |
| `BIR-S05` | P1       | Partial | Validate site/toposolid/building relationship. | Site relationship diagnostics now flag missing site/toposolid rows, invalid wall `siteHostId`, building footprints outside all toposolid boundaries, and property-line closure errors. Evidence: `app/tests/test_site_georeferencing_integrity.py::test_site_relationship_diagnostics_cover_toposolid_hosts_and_property_lines`.                                                                                                |
...
| ID        | Priority | Status  | Item                              | Acceptance                                                                                                                                                             |
| --------- | -------- | ------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BIR-W01` | P0       | Partial | Define fixture classes.           | Fixture corpus distinguishes minimal synthetic, target-house regression, benchmark seed, import/export roundtrip, performance stress, disposable local evidence/rehearsal, and user-realistic sketch cases. Seed-hygiene coverage now includes the disposable local evidence class; remaining fixture classes still need complete ownership rows. |
...
| `BIR-S04` | `app/bim_ai/site_georeferencing_integrity.py`                                                                          | `app/tests/test_site_georeferencing_integrity.py`                                                                                                                                                                     | `roundtripDriftReport_v1`                                                                                                                                                                      | Wave 7 Worker D local commit                                      | Report is source/readback-data based; IFC/glTF route integration remains future work.                                                                                              |
| `BIR-S05` | `app/bim_ai/site_georeferencing_integrity.py`; `app/bim_ai/domain_integrity.py`                                        | `app/tests/test_site_georeferencing_integrity.py`                                                                                                                                                                     | `siteGeoreferencingIntegrityReport_v1.siteRelationships`                                                                                                                                       | Wave 7 Worker D local commit                                      | Drainage and exterior stair/rail semantics still need deeper domain-specific rules.                                                                                                |
...
| `BIR-P01` | `app/bim_ai/model_integrity.py`                                                                                        | `app/tests/test_model_integrity_invariants.py`                                                                                                                                                                        | `modelIntegrityInvariantContract_v1` unit/type/schema contract fields                                                                                                                          | Wave 7 Worker A local commit                                      | Contract is machine-readable but not yet generated as a standalone ledger file.                                                                                                    |
...
| `BIR-P08` | `app/bim_ai/model_integrity.py`                                                                                        | `app/tests/test_model_integrity_invariants.py`                                                                                                                                                                        | `modelIntegritySmokeCommandEvidence_v1` CLI/API evidence and digest                                                                                                                            | Wave 7 Worker A local commit                                      | Evidence helper exists; dedicated REST/CLI endpoint wiring remains follow-up.                                                                                                      |
| `BIR-Q01` | `app/bim_ai/transaction_safety.py`                                                                                     | `app/tests/test_transaction_safety.py`                                                                                                                                                                                | `transactionSafety_v1` parent-revision decisions                                                                                                                                               | Wave 7 Worker B local commit                                      | Pure contract only; route/CLI/MCP integration remains follow-up.                                                                                                                   |
| `BIR-Q02` | `app/bim_ai/transaction_safety.py`                                                                                     | `app/tests/test_transaction_safety.py`                                                                                                                                                                                | `undoRedoContract_v1` validation result                                                                                                                                                        | Wave 7 Worker B local commit                                      | Does not replace existing undo stack persistence.                                                                                                                                  |
| `BIR-Q03` | `app/bim_ai/transaction_safety.py`                                                                                     | `app/tests/test_transaction_safety.py`                                                                                                                                                                                | `revision_conflict` decision payload                                                                                                                                                           | Wave 7 Worker B local commit                                      | Conflict detection is contract-level until all mutation routes call it.                                                                                                            |
...
| `BIR-Q05` | `app/bim_ai/transaction_safety.py`                                                                                     | `app/tests/test_transaction_safety.py`                                                                                                                                                                                | dry-run evidence digest guard                                                                                                                                                                  | Wave 7 Worker B local commit                                      | CLI/MCP command entrypoints still need to wire the shared guard.                                                                                                                   |
| `BIR-Q06` | `app/bim_ai/transaction_safety.py`                                                                                     | `app/tests/test_transaction_safety.py`                                                                                                                                                                                | `agentRemediationProposal_v1` provenance rows                                                                                                                                                  | Wave 7 Worker B local commit                                      | Evidence paths are validated as strings; artifact existence is a future gate.                                                                                                      |
| `BIR-Q07` | `app/bim_ai/transaction_safety.py`                                                                                     | `app/tests/test_transaction_safety.py`                                                                                                                                                                                | required permission scope inference                                                                                                                                                            | Wave 7 Worker B local commit                                      | Approval prompting remains client/MCP integration work.                                                                                                                            |
