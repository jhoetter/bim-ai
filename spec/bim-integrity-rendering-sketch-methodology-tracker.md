...
| `BIR-L02` | P0       | Partial | Profile renderer update cost.       | Pure renderer cost profiling now estimates orbit, select, lens-switch, Advisor toggle, and update workloads with budgets, dominant factors, and budget diagnostics. W13-A adds a Playwright live-browser capture/validation hook for target-house orbit, selection, lens switching, and Advisor open/close metrics. Remaining: collect and archive accepted real browser timing samples from target-house and benchmark models. |
| `BIR-L03` | P0       | Partial | Investigate WebSocket proxy errors. | W6-C classifies Vite proxy `EPIPE`/`ECONNRESET` as benign dev reconnect/browser teardown noise, keeps unexpected proxy errors actionable, and covers app reconnect/backoff/state-churn decisions in `packages/web/src/lib/wsStability.test.ts`. W13-A adds machine-readable live evidence rows and proxy-log parsing for WebSocket churn in `packages/web/scripts/target-house-live-responsiveness.mjs`. Remaining: wire helper into dirty WebSocket consumers/proxy config once parallel edits settle and verify live dev-server behavior. |
...
| `BIR-N07` | P1       | Partial | Verify performance on target-house.                  | Deterministic target-house performance evidence now profiles orbit, selection, lens switching, and Advisor opening from the final seed snapshot with renderer-cost budgets. W13-A adds `pnpm --filter @bim-ai/web evidence:target-house-live -- --url <web-url>` to capture/validate live browser timing for orbit, selection, lens switch, Advisor open, and Advisor close. Remaining closeout requires an accepted archived browser run.                                                                                                                                       |
...
| `BIR-N10` | P0       | Partial | Require final package readiness.                     | `node scripts/target-house-final-package.mjs --seed target-house-1` produces a manifest with fresh evidence, clean performance evidence, clean clean-pass gate, clean geometry diagnostic, and clean acceptance gates. It remains Partial because `--require-ready` still blocks on `tracker_not_done`; this is intentional until the exhaustive BIM integrity tracker is complete.                                                                                                                                   |
...
| `BIR-O02` | P0       | Partial     | Add target-house regression fixture.          | The known detached-door/access-wall and roof-cut cases fail before fixes and pass after. Reduced target-house regression cases now cover detached access-wall/access-door and roof cut outside/inside host footprint. |
...
| ID        | Priority | Status  | Item                                     | Acceptance                                                                                                                                                                                                                                                           |
| --------- | -------- | ------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BIR-Q01` | P0       | Partial | Protect transaction boundaries.          | `transactionSafety_v1` checks require parent revisions and are now returned by CMD-v3 bundle and raw command dry-run/commit responses; explicit agent/MCP bundle commits are gated before mutation. Evidence: `app/bim_ai/transaction_safety.py`, `app/bim_ai/routes_api.py`, `app/bim_ai/routes_commands.py`, `app/tests/test_transaction_safety.py`. |
| `BIR-Q02` | P0       | Partial | Preserve undo/redo semantics.            | Added `undoRedoContract_v1` validation for inspectable forward commands, inverse commands, changed ids, undo availability metadata, and advancing revisions; commit route metadata now embeds transaction safety alongside existing undo rows. Evidence: `app/bim_ai/transaction_safety.py`, `app/bim_ai/routes_api.py`, `app/bim_ai/routes_commands.py`, `app/tests/test_transaction_safety.py`. |
| `BIR-Q03` | P0       | Partial | Add collaboration conflict safety.       | Stale parent revisions now have deterministic `revision_conflict` decisions with current/parent revision, retry-safe flag, and rebase guidance; CMD-v3 bundle route returns the decision as structured API detail before mutation. Evidence: `app/bim_ai/transaction_safety.py`, `app/bim_ai/routes_api.py`, `app/tests/test_transaction_safety.py`. |
...
| `BIR-Q05` | P0       | Partial | Require dry-run for agent remediation.   | Agent/MCP commit safety now blocks without successful dry-run evidence matching the current parent revision and exact command digest; dry-run routes emit deterministic `dryRunEvidence_v1` digests for replay. Evidence: `app/bim_ai/transaction_safety.py`, `app/bim_ai/routes_api.py`, `app/bim_ai/routes_commands.py`, `app/tests/test_transaction_safety.py`. |
| `BIR-Q06` | P1       | Partial | Add audit provenance for fixes.          | Added `agentRemediationProposal_v1` provenance rows requiring source finding id, affected elements, before/after summary, actor identity, and evidence path; bundle/command commit metadata now carries the shared safety decision for audit consumers. Evidence: `app/bim_ai/transaction_safety.py`, `app/bim_ai/routes_api.py`, `app/bim_ai/routes_commands.py`, `app/tests/test_transaction_safety.py`. |
...
| `BIR-S05` | P1       | Partial | Validate site/toposolid/building relationship. | Site relationship diagnostics now flag missing site/toposolid rows, invalid wall/building toposolid hosts, degenerate toposolid boundaries, invalid toposolid `siteId`, toposolids outside site boundaries, building footprints outside referenced/all toposolids, and property-line closure errors. Evidence: `app/tests/test_site_georeferencing_integrity.py::test_invalid_site_toposolid_relationships_are_blocking`. |
...
| ID        | Priority | Status      | Item                                    | Acceptance                                                                                                                                  |
| --------- | -------- | ----------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `BIR-V01` | P0       | Partial     | Validate family/type parameter schemas. | `familyTypeContentIntegrity_v1` and model-integrity findings now validate declared required dimensions, host behavior, material slots, schedule fields, IFC mapping, and render support for family types. Remaining: migrate bundled/live family catalogs to strict complete schemas. |
| `BIR-V02` | P0       | Partial     | Validate instance overrides.            | Family instance overrides are checked against declared parameter min/max/options/instance-overridable constraints, wall-host width/height geometry, and schedule-field coverage. Remaining: broaden schedule parity beyond declared `scheduleFields`.                         |
...
| `BIR-B07` | `app/bim_ai/model_integrity_hosting.py`; `packages/web/src/advisor/advisorViolationContext.ts`                         | `app/tests/test_model_integrity_hosting.py`; `packages/web/src/viewport/collectRendererDiagnostics.test.ts`; `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_model_integrity_hosting.py app/tests/test_model_integrity_commit_preflight.py` | Hosted-opening findings include safe existing-command quick fixes for helper/proxy deletion and span resizing; physical wall support-context findings now include a machine-readable `allowDetached` metadata hint. | Wave 14-B local commit; Wave 16-A local commit | Partial: hints are limited to existing safe commands and explicit detached metadata; rehost, move-into-envelope, and support-creation commands remain future work. |
| `BIR-C06` | `app/bim_ai/model_integrity_hosting.py`                                                                                | `app/tests/test_model_integrity_hosting.py::test_opening_conflict_graph_is_deterministic_for_overlap_and_clearance`                                                                                                  | `hostedOpeningConflictGraph_v1` nodes/edges cover wall-hosted intervals, overlap edges, endpoint-clearance edges, and invalid host states.                                                     | Wave 14-B local commit                                           | Lintel/header spacing and richer structural capacity checks remain future work.                                                                                                     |
| `BIR-C07` | `app/bim_ai/model_integrity_hosting.py`; `app/bim_ai/constraints_metadata.py`; `packages/web/src/advisor/advisorViolationContext.ts` | `app/tests/test_model_integrity_hosting.py::test_hosted_family_support_classification_flags_wrong_host_and_orphan_proxy`; `app/tests/test_constraints_metadata.py`                                                    | Advisor/API violations for declared wall/face/ceiling/workplane support classes with missing or unsupported host classes.                                                                        | Wave 14-B local commit                                           | Classification currently uses explicit family/asset parameters plus door/window defaults; full authoring schema enforcement remains open.                                           |
| `BIR-C08` | `app/bim_ai/model_integrity_hosting.py`; `packages/web/src/viewport/collectRendererDiagnostics.ts`                     | `app/tests/test_model_integrity_hosting.py`; `packages/web/src/viewport/collectRendererDiagnostics.test.ts`                                                                                                           | Backend `hosted_render_proxy_orphan` integrity findings and renderer `detached_or_proxy_render_risk` diagnostics carry `BIR-C08` evidence.                                                     | Wave 14-B local commit                                           | Renderer-side coverage is wall-hosted cut diagnostics; non-wall hosted family proxy diagnostics remain partial.                                                                     |
...
| `BIR-B03/B04` | `app/bim_ai/engine_commit.py`; `app/bim_ai/model_integrity_hosting.py`                                             | `app/tests/test_model_integrity_commit_preflight.py`                                                                                                                                                                   | Commit-time `hosted_opening_integrity_violations` blocks physical helper/access proxy leakage and helper-hosted doors before persistence.                                                       | Wave 14-A local commit                                            | Covers hosted-opening/helper-access P0 integrity at the kernel commit boundary; broader detached support context and explicit helper serialization semantics remain follow-up.       |
| `BIR-B02` | `app/bim_ai/model_integrity_hosting.py`; `app/bim_ai/commands.py`; `app/bim_ai/engine_dispatch_core.py`; `app/bim_ai/engine_dispatch_building_envelope.py` | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_model_integrity_hosting.py app/tests/test_model_integrity_commit_preflight.py`; `python -m ruff check app/bim_ai/commands.py app/bim_ai/engine_commit.py app/bim_ai/engine_dispatch_core.py app/bim_ai/engine_dispatch_building_envelope.py app/bim_ai/model_integrity_hosting.py app/bim_ai/constraints_metadata.py app/tests/test_model_integrity_hosting.py app/tests/test_model_integrity_commit_preflight.py` | Commit preflight now blocks physical walls outside the level floor/building support context unless `allowDetached` or detached authoring intent is explicit; the finding includes a machine-readable `set_element_prop allowDetached` hint. | Wave 16-A local commit | Partial: wall support-context authoring is covered through backend commit/bundle surfaces; stairs, rails, assets, and slabs still need equivalent deterministic authoring guards. |
...
| `BIR-B06` | `app/bim_ai/engine_commit.py` | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_model_integrity_commit_preflight.py`; `python -m ruff check app/bim_ai/engine_commit.py` | Agent-authored command payloads marked with `agentAuthored`, `actor: agent`, `source: agent`, or `agentTrace` are rejected unless they provide explicit level/host/type-or-material/category context and `physicalRole` of `physical` or `analysis` where relevant. | Wave 16-A local commit | Partial: raw command and bundle commit paths now have strict agent validation; CLI/MCP generators still need to stamp `agentAuthored` or provide complete explicit defaults consistently. |
| `BIR-H02` | `claude-skills/sketch-to-bim/sketch_bim.py`; `app/bim_ai/integrity_preflight.py`; `app/bim_ai/routes_integrity.py`; `app/bim_ai/api/registry.py`; `packages/cli/cli.mjs` | `packages/cli/sketchSkillHelper.smoke.test.mjs`; `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_integrity_preflight.py app/tests/test_api_v3_registry.py -q`; `node --check packages/cli/cli.mjs` | `agent-loop-packet.json` plus `integrityPreflightReport_v1` link findings to rule ids, severity, recommendations, affected ids, fix hints, remediation proposals, diagnostics, and CLI/API/MCP descriptors. | Wave 13-E local commit; Wave 16-B local commit | Partial: product CLI/API/MCP payload parity is stronger for integrity preflight; broader normal Advisor UI/action parity remains split across H01/H05/U02. |
...
| `BIR-H04` | `app/bim_ai/integrity_preflight.py`; `app/bim_ai/routes_integrity.py`; `packages/cli/cli.mjs`; `app/bim_ai/transaction_safety.py` | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_integrity_preflight.py -q`; `node --check packages/cli/cli.mjs` | `integrityRemediationLoop_v1`, `agentRemediationProposal_v1`, CLI `--dry-run-fixes` / `--commit-fixes`, existing bundle dry-run/commit routes, and recaptured preflight evidence | Wave 16-B local commit | Partial: loop is deterministic and uses existing bundle commit surfaces; accepted-fix UI and artifact storage for recaptured evidence remain follow-up. |
...
| `BIR-D04` | `app/bim_ai/room_access_integrity.py`; `app/bim_ai/domain_integrity.py`                                                | `app/tests/test_room_access_integrity.py::test_door_inside_room_but_not_on_boundary_does_not_create_access_path`; `app/tests/test_room_access_integrity.py::test_open_room_separation_only_access_is_reported_without_blocking_open_plan` | `roomAccessIntegritySmoke_v1` findings for invalid door room-boundary evidence and analytical room-separation-only access                                                                    | Wave 13-C local commit                                           | Open room-separation adjacency remains a warning-only signal so intentional open-plan layouts are not blocked.                                                                      |
| `BIR-D05` | `app/bim_ai/room_access_integrity.py`; `app/bim_ai/domain_integrity.py`                                                | `app/tests/test_room_access_integrity.py::test_unresolved_egress_path_is_reported_for_isolated_accessible_room`; `app/tests/test_room_access_integrity.py::test_multilevel_room_graph_reaches_exterior_through_stair_and_landing_doors` | Egress graph findings in normalized domain-integrity output                                                                                                                                   | Wave 13-C local commit                                           | Egress remains deterministic and geometry-based; full code-profile occupancy egress sizing is separate profile work.                                                               |
| `BIR-D06` | `app/bim_ai/room_access_integrity.py`; `app/bim_ai/domain_integrity.py`                                                | `app/tests/test_room_access_integrity.py::test_room_wall_topology_gap_detects_partial_edge_coverage`; `app/tests/test_room_access_integrity.py::test_room_wall_topology_gap_requires_wall_or_explicit_separator`     | Room wall topology interval-coverage findings with affected room ids                                                                                                                           | Wave 13-C local commit                                           | Topology coverage accepts walls and explicit room separations; richer wall role classification remains future D05/D06 hardening.                                                   |
| `BIR-D03` | `app/bim_ai/room_access_integrity.py`; `app/bim_ai/domain_integrity.py`; `app/bim_ai/constraints_metadata.py`          | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_p0_integrity_fixture_corpus.py app/tests/test_room_access_integrity.py -q`                                                                                  | `room_containment_detached_island` and `room_containment_outside_floor_slab` findings in `p0IntegrityFixtureCorpus_v1`                                                                          | Wave 16-C local commit                                           | Partial: validates room containment against same-level floor slabs with explicit extension intent; richer envelope-zone union/holes and authoring-command preflight remain follow-up. |
...
| `BIR-O02` | `app/tests/fixtures/p0_integrity_cases.json`; `app/tests/test_p0_integrity_fixture_corpus.py`; `app/tests/test_model_integrity_advisor_integration.py` | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_p0_integrity_fixture_corpus.py app/tests/test_model_integrity_advisor_integration.py -q`                                                                    | Reduced target-house detached access-wall/access-door and roof-cut outside/inside-host regression cases                                                                                         | Wave 16-C local commit                                           | Partial: reduced fixtures cover known symptoms; full seed replay and renderer visual roof-cut golden coverage remain separate target-house evidence work.                           |
| `BIR-W01` | `app/tests/fixtures/p0_integrity_cases.json`; `app/tests/test_p0_integrity_fixture_corpus.py`                         | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_p0_integrity_fixture_corpus.py::test_p0_integrity_fixture_corpus_classes_are_explicit_and_auditable -q`                                                    | Fixture-class metadata for minimal synthetic, target-house regression, benchmark seed, import/export roundtrip, performance stress, disposable local evidence/rehearsal, and user-realistic sketch | Wave 16-C local commit                                           | Partial: class vocabulary is explicit and audited here; other fixture corpora still need complete ownership metadata.                                                               |
...
| `BIR-I02` | `packages/web/src/viewport/collectRendererDiagnostics.ts`; `packages/web/src/viewport/roomVisualizationRenderDiagnostics.ts`; `packages/web/src/viewport/wallHostedCutRenderDiagnostics.ts`; `packages/web/src/viewport/verticalCirculationRenderDiagnostics.ts` | `packages/web/src/viewport/collectRendererDiagnostics.test.ts`; `packages/web/src/viewport/wallHostedCutRenderDiagnostics.test.ts`; `packages/web/src/viewport/verticalCirculationRenderDiagnostics.test.ts` | Structured viewport diagnostics for room overlays, room separations, slab openings, stair penetrations, and hosted wall-cut fallbacks | Wave 13-D local commit                                            | Pure diagnostics/goldens only; live UI display and CLI evidence-package refresh remain broader renderer-evidence work.                                                             |
| `BIR-I03` | `packages/web/src/viewport/collectRendererDiagnostics.ts`; `seed-artifacts/target-house-1/evidence/live-run-current/snapshot.json` | `packages/web/src/viewport/collectRendererDiagnostics.test.ts`                                                                                                                                                         | Target-house renderer golden asserts rooms/room separations/slab openings are clean and hosted curtain-wall cuts emit structured fallback diagnostics | Wave 13-D local commit                                            | This prevents silent target-house-critical drops in the covered kinds; roof/loggia/material/sheet visual goldens remain tracked separately.                                         |
| `BIR-I04` | `packages/web/src/viewport/collectRendererDiagnostics.ts`; `packages/web/src/viewport/rendererDiagnostics.ts`           | `packages/web/src/viewport/collectRendererDiagnostics.test.ts`; `packages/web/src/viewport/rendererDiagnostics.test.ts`                                                                                                | Diagnostics carry tracker IDs, view IDs, evidence context, artifact path, and support-matrix digest for acceptance consumption                                                                   | Wave 13-D local commit                                            | Does not close sketch-acceptance enforcement by itself; phase/final acceptance wiring remains Partial.                                                                              |
...
| `BIR-N07` | `scripts/target-house-final-package.mjs`; `packages/web/src/viewport/rendererCostProfile.ts`; `packages/web/scripts/target-house-live-responsiveness.mjs` | `scripts/target-house-final-package.test.mjs`; `packages/web/src/viewport/rendererCostProfile.test.ts`; `packages/web/scripts/target-house-live-responsiveness.test.mjs`                                             | generated `target-house-1-performance-evidence.json`; `target-house-live-responsiveness.json` from `pnpm --filter @bim-ai/web evidence:target-house-live -- --url <web-url>` or `--input <evidence.json>` | Wave 8 Worker E local commit; Wave 13-A local commit              | Deterministic profile evidence is accepted; live browser traces now have a capture/validation hook but still require an accepted archived target-house run for final closeout.       |
...
| `BIR-S05` | `app/bim_ai/site_georeferencing_integrity.py`; `app/bim_ai/domain_integrity.py`                                        | `app/tests/test_site_georeferencing_integrity.py`                                                                                                                                                                     | `siteGeoreferencingIntegrityReport_v1.siteRelationships` with blocking invalid toposolid/site/host relationships                                                                               | Wave 15 Worker D local commit                                     | Partial: site/toposolid/building relationship enforcement is deeper; drainage and exterior stair/rail semantics still need domain-specific rules.                                  |
...
| `BIR-T02` | `claude-skills/sketch-to-bim/sketch_bim.py`; `claude-skills/sketch-to-bim/tools.json`                                    | `packages/cli/sketchSkillHelper.smoke.test.mjs`; `pnpm --filter @bim-ai/cli exec node --test sketchSkillHelper.smoke.test.mjs`                                                                                         | `sketch-to-bim.agent-loop-packet.v1` records `findingId`, affected elements, bundle source command refs, optional command-log transaction refs, recipe/bundle line hits, phase packet, and next actions. | Wave 13-E local commit                                            | Traces findings to available source artifacts; historical agent wave/commit attribution depends on command-log/evidence metadata being present.                                    |
...
| `BIR-O04` | `spec/sketch-to-bim-agent-workflow-templates.md`; `claude-skills/sketch-to-bim/sketch_bim.py`; `scripts/verify-sketch-seed-artifacts.mjs` | `node --test scripts/verify-sketch-seed-artifacts.test.mjs`; `node --test scripts/target-house-closeout-report.test.mjs`                                                                                              | Rehearsal gate checklist requires integrity, renderer diagnostics, Advisor, evidence freshness, sketch acceptance, and non-committing dry-run closeout.                                        | Wave 16-E local commit                                            | Partial: recognized rehearsal workflow is documented, but no actual end-to-end disposable rehearsal was run in this change.                                                       |
...
| `BIR-V01` | `app/bim_ai/model_integrity.py`; `app/bim_ai/elements.py`; `app/bim_ai/commands.py`                                    | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_model_integrity_invariants.py -q`                                                                                                                           | `familyTypeContentIntegrity_v1`; `model_integrity_family_type_schema_incomplete`; `model_integrity_family_type_required_parameter_missing`; `model_integrity_family_type_parameter_constraint_violation` | Wave 15 Worker E local commit                                     | Partial: strict schema validation exists and is exposed through invariant smoke evidence; bundled/live catalogs still need complete metadata migration.                             |
| `BIR-V02` | `app/bim_ai/model_integrity.py`; `app/bim_ai/elements.py`; `app/bim_ai/commands.py`                                    | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_model_integrity_invariants.py -q`                                                                                                                           | Instance override findings for unknown, non-overridable, min/max-invalid, unscheduled, and wall-host geometry-invalid overrides                                                                 | Wave 15 Worker E local commit                                     | Partial: deterministic override checks are schema-driven; schedule compatibility currently uses declared `scheduleFields`, not derived schedule/export row readback.                |
...
| `BIR-V04` | `app/bim_ai/model_integrity.py`; `app/bim_ai/elements.py`; `app/bim_ai/commands.py`; `seed-artifacts/target-house-1/bundle.json` | `PYTEST_ADDOPTS=--no-cov python -m pytest app/tests/test_family_content_integrity.py app/tests/test_model_integrity_invariants.py -q`; `pnpm --filter @bim-ai/cli exec node generate-target-house-geometry-diagnostic.mjs`; `node scripts/gate-target-house-clean-pass.mjs --evidence-dir seed-artifacts/target-house-1/evidence/live-run-current --out seed-artifacts/target-house-1/evidence/live-run-current/clean-pass-gate.json` | `model_integrity_asset_placement_support_invalid`; `model_integrity_asset_placement_floating`; `model_integrity_asset_placement_embedded_without_intent`; `model_integrity_asset_placement_circulation_overlap`; `placed-bed-upper` moved clear of `main-stair` footprint | Wave 15 Worker E local commit; Wave 16 parent target-house correction | Partial: support/floating/embed/circulation-overlap checks are deterministic for declared supports; oriented footprint/recess geometry and full asset extents need deeper asset geometry data. |
...
| `BIR-Q01` | `app/bim_ai/transaction_safety.py`; `app/bim_ai/routes_api.py`; `app/bim_ai/routes_commands.py`                        | `pytest app/tests/test_transaction_safety.py app/tests/api/test_apply_bundle_route.py --no-cov`                                                                                                                       | `transactionSafety_v1` parent-revision decisions on CMD-v3 bundle route plus raw command/bundle dry-run and commit payload metadata                                                            | Wave 15 Worker B local commit                                     | Partial: explicit agent/MCP bundle commits are gated; broader CLI/MCP tool wrappers still need end-to-end parity evidence.                                                         |
| `BIR-Q02` | `app/bim_ai/transaction_safety.py`; `app/bim_ai/routes_api.py`; `app/bim_ai/routes_commands.py`                        | `pytest app/tests/test_transaction_safety.py app/tests/api/test_apply_bundle_route.py --no-cov`                                                                                                                       | `undoRedoContract_v1` validation result and transaction safety metadata embedded in commit audit payloads                                                                                       | Wave 15 Worker B local commit                                     | Partial: undo/redo persistence remains existing route behavior; dedicated route-level undo/redo safety contract enforcement is still follow-up.                                    |
| `BIR-Q03` | `app/bim_ai/transaction_safety.py`; `app/bim_ai/routes_api.py`                                                         | `pytest app/tests/test_transaction_safety.py app/tests/api/test_apply_bundle_route.py --no-cov`                                                                                                                       | `revision_conflict` decision payload returned as structured CMD-v3 bundle route detail before mutation                                                                                          | Wave 15 Worker B local commit                                     | Partial: raw UI command endpoints still infer current revision for legacy compatibility instead of requiring caller-supplied parent revision.                                      |
...
| `BIR-Q05` | `app/bim_ai/transaction_safety.py`; `app/bim_ai/routes_api.py`; `app/bim_ai/routes_commands.py`                        | `pytest app/tests/test_transaction_safety.py app/tests/api/test_apply_bundle_route.py --no-cov`                                                                                                                       | `dryRunEvidence_v1` digest rows emitted by dry-run surfaces and enforced for explicit agent/MCP bundle commits                                                                                  | Wave 15 Worker B local commit                                     | Partial: product MCP/CLI wrappers still need to pass `actorKind` and replay `dryRunEvidence` through their public tool schemas.                                                    |
| `BIR-Q06` | `app/bim_ai/transaction_safety.py`; `app/bim_ai/routes_api.py`; `app/bim_ai/routes_commands.py`                        | `pytest app/tests/test_transaction_safety.py app/tests/api/test_apply_bundle_route.py --no-cov`                                                                                                                       | `agentRemediationProposal_v1` provenance rows plus commit `transactionSafety` audit metadata                                                                                                    | Wave 15 Worker B local commit                                     | Partial: evidence paths are validated as strings; artifact existence and provenance storage beyond route metadata remain future gates.                                             |
