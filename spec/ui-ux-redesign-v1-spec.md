...
| --------------------------- | --------- | ------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| App shell                   | `partial` | 1 token / primitive | ~20%              | A `Workspace.tsx` exists with header + 3-column grid, but reads as a diagnostic panel stack rather than a Figma/Rayon shell. Top bar mixes branding + dropdowns + "Theme" without clear chrome hierarchy.                    |
...
| Theming                     | `partial` | 1 token / primitive | ~25%              | Two `data-theme` themes exist with shallow tokens (`--background`, `--foreground`, `--surface`, `--border`, `--accent`). No semantic role tokens, no element-category palette, no drafting-color palette, no dark-paper.     |
| Iconography                 | `pending` | 0 sketch            | ~5%               | `lucide-react@0.574` is installed but not imported anywhere. Tools rely on text labels.                                                                                                                                      |
...
| ------------- | ------------------------------------------- | --------- | ------------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **WP-UI-A01** | Design token foundation                     | `partial` | 1 token / primitive | ~25%             | All §9 tokens land in `packages/design-tokens/src/tokens-default.css` + `tokens-dark.css`; tailwind preset extended; no inline hex. |
| **WP-UI-A02** | Iconography (lucide-react)                  | `pending` | 0 sketch            | ~5%              | Every chrome icon comes from lucide; assignments §10.1 implemented; canvas-only inline SVG audited.                                 |
...
| **WP-UI-A07** | App shell — layout grid + breakpoints       | `partial` | 1 token / primitive | ~20%             | §8 grid template at 1024/1280/1600; rail-collapse hotkeys.                                                                          |
| **WP-UI-A08** | Theming (light primary; dark)               | `partial` | 1 token / primitive | ~25%             | §23 theme switch persists; tokens cover both; component code never branches on theme.                                               |
