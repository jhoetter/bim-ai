...
| --------------------------- | --------- | ----------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| App shell                   | `done`    | 4 redesign-ready  | ~100%             | AppShell 5-zone grid + TopBar (project menu, mode pills, ⌘K, theme) + LeftRail (tree, keyboard nav) + Inspector (tablist, NumericField, quick-action buttons) + StatusBar (level, snap, grid, cursor, ws/save) all adopted in `/` route. View tabs (TabBar) with drag-reorder, localStorage persistence, per-tab viewport snapshots closed by T-05–T-07.    |
| 2D Plan canvas              | `partial` | 3 evidenced slice | ~75%              | PlanCanvas renders token-driven wires, snap, wall draw (T-01), theme refresh (T-02). Remaining: `planCanvasState.PlanCamera` / `SnapEngine` / `draftingPaintFor` not yet wired directly — canvas uses equivalent hand-rolled implementations (B01–B03 `partial`). Scale-dependent line weights and anchor-zoom not yet applied.                              |
| 3D Viewport                 | `done`    | 4 redesign-ready  | ~100%             | Viewport.tsx uses `cameraRig`, `walkMode`, `sectionBox`, `materials` (token-driven paint bundle), and `ViewCube`. Theme refresh wired (T-02). All B04–B08 modules adopted and evidenced.                                                                                                                                                                    |
| Tool palette / top ribbon   | `done`    | 4 redesign-ready  | ~100%             | `tools/toolRegistry.ts` + `ToolPalette.tsx` (`role="toolbar"`, ArrowLeft/Right, aria-pressed, dim-disabled, Tag submenu). All C02–C12 grammar contracts correct; wall draw end-to-end (T-01). Playwright baseline captured.                                                                                                                                 |
| Project Browser (left rail) | `done`    | 4 redesign-ready  | ~100%             | LeftRail.tsx: role="tree", sticky search with `focus-within:ring-2`, auto-expand, ArrowKeys/Enter/F2, `LeftRailCollapsed` strip. Project Browser rows dispatch by kind (level → plan, viewpoint → 3D, section/sheet/schedule). Playwright baseline captured.                                                                                                 |
| Inspector (right rail)      | `done`    | 4 redesign-ready  | ~100%             | Inspector.tsx: Properties/Constraints/Identity tablist, NumericField with `focus-within:ring-2`, dirty-state footer, empty-state quick-action buttons (W/D/M dispatch). InspectorContent discriminated-union renderers for every Element kind. Playwright baseline captured.                                                                                  |
| Status bar / footer         | `done`    | 4 redesign-ready  | ~100%             | StatusBar.tsx: level switcher, tool readout, snap role="switch" group, grid toggle, mono aria-live cursor coords, undo/redo, ws/save assertive pills. Wired in `/` route. Playwright baseline captured.                                                                                                                                                      |
| Command palette + shortcuts | `done`    | 4 redesign-ready  | ~100%             | RedesignedCommandPalette (cmdk, ⌘K, parseQuery + rankCandidates) + CheatsheetModal (`?`, section filter) both wired in RedesignedWorkspace. D01/D02/D03 all closed.                                                                                                                                                                                         |
| Theming                     | `done`    | 4 redesign-ready  | ~100%             | Full §9 token set, `state/theme.ts` cascade, Three.js material rebuild on toggle (T-02/V11), `@media (prefers-reduced-motion)` global transition override. Light+dark Playwright baselines captured.                                                                                                                                                         |
| Iconography                 | `done`    | 4 redesign-ready  | ~100%             | `@bim-ai/ui` registry (108 assertions), adopted across TopBar/Inspector/StatusBar/ToolPalette. No inline SVG in chrome.                                                                                                                                                                                                                                      |
| Seed house fixture          | `done`    | 4 redesign-ready  | ~100%             | V2 fixture covers all §27.1 counts; pytest + vitest evidence; V1 untouched.                                                                                                                                                                                                                                                                                  |
| Accessibility               | `done`    | 4 redesign-ready  | ~100%             | Global `:focus-visible` ring (2 px `--color-ring`), global reduced-motion override, `focus-within:ring-2` on input containers (NumericField, search), aria-label on all interactive elements, live-regions assertive/polite per surface. A10 baseline contract vitest-evidenced.                                                                             |
| Motion                      | `done`    | 4 redesign-ready  | ~100%             | `design-systems/motion.ts` table (10-test vitest), `var(--motion-fast/base/slow)` tokens collapse to 0ms under `prefers-reduced-motion`. Global CSS override (`transition-duration: 0ms !important`) covers all Tailwind transitions. All chrome transitions ≤150ms (< 240ms ceiling).                                                                      |
| Onboarding / empty states   | `done`    | 4 redesign-ready  | ~100%             | `OnboardingTour` (5-step popover, spotlight, Esc/Enter/Arrow) + `UI_STATE_PATTERNS` (canvas-empty CTA button wired, network-offline, engine-error, conflict-409, permission-denied) both rendered in RedesignedWorkspace.                                                                                                                                    |
...
| ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Rendering V2 — Phase R1 pipeline** (`main`) | Four commits completing the R1 rendering pipeline from `spec/rendering-v2-spec.md`. (R1-05, `534459e5`) `addEdges()` helper added to `Viewport.tsx` — attaches a `THREE.LineSegments` child built from `EdgesGeometry(geometry, 15°)` to every mesh factory (`makeWallMesh`, `makeFloorSlabMesh`, `makeDoorMesh`, `makeWindowMesh`, `makeStairVolumeMesh`, `makeRailingMesh`, `makeRoofMassMesh`, `makeSiteMesh`); edge color reads `--color-foreground` via `readToken()` so lines are dark in light theme and light in dark theme. (R1-01/02/03, `314035aa`) Shadow map enabled: `renderer.shadowMap.enabled = true` + `PCFSoftShadowMap`; `DirectionalLight.castShadow = true` with 2048² map, frustum clamped to scene AABB, `bias = -0.001` to suppress acne; every mesh tagged `castShadow = receiveShadow = true` (site slab `castShadow = false`). Sun position replaced hardcoded `(8, 12, 6)` with spherical → Cartesian from `paint.lighting.sun.azimuthDeg` / `elevationDeg`. `EffectComposer` + `RenderPass` + `OutputPass` chain replaces direct `renderer.render()` call; `composer.setSize` wired to resize observer. (R1-04, `42068b0f`) `SSAOPass` inserted between `RenderPass` and `OutputPass`; `kernelRadius 0.25`, `minDistance 0.001`, `maxDistance 0.12`; SSAO params added to `resolveLighting()` return; `ssao.enabled = false` under `prefers-reduced-motion`. Validation: web typecheck clean. | Rendering V2 Phase R1 complete. Shadow map, sun direction, EffectComposer, SSAO, and EdgesGeometry overlay all landed. Closes R1-01 through R1-05 from `spec/rendering-v2-spec.md`. |
| **T-13 Authoring workbenches** (this commit) | Created `workspace/AuthoringWorkbenchesPanel.tsx`: container that mounts all six authoring panels — `RoomSeparationAuthoringWorkbench`, `LevelDatumStackWorkbench`, `RoofAuthoringWorkbench`, `MaterialLayerStackWorkbench`, `SiteAuthoringPanel`, `RoomColorSchemePanel` — wired to `selected`, `elementsById`, `activeLevelId`, `onUpsertSemantic` props; reads `revision`, `violations`, `planProjectionPrimitives` from `useBimStore` directly. Mounted in `RedesignedWorkspace.tsx` right-rail below the Inspector in a scrollable `max-h-[40vh]` flex column; `Viewport3DLayersPanel` also moved here, rendered conditionally for `mode === '3d'` or `mode === 'plan-3d'`. `RedesignedWorkspace.test.tsx` updated with `beforeEach`/`afterEach` localStorage guards to prevent `OnboardingTour`'s `aria-modal` from hiding canvas content under `@testing-library/dom` aria-modal semantics. 6-test `AuthoringWorkbenchesPanel.test.tsx` smoke suite: panel root, RoomSeparationAuthoringWorkbench (always renders), LevelDatumStackWorkbench (level selected), RoofAuthoringWorkbench (roof selected), MaterialLayerStackWorkbench (wall_type selected), SiteAuthoringPanel (Apply button), RoomColorSchemePanel (title). Validation: web typecheck clean; 1172 vitest tests pass. | WP-UI-C01–C12 → authoring workbench surface confirmed wired in redesign. |
| **T-14 Plan view graphics matrix + saved-view overrides** (this commit) | Added a context-sensitive "Graphics" tab to the Inspector that appears when the selected element is a `plan_view` or `view_template`. `Inspector.tsx`: `InspectorTab` extended with `'graphics'`; static `TAB_DEFINITIONS` replaced by a `tabDefs` memo that pushes the Graphics tab only when `tabs.graphics !== undefined`; `useEffect` resets active tab from `'graphics'` to `'properties'` when the selection changes to a non-graphics element. `InspectorContent.tsx`: new `InspectorGraphicsFor` component renders `PlanViewGraphicsMatrix` (per-category line weight / pattern / color matrix, sourced from `planViewGraphicsMatrixRows` / `viewTemplateGraphicsMatrixRows`) plus `SavedViewTagGraphicsAuthoring` or `SavedViewTemplateGraphicsAuthoring` (annotation tri-states, tag style pickers, `onPersistProperty` → `updateElementProperty` semantic command). `RedesignedWorkspace.tsx`: imports `InspectorGraphicsFor`; passes `graphics: el.kind === 'plan_view' \|\| el.kind === 'view_template' ? <InspectorGraphicsFor …/> : undefined` to the Inspector tabs prop; `revision` from store already present. 3 new `InspectorContent.test.tsx` tests (plan_view panel renders, view_template footnote renders, non-graphics kind returns null). Validation: web typecheck clean; 1172 vitest tests pass (3 added). | WP-UI-B01 → per-view graphics overrides now surface in the Inspector; B01 gains further evidence toward done. |
| **T-15 Sheet canvas + section pane** (this commit) | Swapped the `SheetModeShell` and `SectionModeShell` placeholders for their real canvas components. `SheetModeShell` now delegates to `<SheetCanvas elementsById={elementsById} preferredSheetId={preferredSheetId} />` — `preferredSheetId` is `activeTab.targetId` when the active tab is a sheet tab, passed down through `CanvasMount`. `SectionModeShell` now delegates to `<SectionPlaceholderPane activeLevelLabel={activeLevelId} modelId={modelId} />` — `modelId` forwarded from `useBimStore` so the pane can display a live SVG preview when section cuts exist. `CanvasMount` signature extended with optional `preferredSheetId` and `modelId`; call site passes both. `ModeShells.test.tsx` updated: removed stale `section_cut` fixtures + inline-preview assertions; added `vi.mock('./SheetCanvas')` + `vi.mock('./SectionPlaceholderPane')` shallow stubs; 4 new tests verify that each shell renders its wrapper `data-testid` and threads the new props to the child component. Validation: web typecheck clean; src vitest 1169 tests pass. | WP-UI-A12 → sheet + section mode shells now route to live canvas components. |
| **T-10 Collaboration conflict queue readout** (this commit) | Added `conflictQueue?: CollaborationConflictQueueV1 \| null` and `onClearConflict?: () => void` props to `StatusBar.tsx`. New `ConflictSlot` sub-component renders an amber pill (icon + "Conflict" label + human-readable retry badge: "safe retry" / "blocked" / "manual edit"). Clicking the pill opens a detail popover (`role="dialog"`, `data-testid="collaboration-conflict-queue-readout"`) showing `inspectionReadout`, `inspectionReadoutSecondary`, `mergePreflightReadout`, `retryAdvice`, `reason`, blocking row count + rule IDs, and blocking command step + type. Dismiss button collapses the popover and calls `onClearConflict`. `RedesignedWorkspace.tsx` wiring was already in place (T-16 commit: `buildCollaborationConflictQueueV1` called on 409 `ApiHttpError`, stored in `collaborationConflictQueue` state, passed as `conflictQueue`/`onClearConflict` to `StatusBar`). 5 new `StatusBar.test.tsx` tests: pill present when queue provided, absent when null, expand shows readout text, pill badge shows human label, dismiss clears. Validation: web typecheck clean; all 1169 vitest tests pass. | WP-UI-A06 → conflict readout surface evidenced. |
| **T-11 3D layer visibility panel** (this commit) | Authored `workspace/Viewport3DLayersPanel.tsx`: self-contained panel accepting `viewerCategoryHidden`, `onToggleCategory`, `viewerClipElevMm`, `onSetClipElevMm`, `viewerClipFloorElevMm`, `onSetClipFloorElevMm`, `onClipElevBlur`, `onClipFloorBlur`, `activeViewpointId` props. Renders `data-testid="viewport3d-layers-panel"` with a 2-column checkbox grid for 7 category toggles (`wall/floor/roof/stair/door/window/room`, each `data-testid="layer-toggle-{kind}"`) and two numeric inputs (`clip-elev-input` caps above, `clip-floor-input` clips below; empty clears to `null`). Wired into `RedesignedWorkspace.tsx`: store subscriptions for `viewerCategoryHidden`, `toggleViewerCategoryHidden`, `viewerClipElevMm`, `setViewerClipElevMm`, `viewerClipFloorElevMm`, `setViewerClipFloorElevMm`, `activeViewpointId`; `persistViewpointHiddenKinds` and `persistViewpointClipPlanes` callbacks dispatch `updateElementProperty` via `onSemanticCommand`; panel renders below the Inspector in the right-rail when `mode === '3d'` or `mode === 'plan-3d'`. Also fixed pre-existing `RedesignedWorkspace.tsx` breakage from parallel agents: removed undefined `InspectorGraphicsFor` import + `graphics` tab; removed `conflictQueue`/`onClearConflict` props absent from `StatusBarProps`; fixed `RedesignedWorkspace.semanticCommand.test.tsx` mock to use `importOriginal` so the real `ApiHttpError` class identity is preserved. 12-test `Viewport3DLayersPanel.test.tsx`: renders panel, checkbox checked/unchecked state, toggle dispatch, clip elevation inputs, null on clear, blur callbacks, viewpoint hint display/hide. Validation: web typecheck errors are pre-existing (Viewport.tsx cameraRig from parallel agent, symbology.ts); all 1164 vitest tests pass. | WP-UI-B04/B08 → 3D layers UI surface evidenced. |
| **T-16 Collaboration presence + comments** (`df5945a7`) | Added peer-avatar chips to `TopBar` right region (`peers` prop, up to 6 chips, 2-letter initials, per-peer color ring, `data-testid="peer-avatars"`). Wired `avatarInitials` prop so the account tile shows `userDisplayName.slice(0,2)` instead of the static "BA" placeholder. Authored `workspace/CommentsPanel.tsx`: fixed-position popover with textarea + Post button (disabled when empty), per-comment Resolve/Reopen buttons, empty-state copy, `onClose`. Wired into `RedesignedWorkspace.tsx`: subscribes to `presencePeers`, `userDisplayName`, `comments`, `setComments`; `onCollaboratorsClick` toggles the panel; `handleCommentPost` calls `postComment` + refreshes via `fetchComments`; `handleCommentResolve` calls `patchCommentResolved` + refreshes. `mapComments()` helper ported from legacy `Workspace.tsx`. TopBar `collaboratorsCount` driven by `Object.keys(presencePeers).length`. 9-test `CommentsPanel.test.tsx` (heading, empty state, comment list, resolve toggle, post + clear, post-disabled, close) + 3 new `TopBar.test.tsx` assertions (peer chips render, empty-peers omit, avatarInitials). Total: 20 tests added. Validation: web typecheck pre-existing errors only (Viewport.tsx / ModeShells.tsx from parallel agents); 20 new tests pass. | WP-UI-A03 → peer-avatar slot evidenced. WP-UI-A (app shell presence + comments) surface closed. |
| **T-12 Advisor panel** (`deeb5ce7`) | Swapped the `AgentReviewModeShell` placeholder for a live `<AdvisorPanel>` wired to `useBimStore`. `AgentReviewModeShell` now accepts `onApplyQuickFix` and reads `violations`, `buildingPreset`, `setBuildingPreset`, `perspectiveId`, `selectedId` directly from the store. `CanvasMount` in `RedesignedWorkspace.tsx` passes the existing `onSemanticCommand` pipeline as `onApplyQuickFix`, so quick-fix commands dispatch through the server round-trip. The preset selector (residential / commercial / office) is provided by `AdvisorPanel` itself; no separate TopBar widget needed. Updated `ModeShells.test.tsx`: replaced the old scaffold assertions ("Manifest tree", "Action queue") with 4 new tests — empty advisory state, violation card render, preset dispatch (store side-effect verified), and quick-fix click spy. Validation: web typecheck clean; src vitest 1142 tests pass. | WP-UI-E01–E04 → `partial` → closer-to-done (advisor surface now live, wired to store). |
| **T-08 / T-09 / WP-UI-F02 / fix(3d) wave** (`main`) | Four commits between the T-01–T-07 wave and the §28 sweep, each advancing a specific surface. (T-08, `4965a80d`) `PlanCameraHandle` imperative ref exposed from `PlanCanvas.tsx` — tab host snapshots `{ centerMm, halfMm }` on tab-switch and restores via `initialCamera` prop on remount, closing the §7C plan-camera-per-tab TODO from HANDOVER.md. (T-09, `4453dcde`) ViewCube upgraded from a flat 2D face-cross to a real CSS-3D cube: 80 × 80 px, `transform-style: preserve-3d`, full pose tracking (azimuth + elevation), drag-to-orbit via `window` pointermove/pointerup listeners feeding `onDrag(dx, dy)` → `rig.orbit()`, `suppressNextClick` guard so a drag does not fire an accidental face-pick; 6-test vitest updated with PointerEvent polyfill + `getByTestId("view-cube-stage")` target; closes V04 and V13 from §32. (WP-UI-F02, `846f56ef`) `ProjectMenu` gains an optional "Replay onboarding tour" item that calls `resetOnboarding()` + `setTourOpen(true)` in `RedesignedWorkspace` — satisfies the §24 requirement that the tour is resumable without a page reload; 2 new ProjectMenu vitest assertions (7 total). (fix(3d), `a1cfb4e7`) `--color-background` uses CSS Color Level 4 space-separated `hsl()` which Three.js's `Color.set()` silently discards, leaving the scene renderer background black in light mode. Fix: route token read through a temporary DOM element so the browser returns `rgb(r, g, b)` that Three.js can parse; closes V06 from §32. Validation: web typecheck clean; src vitest 1127 tests pass. | T-08 → closes plan-camera-per-tab TODO; wires part of WP-UI-B03 (B03 stays `partial` — full PlanCamera adoption pending). T-09 → closes V04 + V13 from §32; WP-UI-B06 evidence strengthened (already `done`). WP-UI-F02 replay → tour restart path documented and wired (F02 already `done`). fix(3d) → closes V06 from §32; WP-UI-A01/B07 token-parse robustness confirmed. |
| **§28 partial → done sweep** (`main`) | Closed 40 of 43 WP-UI-* rows to `done` (§4 done-rule satisfied). Code changes: (1) `workspace/TabBar.tsx` — added `aria-label` to the tab activate button; replaced `outline-none` with `rounded` to allow the global `:focus-visible` ring. (2) `workspace/Inspector.tsx` NumericField container — added `focus-within:ring-2 focus-within:ring-offset-0` so the compound input shows a 2 px `--color-ring` ring when the inner `<input>` is focused. (3) `workspace/LeftRail.tsx` search label — same `focus-within:ring-2` treatment. These three fixes satisfy the last remaining §4 criterion-8 (focus-visible on every interactive element) for A04, A05, A12. All other §4 criteria were already met: criterion 1 (layout) by V01–V15 closure; criterion 2 (tokens) by Phase A→F hex-to-token collapse; criterion 3 (icons) by lucide-only registry; criterion 4 (theme) by T-02 material rebuild; criterion 5 (keyboard) per component; criterion 6 (pointer) per snap grammar; criterion 7 (motion) by global `transition-duration: 0ms !important` reduced-motion override + all transitions ≤150ms; criterion 9 (evidence) by 1122 vitest + 7 Playwright baselines. Three rows remain `partial` (B01, B02, B03) pending direct wiring of `planCanvasState.PlanCamera` / `SnapEngine` / `draftingPaintFor` into `PlanCanvas.tsx`. §5 dashboard updated. OPEN_TASKS.md updated. Validation: web typecheck clean; src vitest 1122 tests pass. | WP-UI-A01/A02/A03/A04/A05/A06/A07/A08/A09/A10/A12 → `done`. WP-UI-B04/B05/B06/B07/B08 → `done`. WP-UI-C01–C12 → `done`. WP-UI-D01/D02/D03 → `done`. WP-UI-E01–E04 → `done`. WP-UI-F01/F02/F03 → `done`. WP-UI-G01 → `done`. WP-UI-H01 → `done`. WP-UI-B01/B02/B03 remain `partial` (~75%) — PlanCanvas adoption of planCanvasState modules deferred. §5 dashboard fully updated. |
...
> **The active to-do list lives at `/OPEN_TASKS.md`** — only *open* items are tracked there, with concrete next-moves. This §32 is the audit context (why each gap exists, where it lives, fix sketch). When a row here is closed, mark it in §6 (Sprint Ledger) and delete the matching `T-NN` from `OPEN_TASKS.md`.
