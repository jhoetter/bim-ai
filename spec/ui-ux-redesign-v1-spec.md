...
| **WP-UI-A03** | App shell — TopBar                          | `partial` | 3 evidenced slice | ~70%             | `packages/web/src/workspace/TopBar.tsx` renders the §11 anatomy (left: hamburger + logo + project-name button with disclosure; center: 7 mode pills traversable via `ArrowLeft`/`ArrowRight` with `role="tablist"`/`role="tab"`/`aria-selected`/`aria-keyshortcuts`; right: ⌘K command palette button, collaborators with badge, settings, theme toggle, avatar). Pulls icons + labels from `@bim-ai/ui` (no inline SVG). `WORKSPACE_MODES` constant is the source of truth for `1`–`7` hotkeys and is consumed by WP-UI-D03. 8 vitest assertions cover render, active state, click + keyboard interaction, theme-icon swap, project-name click, collaborator badge, command-palette button. Workspace.tsx adoption deferred to WP-UI-D03 + adoption sweep.                                                                                                                                                                                                                                                                                                                                   |
| **WP-UI-A04** | App shell — Left rail (Project Browser)     | `partial` | 3 evidenced slice | ~75%             | `packages/web/src/workspace/LeftRail.tsx` is the §12 redesigned project browser: sticky search field on top, uppercase eyebrow section headers, `role="tree"` with `treeitem` rows, disclosure chevrons on parents, ArrowDown/Up/Right/Left + Enter + F2 keyboard model, fuzzy search (label + id + hint) auto-expands matching parents, accent-soft active row, `LeftRailCollapsed` icon strip for the 56 px collapsed state. Kept old `ProjectBrowser.tsx` untouched (engineering-readout panels) — Workspace.tsx adoption defers to the post-Phase-3 sweep. 7-test vitest covers section render, expand-on-click, active state, click + F2 dispatch, ArrowDown/Right/Enter navigation, search filter, collapsed-strip aria-labels.                                                                                                                                                                                                                                                                                                                                                         |
| **WP-UI-A05** | App shell — Right rail (Inspector)          | `partial` | 3 evidenced slice | ~75%             | `packages/web/src/workspace/Inspector.tsx` is the §13 redesigned right rail: header (icon + element type + mono id + close button), `role="tablist"` with Properties / Constraints / Identity tabs (`ArrowLeft`/`ArrowRight` cycling), tab body slot props for caller-supplied content, sticky Apply (⏎) / Reset (Esc) footer that only appears when `dirty=true`, empty-state quick-actions block. `evaluateExpression` exposes the §13.3 numeric grammar (`+`, `-`, `*`, `/`, parens) sandboxed via a strict regex; `<NumericField>` is a primitive that combines mm-canonical state with a click-to-cycle `mm`/`cm`/`m` unit chip and surfaces invalid expressions via `aria-invalid`. 12-test vitest covers empty state, header + tabs render, click + keyboard tab switching, dirty-state footer, close button, evaluator happy + reject paths, NumericField formatting + unit cycling + commit + invalid path. Workspace.tsx adoption deferred to the post-Phase-3 sweep.                                                                                                               |
| **WP-UI-A06** | App shell — StatusBar                       | `partial` | 3 evidenced slice | ~75%             | `packages/web/src/workspace/StatusBar.tsx` lands the §17 cluster set: active level switcher (popover + PageUp/Down cycling), tool readout, snap-mode group as `role="switch"` toggles with `Magnet` glyph, grid switch (Grid3x3 icon + Grid: ON/OFF), monospace `aria-live="polite"` cursor coordinates, undo cluster (Undo / Redo with depth), ws status pill (`aria-live="assertive"` when offline), save state pill (`aria-live="assertive"` on `error`). 9-test vitest covers cluster render, level popover + PageUp/Down cycling, snap toggle dispatch, grid toggle, ws/save assertive aria-live, undo/redo dispatch, off-canvas cursor placeholder.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **WP-UI-C01** | Tool palette (top-floating)                 | `partial` | 3 evidenced slice | ~70%             | `packages/web/src/tools/toolRegistry.ts` is the single source of truth for §16.1 tool ordering, hotkey labels, lucide icon mapping, per-mode visibility, and `isToolDisabled(tool, ctx)` enablement rules (Floor / Roof / Railing / Dimension all gate on prerequisite elements). `tools/ToolPalette.tsx` renders the top-floating bar (`role="toolbar"`, ArrowLeft/Right cycling, `aria-pressed` per active button, hotkey suffix superscript, `aria-keyshortcuts`, dim-disabled-buttons with reason tooltip) and routes the trailing `Tag ▾` button to a separate `onTagSubmenu` callback. 8 vitest assertions cover plan-mode roster, schedule-mode shrink, render, active state, click + arrow cycling, disabled-floor reason, Tag submenu dispatch.                                                                                                                                                                                                                                                                                                                                      |
...
| **WP-UI-D03** | Mode switching (1–7)                        | `partial` | 3 evidenced slice | ~70%             | `packages/web/src/state/modeController.ts` exposes `MODE_HOTKEYS`, `modeForHotkey(key)` (`1`–`7` mapping), `ModeController` with per-mode `ModeAdapter` capture/restore so a switch round-trips tool / selection / camera per §7. `dragSplitDivider` snaps the §20.3 Plan+3D divider to 33 / 50 / 67 % within a 3 % tolerance, and `applySplitDividerHotkey` covers the `[` (collapse left) / `]` (collapse right) shortcuts. 9-test vitest covers hotkey mapping (positive + null), mode-switch idempotence, capture/restore round-trip, history reset, divider clamp + snap + hotkey collapse.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **WP-UI-H01** | Playwright visual regression baseline       | `partial` | 2 composed slice  | ~50%             | `packages/web/src/design-systems/visualRegressionBaselines.ts` is the manifest of every surface that needs a screenshot baseline (§8 / §11 / §12 / §13 / §14 / §15 / §16 / §17 — light + dark for the shell). `packages/web/e2e/ui-redesign-baselines.spec.ts` wires the Playwright runner with one `test.fixme` placeholder per surface; the fixme flag holds the harness in place until the redesigned chrome is composed inside `Workspace.tsx` (post-Phase adoption sweep) and snapshots can be captured. 4-test vitest covers manifest section coverage, viewport invariants, dual-theme app-shell entries, and unknown-id lookup.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
