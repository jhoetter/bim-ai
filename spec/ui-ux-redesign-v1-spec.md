...
| Theming                     | `partial` | 2 composed slice    | ~50%              | Full §9 token set (chrome semantic, drafting palette, element categories, line weights, motion, elevation, shell sizing) lands in `@bim-ai/design-tokens` and is exposed via tailwind preset. Component-side adoption (Workspace, canvas) still pending. |
