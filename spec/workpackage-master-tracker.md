...
**Status (2026-05-07).** `done` in `cf3552d5` — full WP shipped. SelectionSetRule gains optional `linkScope: 'host' | 'all_links' | { specificLinkId }`; ClashResult gains `linkChainA` / `linkChainB`. Python side adds `selection_set` + `clash_test` element kinds plus three commands (`upsertSelectionSet`, `upsertClashTest`, `runClashTest`); `clash_engine.py` resolves rules across links, transforms AABBs by each link's positionMm + rotationDeg, and computes pair-wise distance. UI: SelectionSetPanel adds a Scope dropdown (Host / All links / each link by name); ClashTestPanel renders linked elements as `from <linkName> — <sourceId>` and exposes `onRun` / `onFlyTo` callbacks for live wiring. 11 pytest cases + 6 vitest cases.
...
**Status (2026-05-08).** `done` — both IFC and DXF halves are shipped. The IFC half (commit `6c2ee24f`) parses an IFC STEP via `authoritativeReplay_v0`, materialises a fresh `ModelRecord` in the host's project, and runs `createLinkModel` against the host. The DXF half adds element kind `link_dxf` (Pydantic + core), `app/bim_ai/dxf_import.py` (`ezdxf`-based parser → `DxfLineworkPrim` dicts; `$INSUNITS` auto-scales to mm; lines / polylines / arcs / circles supported, 3D / hatches / text skipped), `CreateLinkDxfCmd` engine command with discriminated-union dispatch, `POST /api/models/{host_id}/import-dxf`, an enabled "Insert → Link DXF…" file picker in `ProjectMenu`, and a desaturated grey `link_dxf` underlay layer drawn beneath authored geometry on the active level in `PlanCanvas.tsx`. New tests: `test_dxf_import.py` (6 cases — lines, polylines, 3DFACE skip, units scaling, arcs, payload builder), `test_create_link_dxf_command.py` (3 cases — engine command apply, missing-level rejection, wire round-trip), `dxfUnderlay.test.ts` (7 cases — line / polyline / arc rendering, transform composition, level filtering). **Deferred:** Revit `.rvt` import remains out of scope (OpenBIM stance — Forge / Speckle path); long-import progress polish is a separate UX WP.
...
| ID     | Item                                                            | Effort | State                                                                                                                                                                                                              | Depends on |
| ------ | --------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| EDT-01 | Universal grip + temp-dimension protocol on plan canvas         | XL     | `done` (wave2-2 protocol + walls + temp-dimensions w/o locks; wave3-7 propagation: per-kind providers in `plan/grip-providers/` for door / window / floor / column / beam / section_cut / dimension / reference_plane + `MoveBeamEndpointsCmd` schema; sketch elements deferred to follow-up if needed)                                                                                  | —          |
| EDT-02 | Constraint locks via padlock UI                                 | M      | `done` (wave4-A — closeout on top of the load-bearing slice. `CreateConstraintCmd` Pydantic in `app/bim_ai/commands.py` + apply branch in `app/bim_ai/engine.py` materialises a `ConstraintElem`; `try_commit_bundle` runs `bim_ai.edt.constraints.evaluate_all` after every command apply and rejects the bundle when any error-severity constraint breaks, surfacing engine `Violation` rows tagged `edt_constraint_violated` with `constraint_id`, `rule`, and `residual_mm`. Padlock chip in `packages/web/src/plan/GripLayer.tsx` swaps open→filled padlock when a matching constraint is found; click handler in `PlanCanvas.tsx` dispatches `createConstraint` capturing the temp-dim's measured distance via `findLockedConstraintFor` (new `packages/web/src/plan/tempDimensionLockState.ts`). `TempDimTarget` now carries `aId` / `bId` for the two referenced walls. TS additions in `packages/core/src/index.ts` — `'constraint'` ElemKind, the constraint Element shape, plus `ConstraintRule` / `ConstraintAnchor` / `ConstraintRefRow` / `CreateConstraintCmd`. Tests: `app/tests/test_create_constraint_command.py`, `packages/web/src/plan/tempDimensionLockState.test.ts`, `packages/web/src/plan/PlanCanvas.padlock.test.tsx`. Non-goals: additional rules (parallel / perpendicular / collinear / equal_length) remain pass-through; no padlock chip in 3D viewport; no soft-lock UX.) | EDT-01     |
| EDT-03 | 3D direct-manipulation handles                                  | L      | `done` (wave-04 closeout — `Grip3dAxis` extended with `'xy' \| 'xyz'` plus optional `visibleIn: 'orbit' \| 'elevation' \| 'all'` flag. New `viewport/grip3dRenderer.ts` builds Three.js sphere + sprite pickables per descriptor (userData round-trips back to the descriptor for raycast hits) and an emissive `LineSegments` axis indicator coloured red/green/blue per axis with `update(deltaMm)` for live drag preview. Six new providers in `viewport/grip3dProviders.ts` — floor (vertex / corner-extrusion / thickness), roof (ridge / eave / gable-end), column (top / base), beam (start / end), door (width / height — elevation-only), window (width / height / sill — elevation-only) — registered at module load. `Viewport.tsx` adds a grip pre-pass: `gripPreRaycast` runs ahead of the element pick, and `projectGripDelta` projects the cursor ray onto the descriptor's axis line (or horizontal plane for free axes) to feed `onDrag` / `onCommit`; commits route through a slice-→-engine dispatcher (`updateElementProperty` / `moveBeamEndpoints`). New `viewport/wallFaceRadialMenu.tsx` opens on right-click with Insert Door / Window / Opening, resolving `alongT` from the world-space hit point. 51 vitest tests across the four files; full `pnpm typecheck` clean. Open follow-ups: free-axis (xy / xyz) drag projection uses planar-magnitude approximation pending UX-aware drag heuristics; door / window grip rendering relies on elevation-view filtering at the renderer.) | EDT-01     |
| EDT-04 | De-stub the 9 plan-canvas tools                                 | M      | `done` (wave3-3 — all 9 stubs replaced with onSemanticCommand; 7 new kernel commands shipped: splitWallAt + alignElementToReference + trimElementToReference + setWallJoinVariant + createColumn + createBeam + createCeiling; ColumnElem/BeamElem/CeilingElem) | EDT-01     |
| EDT-05 | Snap-engine upgrade (intersection / perp / tangent / extension) | M      | `done` (wave4 closeout — `produceTangentSnaps` (closest tangent point on a polyline curve from an active draft anchor), `produceParallelSnaps` (snaps draft direction parallel to a hovered wall, projected through anchor), and `produceWorkplaneSnaps` (perpendicular foot of cursor on the active workplane's plan trace) ship in `packages/web/src/plan/snapEngine.ts` and feed `snapPlanCandidates` via the new `curves` / `workplane` / `draftAnchor` opts. Sort precedence updated to endpoint > intersection > perpendicular > extension > parallel > tangent > workplane > grid > raw. `snapSettings.ts` extends `ToggleableSnapKind` with `parallel` / `tangent` / `workplane` (defaults: parallel ON, workplane ON, tangent OFF — tangent is the most expensive producer) and `applySnapSettings` filters by kind. `SnapGlyphLayer` renders kind-specific glyphs (circle-tangent symbol, ‖ parallel marks, skewed wireframe square for workplane). `snapTabCycle` exports the canonical `SNAP_TAB_CYCLE_ORDER`. wave2-2 — intersection / perpendicular / extension producers, glyph layer, Tab-cycle, per-kind toggles in localStorage. ) | —          |
| EDT-06 | Tool grammar polish (Chain / Multiple / Tag-on-Place / Numeric) | M      | `done` (wave3-3 — ToolGrammarModifiers + TOOL_CAPABILITIES + reduceNumericInput in toolGrammar.ts; new packages/web/src/tools/OptionsBar.tsx with Chain / Multiple / Tag-on-Place / Wall-location-line toggles; chain mode confirmed to halve four-wall-room click count) | EDT-01     |
...
**As-shipped (wave4-A closeout).** Hard-reject behaviour, not snap-back: the engine's `try_commit_bundle` runs `bim_ai.edt.constraints.evaluate_all` after every command apply and rejects the bundle (returning `code='constraint_error'` with `Violation` rows tagged `edt_constraint_violated` carrying `constraint_id` / `rule` / `residual_mm` in the message) when an error-severity equal_distance lock is broken. The clone-rollback pattern is implicit — failed commits never return the candidate document. Padlock UI lives in `packages/web/src/plan/GripLayer.tsx`'s `TempDimLayer`; `findLockedConstraintFor` in `packages/web/src/plan/tempDimensionLockState.ts` selects open- vs filled-padlock rendering; `handleTempDimLockClick` in `PlanCanvas.tsx` dispatches `createConstraint` capturing the temp-dim's `distanceMm` between the two walls (`aId` / `bId` plumbed through `TempDimTarget`). Other rules (parallel / perpendicular / collinear / equal_length) remain pass-through; no padlock chip in the 3D viewport; no soft-lock UX.
...
| ID     | Item                                              | Effort | State                                                                                                                                                                                                                                                                                                                                                              | Depends on |
| ------ | ------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| SKT-01 | `SketchSession` state machine + canvas mode       | XL     | `done` in wave-04 — all seven sub-modes wired through a `SUBMODES` registry of (validator, Finish-emitter) pairs; `floor` / `roof` / `room_separation` carry forward from wave3-4; `ceiling` dispatches to existing `createCeiling`; `in_place_mass` / `void_cut` are new commands (`CreateMassCmd` → `MassElem` with `phase_id='massing'`; `CreateVoidCutCmd` → marker `VoidCutElem` co-authored with an `AgentDeviationElem` for traceability); `detail_region` dispatches to existing `createDetailRegion`. Validation factored into `assert_closed_loop` (polygon kinds) + `assert_line_set` (line-set kinds) helpers, layered on top of the existing structural `validate_session` (vertex incidence, zero-length, self-intersection). HTTP route accepts a generic `options` bag so each sub-mode can carry its own params (heightMm / hostElementId / hostViewId / etc.) without per-kind request models                                                                                                                                                                                                                                                                                                              | —          |
| SKT-02 | Pick Walls sub-tool inside sketch sessions        | S      | `done` in wave3-4 — `pickWall` toggle endpoint, configurable offset mode (`interior_face` default vs. `centerline`), automatic corner-trim via line-line intersection, `SketchCanvasPickWalls` hit-test + green hover highlight, `OffsetModeChip` toggle, walls passed in from PlanCanvas (filtered by active level)                                                                                                                                                                                                                                                                                                                                                                                                                                              | SKT-01     |
| SKT-03 | Sketch validation feedback (Revit-style messages) | S      | `done` in wave3-4 — multi-row status panel listing every issue, error lines re-stroked red, open vertices marked with red ring overlays, Tab cycles active issue (data-active flag + numeric counter), `Auto-close` one-click fix when there's a single missing segment between two open endpoints, Finish disabled while any errors remain (tooltip lists count)                                                                                                                                                                                                                                                                                                                                                                                                  | SKT-01     |
...
| ID     | Item                                                         | Effort | State                                                                                                                                                                                                                                                                                                                                                                                                         | Depends on       |
| ------ | ------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| FAM-01 | Nested families (loadable family-in-family)                  | L      | `done` (wave3-7 — full nested-family system: data model + resolver + cycle detection (583e726c) + family-editor `Loaded Families` sidebar with drag-drop + per-instance parameter binding editor (literal / host_param / formula) + visibility-binding hookup + composed-family thumbnail rendering via `getFamilyComposedThumbnail`)                                                                                                            | —                |
...
| KRN-02 | Concave / L-shaped roof footprint 3D mesh                                      | Today: `valley_candidate_deferred` placeholder; section view OK. Substantially smaller after SKT-01 lands.                                                                                                                                                                                                                | M      | `done` (wave3-5 — new `gable_pitched_l_shape` mode added to `RoofGeometryMode` (Python + core/index.ts); engine validates 6-vertex axis-aligned L footprint with one reflex corner; `roof_geometry_support_token_v0` emits `gable_pitched_l_shape_supported`; renderer dispatches to existing `_buildLShapeGeometry`; tests in `meshBuilders.lShapeRoof.test.ts` + `test_roof_pitch_evidence.py`) |
| KRN-03 | Hip roof (convex polygon >4 corners) 3D mesh                                   | Today: `hip_candidate_deferred` placeholder; section view OK. Substantially smaller after SKT-01 lands.                                                                                                                                                                                                                   | M      | `done` (wave3-5 — `'hip'` added to Python `RoofGeometryMode`; engine validates convex polygon for hip mode; `roof_geometry_support_token_v0` emits `hip_supported`; new `_buildHipPolygonGeometry` pavilion mesh — every edge slopes inward to a centroid apex at `inradius * tan(slope)` for ≥5-vertex convex polygons; tests in `meshBuilders.hipRoof.test.ts`) |
...
| KRN-07 | Multi-run stairs (dog-leg, L-shape, U-shape, spiral, winders)                  | Today's `stair` element is single-run. Real stairs are runs + landings, optionally sketch-based for arbitrary shapes.                                                                                                                                                                                                     | L      | `done` (wave3-5 + wave-05 closeout — `StairElem` + `CreateStairCmd` accept all five shapes (`straight` / `l_shape` / `u_shape` / `spiral` / `sketch`); spiral takes `centerMm` / `innerRadiusMm` / `outerRadiusMm` / `totalRotationDeg` / `riserCount` and the engine derives a single curved-run with helical-midline polyline + N annular treads (no landings); sketch takes `sketchPathMm` and walks treads along the input polyline. SKT-01 sketch sessions gain a `stair` submode whose Finish emits `createStair` with `shape='sketch'`. Renderer factored into `meshBuilders.multiRunStair.ts` with `buildSpiralStairFlight` + `buildSketchStairFlight`; new `plan/stairPlanSymbol.ts` draws fanned wedges (winder-tread plan symbol) for spiral stairs and tangent-perpendicular tread lines for sketch stairs. Tests: `test_create_stair_spiral_and_sketch.py` (engine + sketch-session round-trip), extended `meshBuilders.multiRunStair.test.ts`, new `stairPlanSymbol.test.ts`. Bezier/NURBS curved treads, stair calculator UI, and railing layout helpers remain out of scope.) |
| KRN-08 | `area` element kind (legal / permit area calculations)                         | Distinct from `room`. Areas may include exterior porches and exclude interior shafts; rules differ by jurisdiction.                                                                                                                                                                                                       | M      | `done` (wave3-6 — `area` element kind + `createArea` / `updateArea` / `deleteArea` engine cmds + `area_calculation.py` (`gross` / `net` (subtracts contained slab_opening shafts) / `no_rules`) wired to a per-apply recompute hook so `computedAreaSqMm` always tracks the polygon + plan renderer drawing dashed-red boundary + centroid sprite tag (`<name> · X.XX m²`) + Architecture-ribbon "Area Boundary" tool (hotkey AR, two-click rectangular fallback) + `area` category in `schedule_derivation`; tests cover engine validation + ruleset variants + plan extraction)                                                                                                                                                                                                                |
...
| KRN-10 | Masking region (2D filled region that blocks underlying linework)              | Used in family editor (Hour 4: 2D chair plan symbol blocks floor-tile hatch underneath). View-local 2D element.                                                                                                                                                                                                           | S      | `done` (wave3-6 — `masking_region` element kind + `createMaskingRegion` / `update` / `delete` engine cmds + opaque-polygon plan renderer at z above element wires but below detail components / dimensions / tags + Annotate-ribbon authoring tool emitting an 800×800mm fallback polygon; tests cover engine validation + view-scoped extraction)                                                                                                                                                                                                                                                                                                                |
...
| KRN-14 | Dormer element kind (cuts host roof, adds own walls + roof)                    | Target house has a "massive rectangular dormer cut-out" on the east roof slope opening onto the flat roof deck. Common attic-conversion primitive in residential.                                                                                                                                                         | L      | `done` (seed-fidelity sprint + wave-05 closeout — element kind + `createDormer` + 3-walls + flat / gable / hipped roof renderers + three-bvh-csg subtraction of the host roof footprint; `ridgeHeightMm` field with model validator requiring it for gable/hipped; `hasFloorOpening: true` emits a derived `slab_opening` element on the host roof's reference-level floor; plan symbology renders a dashed footprint outline + "DR" label on the host level's plan view; new advisor rule `dormer_overflow_v1` (warning, geometry) fires when any dormer footprint vertex falls outside the host roof's polygon. Tests in `test_dormer_completeness.py` + `dormerMesh.test.ts` (gable + hipped face counts) + new `dormerPlanSymbol.test.ts`. Shed dormers, eyebrow / wall dormers, and auto-window placement remain out of scope.) |
...
| IFC-04 | Broader QTO + materials + classifications + composites | Narrow QTO slice shipped; full takeoff pending. Layered composites become more relevant after FL-08.    | L      | `done` (wave-05 closeout — non-architectural `Pset_*Common`: stairs gain `Pset_StairCommon` `NumberOfRiser` / `NumberOfTreads` / `RiserHeight` / `TreadLength`; new export blocks emit `IfcColumn` / `IfcBeam` / `IfcCovering` (ceilings) / `IfcRailing` each with their canonical `Pset_*Common` populated from kernel fields (Reference / LoadBearing / IsExternal / Span / Height). Element-occurrence classifications: `ifcClassificationCode` added to `StairElem` / `ColumnElem` / `BeamElem` (TS mirrored), and a shared `_maybe_attach_classification` helper emits `IfcRelAssociatesClassification` on stairs / columns / beams via the existing reference cache. Material category mapping: new `_MATERIAL_CATEGORY_TO_IFC` in `material_catalog.py` maps MAT-01 categories — `timber→Wood`, `concrete→Concrete`, `metal`/`metal_roof→Metal`, `glass→Glass`, `brick→Masonry`, `stone→Stone`, `plaster→Gypsum`, `render→Render`, `cladding→Cladding`, `membrane→Membrane` — and `try_attach_kernel_ifc_single_material` + the layer-set path now stamp the IFC4-standard string onto `IfcMaterial.Category` (`placeholder` / `air` → unset). Round-trip pytest coverage in `test_export_ifc_broader_coverage.py`.) |
...
**Architectural principle: software stays deterministic, intelligence lives in the agent.** The bim-ai software is a deterministic toolkit — engine commands, validators, geometry primitives, screenshot rendering, similarity scoring (SSIM / pixel diff / classical CV). **No LLM and no vision model are embedded in any binary that runs as part of the BIM AI product.** Every interpretive step (read the sketch, decide on dimensions, pick materials, judge whether the silhouette matches, write corrective commands) happens in an external AI agent (e.g. Claude via Claude Code). The agent reaches the deterministic toolkit through CLI commands, file I/O, and reading rendered PNGs / structured JSON itself — using its own multimodal capabilities to look at images. The "AI architect" is the agent loaded with a `skill.md` describing the methodology + tool catalog, _not_ a model baked into the software. This decoupling keeps the product portable across AI substrates, keeps the test surface deterministic, and keeps inference cost outside the runtime path.

**Skill-file pattern.** Project-level skills live at `claude-skills/<name>/SKILL.md` at the repo root, with each skill's frontmatter `description:` field describing when an agent should load it. SKB-12 ships at `claude-skills/sketch-to-bim/SKILL.md` and is referenced from the project's `CLAUDE.md` so any agent loading project context discovers it. The skill describes: (a) the phase sequence, (b) the deterministic CLI tools available at each phase, (c) the validation gates the agent must pass, (d) the assumption-log protocol. The agent — not the software — interprets sketches (using its own vision capabilities on the PNG file), selects archetypes, judges silhouette match, and authors corrective commands. The software just provides the tools, the screenshots, and the numerical deltas the agent reasons about.

| ID     | Item                                                                 | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Effort | State                                                                     |
| ------ | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------- |
| SKB-01 | Phased build workflow + `phase` tag on commits                       | `done` — `app/bim_ai/skb/phases.py` defines `SKB_PHASES` ordering, `PhasedBundle` / `PhasedCommand` data shapes, `from_dict_list` parser, `from_legacy_bundle` migration helper, `staged_subbundles()` to drive per-phase commits, `commit_message_prefix()` for conventional commit types, and `validate_phase_order()` soft check for out-of-order revisits. Engine + CLI hookups in subsequent WPs (SKB-10 wires this into `try_commit_bundle`).                                                                                                                                                                                                                                                                                                                       | M      | `done`                                                                    |
| SKB-02 | `mass` element kind (volumetric massing primitive)                   | `done` — Wave-04 closeout adds the renderer + auto-extractor on top of the prior load-bearing data-model slice. `packages/web/src/viewport/meshBuilders.mass.ts` builds a translucent extruded volume (opacity 0.35, `DoubleSide`, `EdgesGeometry` outline child) keyed by `materialKey`; `Viewport.tsx` routes `kind === 'mass'` through the builder and respects `viewerPhaseFilter`. `MaterializeMassToWallsCmd` (Pydantic; alias `materializeMassToWalls`) emits one wall per footprint segment (ids `${massId}-w<i>`), one floor matching the footprint at level base (`${massId}-f`), and one flat roof at level base + heightMm (`${massId}-r`), promotes `phaseId` from `'massing'` → `'skeleton'` on every emitted element (added `phaseId` field to WallElem / FloorElem / RoofElem to carry the tag), forwards `materialKey`, deletes the source mass, and co-authors an `AgentDeviationElem` referencing the source mass + emitted ids. Mirrors: `'mass'` added to `ElemKind`; `MassElement` shape + `MaterializeMassToWallsCmd` exported from `packages/core/src/index.ts`. Tests cover axis-aligned + L-shape footprints, the `'skeleton'` phase promotion, materialKey carry-forward, mass deletion, deviation creation, and unknown-mass error path. SKB-12 cookbook's massing → skeleton transition is now fully automatic. | L      | `done`                                                                    |
| SKB-03 | Visual checkpoint tool (deterministic screenshot + delta vs. target) | `done` (load-bearing slice) — `app/bim_ai/skb/visual_checkpoint.py` ships `compare_pngs(actual, target, regions, threshold)` (pure NumPy + Pillow per-region mean-abs-delta + pct-pixels-above-threshold + weighted overall) returning `CheckpointReport.passed` for SKB-15's loop. `run_checkpoint_against_target()` orchestrates with an injected `screenshot_fn(out_path)` callable so Playwright (headless three.js) hookup is a thin wrapper — comparison math is testable today without a dev server. Auto-resamples size mismatches.                                                                                                                                                                                                                              | L      | `done`                                                                    |
| SKB-04 | Sketch-to-dimensions calibrator                                      | `done` — `app/bim_ai/skb/calibrator.py` defines `Anchor` (label / pixels / millimeters / axis), `calibrate()` (single-anchor exact, multi-anchor median + residual%), and `CalibratedSketch` with `measure_pixels(d)` / `measure_segment(p1, p2)` / `position_to_mm(p, origin_px)` query API. Multi-anchor disagreement surfaces as `residual_pct` so the agent sees when their anchors don't agree.                                                                                                                                                                                                                                                                                                                                                                      | M      | `done`                                                                    |
| SKB-05 | Architectural soundness validator pack                               | `done` — `app/bim_ai/skb/soundness.py` defines four checks and a `run_pack(SoundnessInput)` orchestrator: (a) `check_wall_corner_gaps` near-miss endpoints in [join_tol, near_miss_tol]; (b) `check_floor_matches_walls` Hausdorff polygon-to-walls within tol; (c) `check_levels_monotonic_stack` flags shared elevations + duplicate ids; (d) `check_roof_contains_wall_midpoint` slim-cantilever check (subset of SKB-11 using midpoints). Each emits warning advisories — promotion to blocking is the engine's call.                                                                                                                                                                                                                                                | M      | `done`                                                                    |
| SKB-06 | Proportion / scale linter                                            | `done` — `app/bim_ai/skb/proportions.py` defines `PROPORTION_RANGES` for 12 typical residential thresholds (wall height/thickness, door/window width/height/sill, roof slope, ridge:eave ratio, room aspect, stair tread/riser). `check_value()` / `check_many()` produce `ProportionViolation` records that serialise to `architectural_proportions_v1` `info`-severity advisories.                                                                                                                                                                                                                                                                                                                                                                                  | S      | `done`                                                                    |
| SKB-07 | Region colour sampler → MAT-01 nearest-match                         | `done` — `app/bim_ai/skb/colour_sampler.py` provides `sample_region_mean_rgb(image, xyxy)` (Pillow + NumPy mean RGB), `nearest_matches(rgb, catalog, top_k)` (CIE76 Lab Delta-E sort), `sample_and_match()` orchestrator, and `MAT01_COLOUR_CATALOG` ships 23 catalog entries. No model, deterministic Lab-distance.                                                                                                                                                                                                                                                                                                                                                                                                                                                       | S      | `done`                                                                    |
| SKB-08 | Phase-checkpoint assumption log                                      | `done` — `AgentAssumptionElem` extended with optional `phaseId` (`SkbPhaseId` enum: massing/skeleton/envelope/openings/interior/detail/documentation) + free-form `sketchAnchorMm?` dict (carries pixel coords / panel labels / polygon refs). Optional rather than mandatory to keep legacy assumptions valid; agent loop and SKB-15 refine loop populate them on every new inference.                                                                                                                                                                                                                                                                                                                                                                                  | S      | `done`                                                                    |
| SKB-09 | Architectural archetype library (starter bundles)                    | `done` — `app/bim_ai/skb/archetypes.py` ships four archetype builders (`single_family_two_story_modest`, `l_shape_bungalow`, `cabin_a_frame`, `townhouse_three_story`), each registered with `Archetype` + per-archetype `default_params` (typical proportions sourced from `bim_ai.skb.proportions.PROPORTION_RANGES`). Modest extended with 6 interior partitions + 6 rooms + 1 stair on top of the original 8-wall envelope. L-bungalow ships an axis-aligned 6-vertex L footprint with a `gable_pitched_l_shape` roof + 5 partitions / 6 rooms. A-frame cabin declares its mezzanine as a child `LevelElem` at ~2/3 of the floor-height with a steep gable roof, large front glazing, and a stair. Townhouse pins east/west party walls on every level via `pinElement`. Each archetype emits ≥3 viewpoints from SKB-16 presets (`vp-main-iso`, plus front/rear/side elevations). `ARCHETYPE_PRIORS` extended with a `partition` range per archetype; `out_of_range_kinds()` returns `[]` for every archetype's emitted bundle.                                                                                                                                                                                                                                                                                                                                                                | L      | `done`                                                                    |
| SKB-10 | Per-phase visual-verification gate (mandatory)                       | `done` — `app/bim_ai/skb/phase_gate.py` defines `PhaseGateConfig { threshold, slack, block_on_fail }`, `evaluate_gate(phase, report, config) → PhaseGateOutcome { kind: pass\|advisory\|reject }` with a tri-state outcome and `phase_visual_gate_v1` advisory serialisation. `gate_phase_commit(phase, config, run_checkpoint)` wraps SKB-03 for the engine integration. Engine `try_commit_bundle` wrapper is a thin caller; this WP ships the decision logic.                                                                                                                                                                                                                                                                                                          | M      | `done`                                                                    |
| SKB-11 | Roof-wall alignment validator                                        | `done` — `app/bim_ai/skb/roof_wall_alignment.py` defines `check_alignment(wall, roof_footprint)` + `check_alignments(walls, roofs)` that ray-cast each wall endpoint against the roof footprint polygon and emit `roof_wall_alignment_v1` warning advisories with the worst offending point + distance-outside-mm. Tolerance bound prevents spurious flags on boundary endpoints.                                                                                                                                                                                                                                                                                                                                                                                       | S      | `done`                                                                    |
| SKB-12 | Architect cookbook (sketch-to-BIM methodology skill)                 | A canonical workflow skill at `claude-skills/sketch-to-bim/SKILL.md` (referenced from the root `CLAUDE.md`) that every agent doing sketch-to-BIM is instructed to read. Defines the 7 phases (massing → skeleton → envelope → openings → interior → detail → documentation), per-phase author / validate / checkpoint / refine template, the refine loop with iteration cap, the anti-patterns from the 2026-05-07 seed-fidelity failure, the tool catalog (today vs. planned SKB-XX). Pairs with SKB-21 (brief format) — the brief tells the agent _what_ to build, the cookbook tells it _how_.                                                                                                                                                                     | S      | `done` (initial skill ships in this commit; iterate as SKB-XX tools land) |
| SKB-13 | Element-count priors per archetype                                   | `done` — `app/bim_ai/skb/element_count_priors.py` defines `ARCHETYPE_PRIORS` for 6 archetypes (single-family two-story / one-story / townhouse / cabin A-frame / L-shape bungalow / modernist gable) with `(lo, hi)` per kind; `out_of_range_kinds()` returns the per-kind flag list for surfacing as advisories.                                                                                                                                                                                                                                                                                                                                                                                                                                                       | XS     | `done`                                                                    |
| SKB-14 | Sketch edge-detection helper (classical CV)                          | `done` — `app/bim_ai/skb/edge_detection.py` provides `detect_edges(input, output, canny_low, canny_high, blur_sigma)` (OpenCV with scikit-image fallback) writing an edges-only PNG. Returns `EdgeDetectionResult` with thresholds + edge_pixel_count + image_size for evidence logging; `edge_density()` sanity check. Deterministic, no model.                                                                                                                                                                                                                                                                                                                                                                                                                          | S      | `done`                                                                    |
| SKB-15 | Recursive refine loop (formal agent workflow)                        | `done` — `app/bim_ai/skb/refine_loop.py` defines `run_refine_loop(phase, checkpoint, propose_correction, apply_correction, max_iterations=5)` with bounded iteration. `RefineLoopOutcome.to_evidence_dict()` serialises the agent's chain of reasoning per phase. Callable injection points (checkpoint / propose / apply) decouple from SKB-03 and from any agent backend, so the loop is unit-testable without Playwright or LLMs.                                                                                                                                                                                                                                                                                                                                     | M      | `done`                                                                    |
| SKB-16 | Camera presets matched to sketch perspectives                        | `done` — `app/bim_ai/skb/camera_presets.py` defines 8 presets (`vp-main-iso`, `vp-front-elev`, `vp-rear-elev`, `vp-side-elev-east`, `vp-side-elev-west`, `vp-rear-axo`, `vp-roof-plan`, `vp-cutaway-iso`) with azimuth/elevation/distance-factor/mode; `preset_for_sketch_panel(label)` maps free-form sketch panel labels to preset.                                                                                                                                                                                                                                                                                                                                                                                                                                    | XS     | `done`                                                                    |
| SKB-17 | _(retired — was "LLM-assisted sketch parser")_                       | Removed. The agent itself is the LLM with vision capability; sketch interpretation belongs in the skill, not the software. The agent reads `<sketch>.png` with its own multimodal tooling. SKB-04 (calibrator) + SKB-07 (region colour sampler) + SKB-14 (edges helper) provide deterministic primitives the agent reaches for; orchestration + interpretation stay in the agent layer.                                                                                                                                                                                                                                                                                                                                                                               | —      | `n/a`                                                                     |
| SKB-18 | Phase-by-phase test fixtures                                         | `done` — `app/bim_ai/skb/phase_test_fixtures.py` defines `PhaseFixture` (phase + bundle slice + expected_kind_counts (int or `CountRange`) + expected/forbidden advisory rule_ids) and `evaluate_fixture(fixture, elements, advisories)` returning a `FixtureResult { passed, failures }`. Helpers `kind_counts_from_elements`, `assert_kind_counts`, `assert_advisory_rule_ids` are reusable. Silhouette-PNG asserts deferred to SKB-03 once Playwright lands.                                                                                                                                                                                                                                                                                                            | M      | `done`                                                                    |
| SKB-19 | Wall-graph closure validator (stronger VAL-01)                       | `done` — `app/bim_ai/skb/wall_graph.py` defines `find_orphan_walls` (endpoint not coincident with any other wall's endpoint within tol), `find_non_orthogonal_walls` (direction off the cardinal axes), and `find_t_intersections_without_join` (endpoint on another wall's interior without a join_geometry marker). Each emits `orphan_wall_v1` / `non_orthogonal_wall_v1` / `t_intersection_no_join_v1` warnings; `check_wall_graph()` runs all three and returns the union.                                                                                                                                                                                                                                                                                          | S      | `done`                                                                    |
| SKB-20 | Architectural style bias vectors                                     | `done` — `app/bim_ai/skb/style_bias.py` defines `STYLE_BIASES` for 5 styles (modernist / traditional / farmhouse / industrial_warehouse / scandinavian). Each carries `roof_bias[]`, `palette` (logical surface → MAT-01 key), `glazing_ratio`, eave / floor-height / wall-thickness defaults. `style_for_brief_hint()` maps free-form briefs to a style; agent applies the bias when authoring. Pairs with SKB-09.                                                                                                                                                                                                                                                                                                                                                       | S      | `done`                                                                    |
| SKB-21 | Sketch-to-BIM brief format (canonical input)                         | `done` — `app/bim_ai/skb/brief.py` defines `SketchBrief` Pydantic model with `program[]` / `keyDimensions[]` (with confidence enum + sketchAnchor) / `materialIntent[]` (with catalogKey resolution) / `specialFeatures[]` / `referenceImages[]` (panel + presetId). `SAMPLE_BRIEF` ships the target-house brief as a fixture; `brief_from_dict` / `brief_to_evidence_dict` round-trip via camelCase aliases for the evidence log.                                                                                                                                                                                                                                                                                                                                       | S      | `done`                                                                    |
| SKB-22 | Wall-corner join markers + auto-join                                 | `done` — `app/bim_ai/skb/wall_corner_auto_join.py` defines `detect_corner_joins` (coincident endpoints within tolerance), `detect_abutment_joins` (T-junctions where one wall's endpoint lies on another wall's open interior), `detect_all_joins` union helper, and `joins_missing_from_existing` to skip joins already covered by existing `join_geometry` elements. Each pair serialises to a `join_geometry` element dict via `to_element_dict(join_id)` so the agent / engine can emit them on commit.                                                                                                                                                                                                                                                              | M      | `done`                                                                    |
| SKB-23 | Per-phase preview rendering mode                                     | `done` — `viewerPhaseFilter: { phases[], includeUntagged }` slice on the Zustand store, `setViewerPhaseFilter` / `clearViewerPhaseFilter` actions, pure helper `packages/web/src/viewport/phaseFilter.ts` (`isElementVisibleUnderPhaseFilter` + `applyPhaseFilter`) that consumers (3D builder, plan projector) call before adding geometry. Used by SKB-03 to render per-phase silhouettes and by reviewers to see the model state at the end of any phase.                                                                                                                                                                                                                                                                                                              | S      | `done`                                                                    |

**SKB priority bundle for the next sketch-to-BIM POC.** SKB-12 (the skill) has shipped at `claude-skills/sketch-to-bim/SKILL.md`; the agent now has a documented methodology. The remaining minimum-viable subset to give an AI agent a real chance at converging on the target house: **SKB-03 (rendered screenshot the agent reads with its own vision) + SKB-21 (brief format) + SKB-15 (refine loop, formalised) + SKB-04 (calibrator) + SKB-05 (soundness) + SKB-11 (roof-wall) + SKB-10 (gate)**. SKB-02 (mass primitive) and SKB-09 (archetypes) are the next tier — they cut the cold-start cost dramatically. SKB-07 (region colour sampler) and SKB-14 (edge-detect helper) are deterministic vision aids the agent can reach for when needed; both are optional convenience.
