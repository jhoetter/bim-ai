...
| KRN-07 | Multi-run stairs (dog-leg, L-shape, U-shape, spiral, winders)                  | Today's `stair` element is single-run. Real stairs are runs + landings, optionally sketch-based for arbitrary shapes.                                                                                                                                                                                                     | L      | `partial` (wave3-5 — `StairElem` extended with `shape` + `runs[]` + `landings[]` (Python `StairRun`/`StairLanding` + TS type exports); `CreateStairCmd` accepts the new fields, engine auto-derives a single run from `runStartMm/runEndMm` for legacy callers and a square landing per gap when shape=`l_shape`/`u_shape` and landings omitted; renderer dispatches to new `_makeMultiRunStairMesh` that walks each run as an inclined flight plus extruded landing polygons. Tests in `meshBuilders.multiRunStair.test.ts` + `test_create_stair_multi_run.py`. Spiral / sketch shape variants + plan symbology for winders deferred; sketch path waits on Agent 4's SKT-01 propagation.) |
