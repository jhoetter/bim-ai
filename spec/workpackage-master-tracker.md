...
| SKB-02 | `mass` element kind (volumetric massing primitive)                   | `partial` — load-bearing slice: `MassElem` Pydantic model in `app/bim_ai/elements.py` (kind, levelId, footprintMm ≥3, heightMm > 0, rotationDeg, materialKey, phaseId default 'massing', pinned), discriminated-union registration, `'mass'` added to the central kind list. Renderer (3D translucent volume) + `materializeMassToWalls` engine extractor (auto-derive walls + floor + roof-stub from a mass) deferred to follow-ups; the data model lands first so SKB-09 archetypes / agent loop can reference it.                                                                                                                                                                                                                                                       | L      | `partial`                                                                    |
