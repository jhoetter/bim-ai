...
| SKB-03 | Visual checkpoint tool (deterministic screenshot + delta vs. target) | The single highest-leverage missing capability: **the agent needs a way to render the model**. New CLI command `bim-ai checkpoint --viewpoint <id> --target <png> --out-png <render.png> --out-delta <delta.json>` boots a headless renderer (Playwright, reusing the e2e harness), screenshots the named viewpoint, and computes deterministic metrics against the target image: SSIM, MSE, mismatched-pixel ratio, per-grid-cell delta map. **All metrics are classical image processing — no vision model.** The rendered PNG is written to disk so the agent can read the file with its own multimodal vision and judge the silhouette match itself. The JSON delta is for grep-able diff thresholds (e.g. "abort if SSIM < 0.6"). Runs after every phase commit. | L      | `open`                                                                    |
