...
| SKB-07 | Region colour sampler → MAT-01 nearest-match                         | New CLI tool `bim-ai sample-region --sketch <png> --polygon <points.json> --out <sample.json>`. Given a polygon (in pixel coords) provided by the agent, the tool returns: dominant RGB, mean RGB, colour histogram, and the top-3 nearest MAT-01 catalog matches by Euclidean distance in CIE Lab space. **No model, no inference — pure averaging + nearest-neighbour against the catalog's `baseColor` strings.** The agent looks at the sketch with its own vision, decides which regions to sample, and reads the JSON to pick the catalog key. Replaces "agent guesses `cladding_warm_wood`" with "agent samples the region, sees `(#a87a44, top match: cladding_warm_wood, distance 12.3)`".                                                                   | S      | `open`                                                                    |
...
| SKB-14 | Sketch edge-detection helper (classical CV)                          | New CLI tool `bim-ai sketch-edges --sketch <png> --out <edges.png> [--canny-low N --canny-high N]`. Runs Canny edge-detection (or similar classical CV — no model) on the sketch and writes an edges-only PNG that exposes massing outlines more crisply than the original drawing. The agent reads the edges PNG with its own vision and traces the polygons it cares about. **Deterministic OpenCV / scikit-image; no model.** Optional convenience for the agent — not load-bearing; the agent can already look at the original sketch directly.                                                                                                                                                                                                                   | S      | `open`                                                                    |
