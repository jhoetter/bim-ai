...
| EDT-03 | 3D direct-manipulation handles                                  | L      | `partial` (load-bearing slice — `packages/web/src/viewport/grip3d.ts` registry: `Grip3dDescriptor` { id, role, position, axis, rangeMm, onDrag, onCommit }, `register3dGripProvider(kind, provider)` / `gripsFor(elem)` / `clear3dGripProviders()`, `clampDragDelta` numeric helper. Wall provider implemented end-to-end: top + base edge handles at the wall plan-midpoint, axis=z, drag clamped to [MIN_WALL_HEIGHT_MM, MAX_WALL_HEIGHT_MM], commits `updateElementProperty` for `topConstraintOffsetMm` / `baseConstraintOffsetMm`; zero-delta commits suppressed. 17 tests cover the full provider + registry. Three.js LineSegment / Sprite raycast layer in `Viewport.tsx`, glowing axis indicator, and providers for floor / roof / column / beam / door / window deferred — registry is the load-bearing protocol that future kinds opt into.) | EDT-01     |
