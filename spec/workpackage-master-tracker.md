...
**SKB priority bundle for the next sketch-to-BIM POC.** The minimum-viable subset to give an AI agent a real chance at converging on the target house: **SKB-03 (rendered screenshot the agent reads with its own vision) + SKB-12 (cookbook / skill.md) + SKB-21 (brief format) + SKB-15 (refine loop) + SKB-04 (calibrator) + SKB-05 (soundness) + SKB-11 (roof-wall) + SKB-10 (gate)**. Without these the agent has no rendered output to look at and no enforced workflow. SKB-02 (mass primitive) and SKB-09 (archetypes) are the next tier — they cut the cold-start cost dramatically. SKB-07 (region colour sampler) and SKB-14 (edge-detect helper) are deterministic vision aids the agent can reach for when needed; both are optional convenience.
