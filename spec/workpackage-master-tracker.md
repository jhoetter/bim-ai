...
| EDT-02 | Constraint locks via padlock UI                                 | M      | `partial` (load-bearing slice — `ConstraintElem` Pydantic model in `app/bim_ai/elements.py` w/ rule + refsA/refsB + lockedValueMm + severity, registered in discriminated Element union & ELEMENT_KIND_NAMES; `app/bim_ai/edt/constraints.py` evaluator with `evaluate_all` + `errors_only` + `make_locked_distance_constraint` padlock-UI helper; equal_distance over wall-anchor refs implemented end-to-end with EPSILON_MM=0.5 tolerance, residual reporting, severity gating; other rules pass-through. Padlock chip on temp-dim + `createConstraint` engine command + post-apply rollback wiring deferred — module is callable from the engine but not yet hooked to the post-apply pipeline.) | EDT-01     |
