...
| `M1` Public agent surface for sketch initiation | Done        | P0/P1 CLI/product contract is closed for target-house readiness: a non-browser agent can validate IR, compile/author a phase, run the one-command phase loop, dry-run, commit, query, collect Advisor/constructability evidence, and accept/reject a phase. MCP resources are descriptor-backed where available; CLI remains the canonical executable bridge for skill-local surfaces. |
| `M2` Real BIM information requirements          | Done        | Sketch IR and acceptance gates require rooms/spaces, element semantics, type/material layer intent, classification, quantities, structure-lite, MEP-lite, planning/site, export, and material-passport starter assumptions appropriate to the quality target.                                                                                                                          |
| `M3` Evidence and acceptance hardening          | Done        | Advisor/constructability evidence collection, finding dispositions, visual evidence contract, BIM data checks, export validation, live-golden baseline contracts, current-head freshness checks, and semantic visual checklist enforcement are active. Actual screenshots/live baselines are generation-phase artifacts, not pre-generation readiness blockers.                        |
| `M4` Target-house readiness rehearsal           | Done        | The dry rehearsal planning artifacts for target-house-1 exist: IR, BIM information requirements, capability gap map, phase plan, acceptance checklist, and risk register. No seed artifact is committed yet.                                                                                                                                                                           |
| `M5` Production-quality target-house-1 run      | Done        | `seed-artifacts/target-house-1` is packaged with the corrected 86-command bundle, final live evidence, screenshot/visual artifacts, Advisor/export/tolerance evidence, phase packet, material check, and round-trip verification.                                                                                                                                                       |
...
| `SKB-RDY-A02` | P0       | Done   | Replace stale archive path defaults.      | Skill examples and `sketch_bim.py` default to `spec/data/sketch-to-bim-capability-matrix.json`; no active workflow command requires `spec/archive/*` unless explicitly reading history.    |
| `SKB-RDY-A03` | P0       | Done   | Restore active archetype manifest.        | `spec/data/sketch-to-bim-archetypes.json` exists or the helper intentionally points to the archived manifest with a clear reason. `archetypes --query` works from a clean checkout.        |
...
| `SKB-RDY-E03` | P0       | Done   | Visual gates for sketch-critical features. | `initiation-run`/phase acceptance now generate required semantic checklist gates for sketch-critical features and block final BIM acceptance when required semantic checks are missing, failed, unchecked, or pass without notes/evidence. Nonblank-only screenshots cannot satisfy final acceptance.                                                                                                                                                                                                                                                                                                                  |
...
| `SKB-RDY-E06` | P1       | Done   | IFC/IDS-style validation gate.             | `sketch evidence collect` and `initiation-run` write `export-validation.json` (`sketch.exchange-validation.v1`) from IFC/glTF manifests plus normalized snapshot evidence, checking hierarchy, entity classes, spaces, material layers, classifications, and planned Pset/quantity rows. Seed verification enforces this artifact with `--require-exchange-validation` and automatically enforces it for any generated seed artifact mapped to a live-golden case that requires exchange validation. Concrete backend Pset/quantity rows remain generation-phase/export-depth follow-up, not a pre-generation blocker. |
| `SKB-RDY-E07` | P1       | Done   | Semantic visual evaluator.                 | The current implementation is an enforceable agent-filled semantic checklist contract rather than embedded CV: generated checklist items require roof cutout, wrapper shell thickness, loggia recess, cladding/rhythm, room topology, and diagnostics checks, and acceptance fails until each required item is explicitly passed with notes/evidence.                                                                                                                                                                                                                                                                  |
...
| `SKB-RDY-E09` | P2       | Done   | Benchmark goldens for sketch cases.        | `spec/data/sketch-to-bim-golden-seeds.json` carries planned live-golden structure, post-generation baseline policy, required live artifacts, acceptance flags, and seed-artifact mappings for each case. `initiation-golden` writes per-case `live-golden-plan.json` without creating seed artifacts, and seed verification enforces those live baseline requirements once a mapped artifact exists. Actual live screenshot/Advisor/evidence baselines are generation-phase acceptance items after approved model generation, not pre-generation blockers.                                                                  |

Closeout note for E03/E07: Final Wave Worker 1 added structured semantic visual
checklist gates in `packages/cli/lib/sketch-initiation.mjs` and phase-accept
loading for agent-filled `visual-checklist.json` / `semantic-checklist.json`.
This intentionally keeps true CV outside the product binary; the enforceable
contract is that an external agent must fill each generated semantic check with
pass/fail status plus evidence before final BIM acceptance can pass.
