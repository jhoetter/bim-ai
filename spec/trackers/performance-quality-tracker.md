...
| `PERF-B06` | P1       | `Done`    | Separate undo-stack optimistic availability from full authoritative refresh where safe. | `StatusBar`'s `UndoCluster` now enables the undo button as soon as `pendingCommandCount > 0`, not just when the authoritative `undoDepth > 0`. The displayed depth still tracks the authoritative number (no flicker), and the `+N` pending badge tells the user why the button is reachable when the count reads 0. If the pending command fails, `pendingCommandCount` drops back to 0 and the button re-disables before any race. Tests in `StatusBar.test.tsx` assert the speculative-enable path and the depth=0 + pendingCount=0 baseline. The L02 closeout flagged this as the "speculative undo reservation" follow-up; this commit closes it on the UI side without changing the authoritative undo-stack semantics.                                                                                                                                                                                                                                                                                                                                                               |
...
| `PERF-G03` | P1       | `Done`    | Migrate `Workspace` off broad full-model scans for common derived values. | All five originally-flagged scans now consume narrow `modelIndices.*` slices: `commandPaletteEntities` uses `planViews` + `viewpoints` + `savedViews` + `sectionCuts` + `sheets` + `schedules`; `palettePlanTemplates` uses `viewTemplates`; `showEmptyState` uses `walls.length === 0`; `projectNorthAngleDeg` + `projectSettings` inline reads use `projectBasePoint` + `projectSettings`. `modelIndices` was extended with `viewpoints`/`savedViews`/`sectionCuts`/`viewTemplates` indices to support this. `installModelIndicesInvariant` (store.ts) ensures every elementsById write — including viewport-filter writers — refreshes the indices, so each narrow useMemo only recomputes when its specific slice changes. 249 workspace + state tests still pass. (`driftCount` and the line-202 broad `elementsById` subscription remain because they're used for inline element-by-id lookups across 60+ callsites; those are non-scan reactive reads, not the "broad scans" the audit was concerned about.)    |
...
| ID         | Priority | Status    | Item                                                                | Acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------- | -------- | --------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PERF-H01` | P0       | `Done`    | Add pointermove budget measurement for PlanCanvas.                  | Dev instrumentation `packages/web/src/plan/planPointerMovePerformance.ts` wired into `PlanCanvas.tsx`. `pnpm performance:plan-pointermove` (scripts entry in `packages/web/package.json` → `packages/web/scripts/plan-pointermove-perf.mjs`) drives plan canvas pointermove against a running dev server, harvests the in-app `__BIM_AI_PLAN_POINTERMOVE_PERF__` samples, and writes `spec/generated/plan-pointermove-perf.json` with schemaVersion `plan-pointermove-perf.v1`. The "still missing" note from the prior audit was stale — the script + npm wrapper have landed.                                                                                                                                                                                                                                                                                                     |
...
| `PERF-J04` | P1       | `Partial` | Code split heavy panels.                         | `WorkspaceOverlays.tsx` lazy-loads `FamilyLibraryPanel` (38 KB chunk, ~10 KB gzip) + `ProjectVersionHistoryPanel` (2.5 KB chunk) via `React.lazy` + `<Suspense fallback={null}>`. The FamilyLibraryPanel keeps the always-mounted contract (panel renders null internally while closed) so dependent tests still see the component in the tree; only the JS chunk is deferred. ProjectVersionHistoryPanel is fully conditional. Workspace chunk dropped from 1.75 MB / 437 KB gzip → 1.72 MB / 429 KB gzip after this split. Further candidates (CheatsheetModal, OnboardingTour, JobsPanel, ActivityDrawer, MilestoneDialog, SharePresentationModal) are open follow-ups — they have synchronous test expectations (workspace smoke tests use getByText immediately after open) so lazy-loading them needs paired test updates. |
...
| `PERF-L03` | P1       | `Done`    | Make undo/redo stack latency visible and bounded. | Closed by PERF-B06 + PERF-L02: undo depth updates within one commit round-trip (PERF-B01/B02/B03 made `try_commit` ~150 ms p50 on the small fixture); when the commit is in flight, the `+N pending` badge surfaces the speculative state and the undo button stays reachable via PERF-B06's speculative-enable. Together, the user sees undo availability respond within the pointer-frame budget regardless of network latency.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `PERF-L05` | P2       | `Partial` | Add user-facing degraded-mode warnings.           | `useDegradedModeIndicator()` polls the PERF-I03 (frame-time) + PERF-I04 (mesh-rebuild) probes once per second. When `avgFrameIntervalMs > 33.3` ms (30 fps soft floor) or `avgRebuildMs > 50` ms sustains for 3 consecutive samples, it flips to `{ active: true, reason }` with the kind + the offending number + the budget. Recovers when 3 consecutive samples come back under budget. `<DegradedModeBadge />` (mounted unconditionally in `WorkspaceFooterSlot`) renders a "Slow" chip when degraded, with the full diagnostic in the title attribute. Returns null when budgets are healthy, so no chrome cost in the common case. Regression tests in `useDegradedModeIndicator.test.ts` cover the inactive baseline, the flip-to-active threshold, the recovery threshold, and the rebuild-churn signal path. Active "degraded mode" mode-switching (reduced shadows, simpler materials) is still follow-up — this commit ships the surface and the trigger logic so the UI work has a stable anchor to attach to. |
...
## Findings Status (2026-Q3 reconciliation)

This section was added under `PERF-CQ-04` (see
`spec/trackers/code-quality-debt-tracker.md`). It maps each of the 12
narrative findings (`BE-1..7` + `FE-1..5`) declared in
`## Architectural Findings` above to its current shipped state. Each row
is one of:

- **SHIPPED** — the original concern was closed by a merged PR (linked).
- **OWNED-BY** — the residual work has a tracked work-package ID in this
  tracker or in `code-quality-debt-tracker.md`; the row records the WP
  ID and its current status.
- **DEFERRED** — the finding is acknowledged and intentionally not on
  the near-term plan; rationale is recorded.
- **STILL-OPEN** — known active work without a WP ID; a new WP ID is
  proposed inline.

Status reflects state as of 2026-05-29.

### Backend findings

| Finding                                             | Status                  | Mapping                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BE-1` Global evaluate() in interactive path        | **SHIPPED** + OWNED-BY  | The single-element fast-path generalisation landed in [PR #150](https://github.com/jhoetter/bim-ai/pull/150) (`PERF-CQ-02`): every single-element command whose verb is not in `_SCHEMA_ALTERING_COMMAND_TYPES` now commits with `documentation_advisors=False`, removing the nine info-only advisor passes from the hot path. Bulk / schema-altering commits still pay the full evaluate; tracked under `PERF-B07` (Done) and `PERF-B08` (Partial, per-advisor opt-in for `changed_ids` is the remaining piece). Larger-model scale is owned by `PERF-M01` (CI gate exists on the synthetic small fixture; medium / large fixtures still pending).                                      |
| `BE-2` Room boundary derivation is central hotspot  | **SHIPPED** + OWNED-BY  | The O(h² × v²) Cartesian enumeration was replaced with a corner-index helper in [PR #151](https://github.com/jhoetter/bim-ai/pull/151) (`PERF-CQ-01`): `room_stress` uncached p50 dropped ~132 ms → ~30 ms (4.3×). Cross-request caching is `PERF-C05` (Done); request-scoped caching is `PERF-C04` (Done); level-local invalidation is `PERF-C06` (Not started — opportunistic follow-up); axis-segment pre-bucketing for the corner enumeration is `PERF-C07` (Partial, snap-side landed, bucket-driven enumeration still open).                                                                                                                                                       |
| `BE-3` Evidence package has no request-scoped cache | **SHIPPED**             | [PR #156](https://github.com/jhoetter/bim-ai/pull/156) (`PERF-CQ-03`) added `app/bim_ai/evidence_request_cache.py` (`_EvidenceRequestCache` keyed on doc revision + scope id + derivation kind + params hash) and opened `evidence_request_cache_scope` inside `build_evidence_package_payload`. Measured: `small.evidence_package` 304 ms → 201 ms (-34 %); `documentation_heavy.evidence_package` 3881 ms → 2539 ms (-35 %); `schedule_heavy.evidence_package` 3580 ms → 2906 ms (-19 %). Pins acceptance #5 of `PERF-D01`. Job-backed full-mode evidence remains as `PERF-D07` (Not started — DEFERRED to when the default-mode payload still hurts a real user, not a CI synthetic). |
| `BE-4` Plan projection refetched per revision       | **SHIPPED**             | Server-side revision-keyed cache is `PERF-F01` (Done — `_PLAN_PROJECTION_CACHE`); client-side dedupe + `AbortController` is `PERF-F02` (Done — `usePlanProjectionWireSync.ts`); debug timing is `PERF-F03` (Done — `?debug=true`). The "rapid revision sequence creates stale in-flight requests" risk is closed by `PERF-F02`. No active follow-up.                                                                                                                                                                                                                                                                                                                                     |
| `BE-5` Schedule derivation fetched from multiple UI | **SHIPPED**             | Server-side LRU is `PERF-F04` (Done — `_SCHEDULE_TABLE_CACHE` keyed on `(model_id, revision, schedule_id, lightweight)`); client LRU + AbortController is `PERF-F05` (Done — `packages/web/src/schedules/scheduleTableCache.ts`); lightweight room/finish payload is `PERF-F06` (Done — `?lightweight=true`). Evidence-package schedule re-derivation is folded into `PERF-D04` (Done) via the `PERF-CQ-03` cache scope.                                                                                                                                                                                                                                                                 |
| `BE-6` Snapshot bootstrap may be duplicated         | **SHIPPED** (was STALE) | Already closed by `PERF-E03` (Done): after a REST snapshot load the websocket connects in delta / resume-only mode and does not send another full snapshot. `PERF-E04` (Done — `bim_ai.ws_bootstrap` structured logs) provides the regression observability. The 2026-05-22 audit pre-flagged this finding as STALE; this reconciliation confirms it. No follow-up.                                                                                                                                                                                                                                                                                                                      |
| `BE-7` Websocket broadcast awaits each client       | **OWNED-BY**            | `PERF-E05` Partial (`Hub.broadcast_json` now fans out via `asyncio.gather`; verified by `test_broadcast_json_fans_out_concurrently`) closed the per-client head-of-line blocking. `PERF-E06` Done added age + depth-based backpressure with auto-evict at 1011. A full per-socket queue / sender-task model remains as the residual `PERF-E05` ask; DEFERRED until production load profiles justify the complexity (single-tenant local dev does not).                                                                                                                                                                                                                                   |

### Frontend findings

| Finding                                               | Status       | Mapping                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `FE-1` Large monolith components                      | **OWNED-BY** | `PERF-K01` (Partial — `PlanCanvas` LOC split done, render-ownership still pending), `PERF-K02` (Partial — `Viewport` LOC split done), `PERF-K03` (Partial — `Workspace` LOC split done). The `Workspace.tsx` slice is being further narrowed by `REF-CQ-01` ([PR #165](https://github.com/jhoetter/bim-ai/pull/165), Done — `useMaterialBrowserState`) and `REF-CQ-02` ([PR #165](https://github.com/jhoetter/bim-ai/pull/165), Done — `WorkspacePaneNode`). `REF-CQ-03` (Not started — `Drag3dController` from Viewport.tsx) targets the next monolith. Render-ownership / store-read boundaries are the final piece; see `spec/methodology/render-ownership.md` and the `PERF-G03..G06` roadmap.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `FE-2` Single global `elementsById` invalidates broad | **OWNED-BY** | Derived indices are `PERF-G02` (Done — `walls`, `wallsByLevel`, `roomsByLevel`, `floorsByLevel`, `columnsByLevel`, `placedAssetsByLevel`, `openingsByWall`, `planViews`, `schedules`, `sheets`, `projectSettings`, etc.). Migration of `Workspace.tsx` off broad scans is `PERF-G03` (Done) — five originally-flagged scans now read narrow `modelIndices.*` slices. The remaining broad `useBimStore((s) => s.elementsById)` subscription at `Workspace.tsx:182` plus ~40 inline `elementsById[id]` reads are owned by `FE-CQ-01` (Partial — followups tracked in `code-quality-debt-tracker.md` as `FE-CQ-01-followup`). `PlanCanvas` interaction-path migration is `PERF-G04` (Partial — `planCanvasClickHandler.ts` fully migrated, snap/hover hot paths landed, tags/dimensions still pending). `Viewport` paths are `PERF-G05` (Partial — 3D draft wall-snap, level datums, georeference landed; hosted-opening conflict + wallJoin + dormer / roofHeightSampler still pending). `useShallow` adoption is `PERF-G06` (Done). Inventory of all `elementsById` consumers is `PERF-G01` (Done — `scripts/inventory-elementsById-consumers.mjs` artifact). |
| `FE-3` Full-model scans in interaction paths          | **OWNED-BY** | `PERF-H03` Done — all PlanCanvas pointermove paths (snap-hover, split-wall hover, component-ghost hover) plus the Viewport 3D draft wall-snap path no longer call `Object.values(elementsById)`. Tag / dimension lookups are residual `PERF-G04` work owned by the time-travel / inspector agent. Spatial index for picking + snapping is `PERF-H02` (Partial — `WallSpatialIndex` library + tests ship, end-to-end wiring waits on a `wallSpatialIndex` modelIndices entry).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `FE-4` 3D viewport renders continuously               | **SHIPPED**  | `PERF-I02` Done — `Viewport.tsx` ships a custom `scheduleViewportRender()` / `tick()` / `shouldAnimateViewport()` orchestrator that re-arms RAF only while orbiting / walking / animating and self-terminates at idle. Verified by `viewport/Viewport.authoringSource.test.ts`. Frame-time instrumentation is `PERF-I03` (Done — `viewportFrameStats.ts`, exposed on `window.__BIM_AI_VIEWPORT_FRAME_STATS__` in DEV). Mesh-rebuild instrumentation is `PERF-I04` (Done — `viewportRebuildStats.ts`). Raycast / picking acceleration on large fixtures is `PERF-I05` (Not started — DEFERRED until a large-model scale measurement justifies it).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `FE-5` Main bundle too large                          | **OWNED-BY** | Route splitting is `PERF-J03` (Done — Workspace, family editor, presentation viewer, icon gallery use `React.lazy`). Manual chunks for stable heavy vendors is `PERF-J05` (Done — three / pdf / leaflet / i18n / command-palette vendor chunks). Bundle budget gate is `PERF-J02` (Done — `scripts/check-bundle-budgets.mjs`). Heavy-panel splitting is `PERF-J04` (Partial — `FamilyLibraryPanel` + `ProjectVersionHistoryPanel` deferred via `React.lazy` in `WorkspaceOverlays.tsx`); `FE-CQ-02` ([PR #159](https://github.com/jhoetter/bim-ai/pull/159), Done) lazy-loaded `SchedulePanel` + `SheetReview` and cut the workspace chunk 1720 KB → 1603 KB raw / 427 KB → 399 KB gzip. `jsPDF` dynamic-import work is `FE-CQ-03` (Not started — P2). Bundle analyzer hook is `PERF-J06` (Done — `ANALYZE=1 pnpm build`).                                                                                                                                                                                                                                                                                                                                   |

### Outcome of reconciliation

- **6 of the 12 findings** are closed at the source level: `BE-1` (PR
  #150), `BE-2` (PR #151), `BE-3` (PR #156), `BE-4` (PERF-F01/F02/F03),
  `BE-5` (PERF-F04/F05/F06), `BE-6` (PERF-E03 — was the STALE entry),
  and `FE-4` (PERF-I02). Each row above links to either the closing PR
  or the closing WP IDs in this tracker.
- **5 findings** retain owned residual work: `BE-7` (per-socket queue
  model), `FE-1` (render-ownership boundary), `FE-2` (`elementsById`
  subscription narrowing), `FE-3` (tag/dimension scans + spatial index
  wiring), `FE-5` (heavy panel + `jsPDF` splitting). All residual work
  has a WP ID in this tracker or in
  `spec/trackers/code-quality-debt-tracker.md`; no orphan items.
- **2 explicit deferrals**: `PERF-D07` (job-backed full-mode evidence
  package) and `PERF-I05` (raycast / picking acceleration) — both
  rationally gated on a real large-model regression rather than a
  synthetic fixture today.
- The grade tables in `## Grade Summary` were not updated as part of
  this reconciliation because they are timestamped measurements; the
  next benchmark recheck will refresh them.
