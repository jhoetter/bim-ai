...
- Evidence 2026-05-14:
  ...
  - `packages/web/src/viewport/wallJoinDisplay.ts` and `packages/web/src/viewport/meshBuilders.ts` now render single-thickness 3D walls with cleaned join footprints for endpoint L corners and T-branch butt joins instead of raw overlapping box slabs; tests prove mitered shared diagonals and branch trimming to the host wall face.
  ...
  - Remaining before `Done`: complete 3D cleanup for X joins and layered/CSG wall variants, rectangle semantic topology command proof, selected-wall join controls with seeded visible disallow separation in plan/3D, modify command completion for unjoin/attach and cross-view previews, seeded screenshots for endpoint join/T join/trim/disallow/flip/undo/redo.
