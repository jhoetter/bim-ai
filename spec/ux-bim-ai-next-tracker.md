...
- Evidence 2026-05-16:
  ...
  - Remaining before `Done`: wire `roofParamsFromWallLoop` path for wall-loop → footprint derivation (separate from sketch-mode); add roof/ceiling/shaft to the element sidebar edit-boundary action set; seeded Playwright proof building floor → walls → roof → ceiling → shaft → attach-top → validate no orphaned constraints.
...
- Evidence 2026-05-16:
  ...
  - Remaining before `Done`: add joined-wall cleanup failure detection; seeded proof creating, detecting, and fixing one issue per category group (boundary error → dismiss; duplicate wall → ribbon repair; orphan → ribbon repair).
