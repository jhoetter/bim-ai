...
- Evidence 2026-05-16:
  ...
  - `packages/web/src/plan/structuralValidation.test.ts` covers 21 cases: valid boundary, < 3 vertex open loop, self-intersecting bowtie, too-small edge, no duplicates, exact-duplicate walls, reversed-duplicate walls, tolerance matching, hosted with valid host, orphaned (no hostId), orphaned (missing host), non-hosted element ignored, offset inside span, offset beyond span, clean model yields zero issues, aggregated multi-issue pass in one call; L-corner no join failure, T-join no join failure, parallel-endpoint walls flagged as `join_cleanup_failure`, `runStructuralValidation` aggregates join failures.
...
- Evidence requirements:
  ...
  - `git diff --check`.
- Evidence 2026-05-16:
  - Core geometry/lifecycle helper tests: `roofByFootprint.test.ts` (16), `structuralValidation.test.ts` (21), `modifyAvailability.test.ts` (17), `commandCapabilities.test.ts` (20), `wallConnectivity.test.ts` (11), `advisorStructuralViolations.test.ts` (6), `joinGeometry.test.ts`, `moveTool.test.ts`, `rotateTool.test.ts`, `wallOffsetTool.test.ts`, `arrayTool.test.ts`, `pinUnpin.test.ts`, `createSimilar.test.ts`, `modifyPreview.test.ts` (9), `authoring3d.test.ts` (+8), `commandCapabilities.3dTools.test.ts` (3) — all pass (3748 total; 1 pre-existing Leaflet failure).
  - `packages/web/src/plan/authoringPipeline.integration.test.ts` — 12 tests proving the end-to-end helper chain: floor boundary validation → 4-wall rectangle topology (4 endpoint joins, no T/X) → join records all allowed → no duplicate walls → roof params from wall loop (footprint expanded beyond bounding box) → overhang preserves vertex count → door/wall model has no orphan issues → full `runStructuralValidation` clean → 2-level shaft span valid → wall modify verbs enabled in plan (move/copy/rotate/attach/unjoin) → duplicate wall yields `structural_authoring.duplicate_wall` blocking advisor violation → schedule view disables attach.
