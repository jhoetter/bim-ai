...
| --- | --- | --- | --- | --- | --- |
| CW-001 | open | Critical | Universal physical element collision contract missing. | `family_instance` and `placed_asset` are not collected by primary evaluator and not AABB-supported by clash engine. | Every physical element exposes deterministic collision proxies or explicit unsupported diagnostics. |
| CW-002 | open | Critical | Furniture/fixture/family-vs-wall hard clash missing. | Shelf-through-wall scenario emits no relevant violation. | `furniture_wall_hard_clash` warning/error with viewpoint and affected ids. |
| CW-003 | partial | High | Clash engine is AABB-only and category-limited. | `aabb_for_element()` only handles wall/floor/grid_line. | AABB broad phase plus narrow phase across all physical categories. |
| CW-004 | open | High | Constructability matrix missing. | Current rules are hard-coded individual checks. | Versioned matrix with pair rules, tolerance, severity, and profile overrides. |
| CW-005 | open | High | Persistent issue lifecycle missing. | Current violations are transient. Clash results exist but limited. | Stable issues with new/active/reviewed/approved/not-issue/resolved/suppressed states. |
| CW-006 | open | High | Load-bearing wall semantics missing. | `WallElem` lacks load-bearing/structural role fields. | IFC-aligned load-bearing intent and structural role metadata. |
| CW-007 | open | High | Structural load-path sanity missing. | Structural columns/beams exist, but no support graph. | Warnings for unsupported beams/columns, load-bearing wall discontinuity, large openings. |
| CW-008 | partial | High | Stairs are not constructability-complete. | Riser/tread and schedule diagnostics exist. | Headroom, landing, shaft/opening, guardrail, wall/furniture clash checks. |
...
| CW-010 | open | High | Door/window operation envelopes missing. | Hosted placement exists; swing clearance not checked. | Door swing/window operation clearances collide with furniture/walls/fixtures. |
| CW-011 | open | High | MEP penetration logic missing. | Duct/pipe categories exist but no penetration/opening validation. | Pipes/ducts through walls/floors/ceilings require sleeve/opening/approval. |
...
| CW-014 | open | Medium | Clearance checks missing. | No generic distance engine. | Clearance volumes and min-distance checks by matrix/profile. |
| CW-015 | open | Medium | Duplicate geometry detection missing outside floor overlap. | Navisworks-style duplicate tests not represented. | Duplicate wall/floor/asset/import detection. |
| CW-016 | open | Medium | Suppression/ignore rules missing. | No first-class suppression records. | Audited suppressions with reason, scope, expiry. |
...
| CW-018 | open | Medium | IDS/data-requirement validation not first-class. | IFC/advisory exports exist. | IDS-like model metadata requirements for constructability profiles. |
