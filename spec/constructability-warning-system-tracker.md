...
| CW-009 | partial | High | Room access/enclosure is heuristic. | Boundary edge coverage exists, the legacy centroid-distance heuristic remains, and `room_without_door_access` now verifies that a door midpoint connects to each room boundary; egress graph/depth and realistic room topology remain incomplete. | Door-connected access graph, egress hints, realistic room boundary model. |
...
| CW-012 | partial | Medium | Roof-specific constructability rules deferred. | Primary advisor now checks roof-wall footprint coverage and `roof_low_slope_without_drainage_metadata`; roof attachments, roof voids, drainage routing, and exact roof geometry remain incomplete. | Roof slope/footprint/wall attachment/void checks in primary advisor. |
...
| CW-014 | partial | Medium | Clearance checks missing. | Door operation clearance, stair headroom clearance, and profile-gated `furniture_wall_clearance_conflict` checks exist; generic distance/maintenance/cabinet clearance profiles remain incomplete. | Clearance volumes and min-distance checks by matrix/profile. |
