...
Archived because UX-01..UX-10 shipped and are covered by regression tests. Code evidence:

- UX-02 plan-view routing: `activePlanViewId` in `packages/web/src/viewport/PlanCanvasContextOverlays.tsx`.
- UX-03 sheet experience: `placeRecommendedViews` action in `packages/web/src/cmdPalette/registry.ts`.
- Seven-region layout, tab routing, 3D controls moved to secondary sidebar, advisor footer badge — all live in `packages/web/src/workspace/` and `shell/`.
- Regression coverage in `Workspace.test.tsx`, `AppShell.test.tsx`, and the visual evidence folder under `spec/generated/visual-evidence/packages-web/ux-*-20260512/`.

UX-11 (3D editing / view controls) and UX-12 (icon system completion) were marked `in progress` and received no commits since 2026-05-10. Treat them as deferred to a later UI-polish cycle; they are no longer tracked here.
