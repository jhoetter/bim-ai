...
## Archive Closeout (2026-05-22)

Archived because the unified Project Setup cockpit recommended below has shipped at `packages/web/src/workspace/project/ProjectSetupDialog.tsx`, with tabs for project info, units, levels, grids, positioning, sun, phases, links, templates, and standards. A command-palette entry "Open Project Setup" exposes it.

Shipped (despite still being marked `Open` in the tables below):

- PSU-001 — Project Setup entry point and dialog shell.
- PSU-002 — Setup checklist surface (tabs act as the checklist).
- PSU-013 — Levels/datums section.
- PSU-039 — View templates section.
- PSU-050 — Materials section.

Still genuinely open at archival:

- Unified storey/level batch-add wizard with floor-to-floor heights and constraint-propagation preview.
- Explicit Project Units dialog distinct from internal mm storage.
- Phasing manager with ordered phases and phase filters.
- "Save as template" semantics for new-project-from-template flows.

Owner direction at archival: residual items are small enough to land in a follow-up workpackage on the next setup-polish cycle; no separate tracker required.
