...
Archived because the architectural decision this tracker prescribed has shipped: `packages/web/src/viewport/ViewCube.tsx` is a Three.js `WebGLRenderer` widget paired with `packages/web/src/viewport/viewcube/orientation.ts` for camera/cube state and face/edge/corner raycasting. The "incremental SVG patches are wrong" conclusion in the Research Conclusions section is no longer the blocker.

Shipped (despite still being marked `Open` or `In Progress` in the rows below):

- Three.js mini-renderer for the cube (`ViewCube.tsx`).
- Camera orientation contract (`viewcube/orientation.ts`).
- 26-view hit testing across face / edge / corner pieces.

Still genuinely open at archival:

- Hi-fi face labels.
- Right-click context menu (home, look at, lock to selection, options).
- Snap-and-go visual cue at fixed views.
- Animated camera transitions tuned to Revit feel.

Owner direction at archival: residual polish can be tracked in a smaller workpackage on the next UI-polish cycle; no full rebuild required.
