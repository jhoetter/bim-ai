...
| UX-R-003 | Structure tools                                                                                                                                            | Ribbon                           | Plan/3D contextual ribbons where supported | Partial | Some structure tools are plan authoring, some model edit                   | Scope each command by active view type and selection                          |
...
| UX-R-011 | Tool modifier/options bars                                                                                                                                 | Below ribbon via `AppShell`      | Ribbon option row                          | Partial | Separate bars create extra chrome and include discipline controls          | Merge into ribbon as contextual options for active tool                       |
...
Evidence (2026-05-12): seeded live capture at `packages/web/tmp/ux-ribbon-per-view-20260512/` confirms task-based plan tabs (`Create/Sketch/Insert/Annotate/Review`), scoped per-view ribbons for 3D (`3D View/Insert/Review` with `Load Family` in `05-3d-insert-load-family.png`), sheet (`Sheet`), and schedule (`Schedule`).
