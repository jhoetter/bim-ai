...
| -------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| UX-L-001 | Project selector/name                                                     | Header, `TopBar` project button                                      | Primary sidebar top                                                     | Fixed  | Project selection is navigation context, not tab/header content                                      | Primary selector owns the project menu; DOM coverage verifies no header project menu anchor                                         |
| UX-L-002 | User/account menu                                                         | Header right/avatar                                                  | Primary sidebar bottom                                                  | Fixed  | User menu competes with tabs and collaboration controls                                              | Primary sidebar bottom user menu is guarded by ownership tests                                                                      |
| UX-L-003 | Workspace discipline switcher                                             | Header `WorkspaceSwitcher`, left rail discipline select, options bar | Secondary sidebar per view and Cmd+K                                    | Fixed  | Discipline is duplicated and unclear: global workspace, view filter, or tool option                  | Lens control now lives in secondary sidebar and Cmd+K lens commands target the same lens state                                      |
| UX-L-004 | Mode buttons: Plan, 3D, Plan+3D, Section, Sheet, Schedule, Agent, Concept | Header mode/tab area                                                 | Primary sidebar navigation and header tabs                              | Fixed  | Duplicates the primary navigation and makes header less tab-focused                                  | Header ownership tests prove mode navigation is absent; primary nav opens named tabs                                                |
| UX-L-005 | Header QAT authoring shortcuts                                            | Header left, `TopBar` QAT buttons                                    | Ribbon and Cmd+K                                                        | Fixed  | Section/measure/dimension/tag are editing/view commands, not header commands                         | Header ownership and command capability tests keep authoring shortcuts out of header                                                |
| UX-L-006 | Undo/redo in header                                                       | Header left                                                          | Footer or compact header optional                                       | Fixed  | Undo/redo are global editing status, but header is overloaded                                        | Footer undo/redo cluster is canonical; header ownership tests guard against topbar undo/redo regression                             |
| UX-L-007 | Thin Lines                                                                | Header QAT                                                           | Secondary sidebar for view graphics or footer compact view status       | Fixed  | Thin lines affects view display, not header                                                          | Header ownership tests keep thin lines out of header; view graphics live in secondary                                               |
| UX-L-008 | Close inactive views                                                      | Header QAT/customize menu                                            | Header tab overflow/context menu and Cmd+K                              | Fixed  | Tab management belongs to tabs, not QAT                                                              | Implemented in `TabBar` overflow menu with `tabs.close-inactive` Cmd+K capability and unit coverage                                 |
...
| UX-L-012 | Primary sidebar collapse                                                  | `AppShell` icon strip                                                | Primary sidebar/header                                                  | Fixed  | Existing icon-only state mostly expands the same sidebar; zero-width recovery requires header button | Collapse, zero-width recovery, and header restore are covered by AppShell/Playwright regression evidence                            |
| UX-L-013 | Primary sidebar resize                                                    | Fixed grid columns in `AppShell`                                     | Primary sidebar                                                         | Fixed  | No Notion-like drag-to-zero behavior                                                                 | AppShell owns resizable primary width with zero-width restore coverage                                                              |
...
| UX-L-015 | Level stack editor                                                        | Primary sidebar top                                                  | Secondary sidebar for plan/section, element sidebar when level selected | Fixed  | Level datum editing is model/view context, not project navigation                                    | Level switching now lives in plan secondary (`secondary-plan-level`) and selected level datum resolves in element sidebar inspector |
...
| UX-L-017 | Families button/tree                                                      | Primary sidebar                                                      | Ribbon Insert, secondary sidebar library, Cmd+K                         | Fixed  | Families are resources for editing, not navigation view groups                                       | Primary ownership tests remove family nav; ribbon/Cmd+K capabilities keep library reachable                                         |
| UX-L-018 | Types tree                                                                | Primary sidebar                                                      | Secondary sidebar type manager or modal from ribbon/Cmd+K               | Fixed  | Types are resources and templates, not view navigation                                               | Type resources are no longer in primary nav; family/type catalog is launched from Insert ribbon/Cmd+K modal surfaces                |
| UX-L-019 | Concept group placement                                                   | Primary sidebar lower group                                          | Primary sidebar near top                                                | Fixed  | User requested Concept near top                                                                      | Utility and DOM tests enforce primary nav order: Concept, Floor Plans, 3D Views, Sections, Sheets, Schedules                        |
| UX-L-020 | Sections nested under Views                                               | Primary sidebar                                                      | Primary sidebar top-level group                                         | Fixed  | Sections are a view type and should be peer to floor plans/3D/sheets                                 | Utility and DOM tests enforce Sections as a top-level primary group                                                                 |
...
Evidence (2026-05-12): seeded live verification at `http://127.0.0.1:2000/` with `make seed name=target-house-3` and `make dev name=target-house-3` confirms lens ownership moved out of footer. Fresh captures in `packages/web/tmp/ux-lens-secondary-20260512/` show `secondary-lens-filter` in both plan (`01-live-plan-secondary-lens.png`) and 3D (`02-live-3d-secondary-lens.png`) while `status-bar` contains no `lens-dropdown-trigger`; Cmd+K lens reachability is preserved (`03-live-cmdk-lens-commands.png`).
...
| UX-H-002 | `TabBar` mounted beside `TopBar` | Header wrapper in `Workspace` | Header core                               | Fixed  | Good concept but visually competes with mode/QAT/project controls | Header now keeps tabs as dominant center strip with compact global actions at right |
...
| UX-H-004 | Source view chip                 | Header in sheet mode          | Header or secondary sidebar sheet context | Fixed  | Source context may be useful, but can crowd tabs                  | Header ownership tests assert chip is absent from header (`source-view-chip`)       |
| UX-H-005 | Sidebar hamburger                | Header left                   | Header left when sidebar visible/hidden   | Fixed  | Needed for recovery after full collapse                           | Hide/reveal controls exist in header for both visible and collapsed primary states  |
| UX-H-006 | Project menu anchor              | Header project button         | Primary sidebar project selector          | Fixed  | Same as UX-L-001                                                  | DOM test verifies `ProjectMenu` opens from the primary project selector             |
...
| ID       | Current UI element                                                                                                                                         | Current location/source          | Target location                            | Status | Problem                                                                    | Target behavior                                                               |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------------------ | ------ | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| UX-R-001 | Revit-like ribbon tab set: Architecture, Structure, Steel, Precast, Systems, Insert, Annotate, Analyze, Massing & Site, Collaborate, View, Manage, Add-Ins | `RibbonBar`                      | View-type ribbon definitions               | Fixed  | Same broad tabs show in every view and include invalid/duplicated concepts | Build per-view ribbon schemas                                                 |
| UX-R-002 | Architecture build tools                                                                                                                                   | Ribbon                           | Plan ribbon Create/Openings/Rooms          | Fixed  | Correct commands, wrong global tab model                                   | Keep in floor plan ribbon only                                                |
| UX-R-003 | Structure tools                                                                                                                                            | Ribbon                           | Plan/3D contextual ribbons where supported | Fixed  | Some structure tools are plan authoring, some model edit                   | Scope each command by active view type and selection                          |
| UX-R-004 | View switch panel                                                                                                                                          | Ribbon View tab                  | Remove from ribbon                         | Fixed  | View switching belongs to primary navigation/header tabs                   | Use primary sidebar and tabs                                                  |
| UX-R-005 | Visibility/graphics action                                                                                                                                 | Ribbon View/Analyze tabs         | Secondary sidebar and Cmd+K                | Fixed  | View setting, not editing command                                          | Secondary sidebar owns view graphics; Cmd+K opens it                          |
| UX-R-006 | Load Family                                                                                                                                                | Ribbon Insert                    | Ribbon Insert                              | Fixed  | Correct authoring resource entry                                           | Keep in plan/concept/3D ribbons where placement/loading is meaningful         |
| UX-R-007 | Project Files/Project                                                                                                                                      | Ribbon Insert/Manage/Collaborate | Primary project menu or Cmd+K              | Fixed  | Project management is not active-view editing                              | Project selector/menu and Cmd+K                                               |
| UX-R-008 | Settings/help/add-ins                                                                                                                                      | Ribbon                           | Cmd+K/account/project menu                 | Fixed  | Global app/system actions dilute editing ribbon                            | Move to command palette or account/project menus                              |
| UX-R-009 | Modify contextual tab                                                                                                                                      | Ribbon when selected             | Ribbon contextual group                    | Fixed  | Good idea, but should vary by selected element and active view             | Keep contextual modify group only when selection exists and command can apply |
| UX-R-010 | Floating tool palette                                                                                                                                      | Canvas via `FloatingPalette`     | Ribbon                                     | Fixed  | Duplicates ribbon and creates another tool system                          | Remove persistent canvas palette; ribbon is tool entry                        |
| UX-R-011 | Tool modifier/options bars                                                                                                                                 | Below ribbon via `AppShell`      | Ribbon option row                          | Fixed  | Separate bars create extra chrome and include discipline controls          | Merge into ribbon as contextual options for active tool                       |
| UX-R-012 | Plan annotate ribbon                                                                                                                                       | Canvas via `AnnotateRibbon`      | Plan ribbon Annotate group                 | Fixed  | A second ribbon inside canvas is inconsistent                              | Consolidate into main ribbon                                                  |

Evidence (2026-05-12): seeded live capture at `packages/web/tmp/ux-ribbon-per-view-20260512/` confirms task-based plan tabs (`Create/Sketch/Insert/Annotate/Review`), scoped per-view ribbons for 3D (`3D View/Insert/Review` with `Load Family` in `05-3d-insert-load-family.png`), sheet (`Sheet`), and schedule (`Schedule`). Additional seeded screenshots `06-plan-wall-options-no-discipline.png` and `07-3d-ribbon-no-structure-create.png` prove tool options stay contextual without discipline selectors and 3D ribbon omits plan-only structure create tools. Ownership tests also enforce contextual 3D modify scoping (`UX-R-003`) plus removal of discipline/workspace controls from tool options rows (`UX-R-011`).
...
| UX-C-002 | 3D navigation hints                     | 3D canvas bottom         | Footer or transient canvas hint               | Fixed  | Helpful but competes with workspace status                                             | Persistent orbit hints are removed from canvas; only walk-mode transient hint remains     |
| UX-C-003 | Saved 3D viewpoint HUD                  | 3D canvas                | Secondary sidebar                             | Fixed  | Saved view state affects whole view and should not overlay model                       | Show active saved view in 3D secondary sidebar                                            |
...
| UX-C-005 | Plan detail toolbar                     | Plan canvas bottom       | Secondary sidebar                             | Fixed  | Detail level is view-level setting                                                     | Move to plan secondary sidebar                                                            |
| UX-C-006 | Plan crop panel                         | Plan canvas bottom right | Secondary sidebar                             | Fixed  | Crop is view-level setting                                                             | Move to plan secondary sidebar                                                            |
| UX-C-007 | Zoom/fit scale widget                   | Plan canvas              | Footer or secondary sidebar                   | Fixed  | Scale readout can be canvas-adjacent but commands should not become another toolbar    | Canvas now keeps compact passive scale readout (`plan-scale-readout`) without menu chrome |
...
| UX-C-014 | Temporary visibility menu               | Canvas bottom right      | Secondary sidebar / element sidebar           | Fixed  | Visibility is view/selection state, not canvas chrome                                  | View-level temporary visibility in secondary; selected isolate/hide in element sidebar    |
| UX-C-015 | Reveal hidden button                    | Canvas bottom right      | Secondary sidebar or footer                   | Fixed  | View display mode should not float on canvas                                           | Move to view visibility section                                                           |
| UX-C-016 | Snap settings toolbar                   | Canvas bottom right      | Ribbon options or footer compact snap cluster | Fixed  | Snap is tool/editing context, not canvas menu                                          | Floating snap toolbar is removed from canvas; compact snap/grid controls stay in footer   |
...
Evidence (2026-05-12): seeded capture at `packages/web/tmp/ux-canvas-secondary-reconcile-20260512/` proves plan view no longer mounts legacy canvas control docks (`plan-crop-view-properties`, `reveal-hidden-toggle`, `temp-visibility-toggle`) while secondary owns visibility/view-state controls (`secondary-plan-view-state`, `secondary-plan-visibility`, `secondary-reveal-hidden-toggle`). 3D capture proves scene/graphics/clipping and saved-view controls live in secondary (`secondary-sidebar-3d`, `secondary-3d-graphics`, `viewport3d-layers-panel`, `clip-elev-input`, `clip-floor-input`) with no canvas HUD (`orbit-viewpoint-persisted-hud`) and no element sidebar when nothing is selected. Follow-up seeded evidence in `packages/web/tmp/ux-canvas-hints-cleanup-20260512/01-live-3d-no-persistent-orbit-hints.png` confirms default orbit keyboard hint chrome is removed from canvas and only walk-mode transient hints remain.
Additional seeded check (2026-05-12): `packages/web/tmp/ux-canvas-scale-readout-20260512/01-live-plan-scale-readout.png` + `summary.json` confirm `plan-scale-readout` is present (`1`) and the old zoom preset trigger title count is `0`, closing plan canvas zoom-menu chrome ownership (`UX-C-007`, `UX-CAN-011`).
...
| UX-E-002 | No-selection right rail in 3D         | Right rail remains open for view controls | Hidden element sidebar + secondary sidebar view controls      | Fixed     | Violates selected-element-only rule                                            | Element sidebar hidden when no selection                                           |
| UX-E-003 | Scene/sun panel                       | Right rail no-selection section           | Secondary sidebar 3D                                          | Fixed     | Applies to whole 3D view                                                       | Move to 3D secondary sidebar                                                       |
| UX-E-004 | Active plan view editor in right rail | Right rail no-selection section           | Secondary sidebar plan                                        | Fixed     | Applies to active view, not selected element                                   | Move to plan secondary sidebar                                                     |
...
| UX-F-005 | Snap/grid clusters          | Footer/status bar                             | Footer/ribbon options       | Fixed     | Global readout is ok; detailed settings should be popover/options         | Footer owns compact snap/grid toggles while canvas toolbar chrome is removed |
| UX-F-006 | Undo/redo depth             | Footer/status bar                             | Footer                      | Keep      | Good global status                                                        | Keep; decide whether header undo buttons remain                              |
| UX-F-007 | Lens dropdown               | Footer/status bar                             | Secondary sidebar or footer | Fixed     | If lens affects active view, secondary sidebar; if project-global, footer | Lens is now owned by secondary sidebar; footer lens control removed          |
...
| ---------- | -------------------------------- | ------------------------------------------ | ------------------------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| UX-PRI-001 | Project name button              | Header `TopBar`                            | Primary sidebar top            | Fixed    | Primary selector is the project menu anchor; header project anchor is absent in DOM tests                                                |
...
| UX-PRI-006 | Account/avatar menu              | Header right                               | Primary sidebar bottom         | Fixed    | Primary ownership tests verify the account menu is bottom-anchored in the primary sidebar                                                |
...
| UX-PRI-008 | Concept navigation               | Lower browser group                        | Primary sidebar top group      | Fixed    | Utility and DOM tests enforce Concept as the first primary navigation group                                                              |
...
| UX-PRI-011 | Sections navigation              | Views/Sections browser                     | Primary sidebar nav            | Fixed    | Utility and DOM tests enforce Sections as a top-level primary navigation group                                                           |
...
| UX-PRI-014 | Project levels group             | Left rail level stack/browser              | Secondary sidebar or resources | Fixed    | Primary ownership tests verify level groups are absent from navigation chrome                                                            |
| UX-PRI-015 | Level edit controls              | Level stack                                | Secondary/element sidebar      | Fixed    | Level controls are removed from primary and active level now lives in secondary; selected level datum opens in element sidebar inspector |
| UX-PRI-016 | Types browser                    | Primary rail browser                       | Project resources dialog       | Fixed    | Types are removed from primary and routed to resource modal surfaces (Insert family library + element inspector type editing)            |
| UX-PRI-017 | Families browser                 | Primary rail browser and Families shortcut | Insert ribbon/resource dialog  | Fixed    | Primary tests remove family browser chrome; library remains reachable from Insert ribbon/Cmd+K                                           |
| UX-PRI-018 | Browser legend                   | Primary rail                               | None                           | Fixed    | Primary ownership tests verify browser legend is absent                                                                                  |
| UX-PRI-019 | Architecture/neutral selectors   | Top of left rail                           | Secondary sidebar or context   | Fixed    | Primary ownership tests verify authoring/context selectors are absent from the primary sidebar                                           |
| UX-PRI-020 | Collapsed icon strip             | `AppShell` left rail collapsed mode        | Primary sidebar/header         | Fixed    | Collapse/recovery behavior is covered by AppShell and live screenshot regressions                                                        |
| UX-PRI-021 | Full hide to zero width          | Not currently supported as target          | Primary sidebar                | Fixed    | Primary can collapse to zero and recover from the header in unit and Playwright coverage                                                 |
| UX-PRI-022 | Resize handle                    | Notion-like target only                    | Primary sidebar                | Fixed    | AppShell resize handle and persisted width behavior are covered by shell tests                                                           |
...
| UX-PRI-024 | Primary group ordering           | Mixed project/resources/view groups        | Primary sidebar                | Fixed    | Utility and DOM tests enforce project, search, Concept, Floor Plans, 3D Views, Sections, Sheets, Schedules, user menu                    |
...
| UX-PRI-029 | View templates listed as nav     | Potential browser/resource area            | Secondary/resources            | Fixed    | Templates are view configuration resources, not navigable tabs                                                                           |
| UX-PRI-030 | Links listed as nav              | Project browser/link dialogs               | Project resources              | Fixed    | Links are project resources; visibility in active view belongs to secondary sidebar                                                      |

Evidence (2026-05-12): seeded live capture at `packages/web/tmp/ux-primary-resource-reconcile-20260512/` shows primary nav without type/resource rows (`01-live-primary-nav-no-types.png`), plan secondary owning level/view-state controls (`02-live-secondary-plan-view-state.png`), and selected level datum reopening element sidebar inspector (`05-live-selected-level-element-sidebar.png`). Footer no longer owns level controls (`06-live-footer-no-level-controls.png`), matching `UX-STAT-003/004`.
...
| UX-HEAD-003 | Generic mode pills              | Header `TopBar`              | Primary nav and tab model | Fixed  | Header ownership tests verify mode pills are removed from workspace header and tabs/primary own navigation |
| UX-HEAD-004 | Add tab menu                    | Header tab controls          | Header                    | Keep   | Open/create named view choices grouped by view type                                                        |
| UX-HEAD-005 | Close inactive views            | Header QAT                   | Tab overflow menu         | Fixed  | Implemented in the tab overflow menu; no persistent tab-strip text action remains                          |
...
| UX-HEAD-009 | Participant avatars             | Header absolute overlay      | Header right              | Fixed  | Participant strip now renders inside a bounded right-side header control, avoiding tab overlap             |
| UX-HEAD-010 | Activity drawer entry           | Header/collaboration surface | Header or footer          | Fixed  | Activity stream ownership is consolidated in footer entry (`status-bar-activity-entry`)                    |
...
| UX-HEAD-013 | Undo/redo                       | Header QAT                   | Header compact or footer  | Fixed  | Keep only if visually secondary; otherwise move to footer/global edit status                               |
| UX-HEAD-014 | Measure shortcut                | Header QAT                   | Ribbon                    | Fixed  | Header ownership tests remove measure from header; ribbon/Cmd+K retain reachability                        |
| UX-HEAD-015 | Section shortcut                | Header QAT                   | Ribbon                    | Fixed  | Header ownership tests remove section shortcut from header; ribbon/Cmd+K retain reachability               |
| UX-HEAD-016 | Dimension shortcut              | Header QAT                   | Ribbon                    | Fixed  | Header ownership tests remove dimension from header; ribbon/Cmd+K retain reachability                      |
...
| UX-HEAD-019 | Project/workspace selector      | Header left                  | Primary sidebar           | Fixed  | Project selector lives in primary; workspace lens/view semantics are owned by secondary, not header        |
| UX-HEAD-020 | Offline/sync status             | Header overlay               | Footer                    | Fixed  | Global status belongs in footer unless blocking user action                                                |
| UX-HEAD-021 | Source view chip                | Header in sheet context      | Secondary or tab subtitle | Fixed  | Header no longer renders source-view chip; tab title/secondary own context                                 |
| UX-HEAD-022 | Header height                   | Current mixed controls       | Header                    | Fixed  | Header is now compact tab-first chrome (`min-h-[44px]`) with global actions only                           |
| UX-HEAD-023 | Header overflow                 | Current ad hoc action groups | Header                    | Fixed  | Tab overflow is isolated to tab strip menu while share/cmdk/presence remain separate global actions        |
...
Evidence (2026-05-12): seeded live captures in `packages/web/tmp/ux-header-reconcile-20260512/` (`01-plan.png` … `09-narrow-primary-restored.png`) and header overflow captures in `tmp/ux-tab-overflow-20260512/` (`01-header-tabs-overflow-closed.png`, `02-header-tabs-overflow-menu.png`) plus ownership assertions in `packages/web/src/workspace/Workspace.test.tsx` and `packages/web/src/workspace/shell/TabBar.test.tsx` confirm tab-first header dominance, source-chip removal, mode-pill removal, and separated tab-overflow versus global header actions.
...
| UX-CAN-006 | Saved 3D view HUD            | `OrbitViewpointPersistedHud`           | Secondary sidebar        | Fixed  | Move persistent saved-view fields out of canvas                                      |
| UX-CAN-007 | Floating palette             | Workspace canvas                       | Ribbon                   | Fixed  | Persistent tools belong in ribbon                                                    |
| UX-CAN-008 | Plan detail toolbar          | `PlanCanvas`                           | Ribbon/secondary         | Fixed  | Editing commands to ribbon, display/detail settings to secondary                     |
| UX-CAN-009 | Annotation bar               | `PlanCanvas`                           | Ribbon                   | Fixed  | Annotation commands belong in annotate ribbon group                                  |
| UX-CAN-010 | Crop properties overlay      | `PlanCanvas`                           | Secondary/element        | Fixed  | View crop state in secondary; selected crop boundary properties in element sidebar   |
| UX-CAN-011 | Zoom/fit widget              | `PlanCanvas`                           | Canvas exception/footer  | Fixed  | Plan canvas now exposes passive scale readout only; zoom-command menu chrome removed |
...
| UX-CAN-019 | Temporary visibility chip    | `PlanCanvas`                           | Secondary plus transient | Fixed  | Persistent toggle in secondary; canvas chip only indicates active temporary mode     |
...
| UX-CAN-027 | Keyboard shortcut hints      | Canvas/help overlays                   | Cmd+K/help dialog        | Fixed  | Keep transient hints only; avoid persistent instructional chrome                     |
...
| ---------- | ---------------------------------- | ------------------------------------------ | ---------------------- | -------- | ------------------------------------------------------------------------------------------ |
| UX-ELE-001 | Nothing selected                   | Right rail still shows scene/view controls | Hidden                 | Fixed    | Right element sidebar must disappear completely with no selection                          |
...
| UX-ELE-015 | Scene sun controls                 | Right rail no-selection                    | Secondary sidebar      | Fixed    | Remove from element sidebar                                                                |
...
| UX-DIA-002 | Project menu                    | `ProjectMenu.tsx`           | Primary project selector      | Fixed    | DOM test verifies the project menu opens from `primary-project-selector`                                                              |
| UX-DIA-003 | Manage links                    | `ManageLinksDialog.tsx`     | Project resources/secondary   | Fixed    | Manage links launches from primary project resources menu and stays reachable from Cmd+K while secondary remains the view-level owner |
| UX-DIA-004 | Visibility/Graphics             | `VVDialog.tsx`              | Secondary sidebar advanced    | Fixed    | Plan secondary now owns direct advanced dialog trigger (`secondary-plan-open-vv-dialog`) with Cmd+K fallback                            |
| UX-DIA-005 | Family library                  | Family panels/browser       | Insert ribbon/resource dialog | Fixed    | Family/type library is launched from Insert ribbon and Cmd+K, not from primary navigation                                             |
...
| UX-DIA-013 | Onboarding tour                 | App overlay                 | Help/onboarding               | Fixed    | Tour steps now describe canonical header/primary/ribbon/secondary/footer ownership and replay from Cmd+K/project resources            |
...
| UX-DIA-017 | Type editor                     | Type/family browser         | Resources or element sidebar  | Fixed    | Type editing is selection/resource scoped (element inspector + family/type resource dialog), not primary navigation                   |
| UX-DIA-018 | View template editor            | Graphics/view settings      | Secondary advanced dialog     | Fixed    | View template controls are owned by plan secondary view-state/editor surfaces instead of primary navigation                           |
...
Additional evidence (2026-05-12): onboarding copy/targets are rewritten in `onboarding/tour.ts` and validated by `tour.test.ts`; `Workspace.test.tsx` plus UX regression e2e now verify replay via `palette-entry-help.replay-onboarding-tour`; seeded live capture `packages/web/tmp/ux-dia-013-onboarding-live-20260512/summary.json` reports `initialTourCount: 0`, `paletteReplayCount: 1`, `replayTourCount: 1`.
Additional evidence (2026-05-12): `Workspace.test.tsx` now verifies plan secondary opens `VVDialog` via `secondary-plan-open-vv-dialog`, and UX regression e2e captures the same ownership path in `15-visibility-graphics-dialog.png`.
...
| UX-STAT-003 | Active level dropdown         | `StatusBar.tsx`                        | Fixed                    | Level switching moved out of footer; active level owner is plan secondary (`secondary-plan-level`)                                                                 |
| UX-STAT-004 | Level elevation readout       | `statusbar-level-elevation`            | Fixed                    | Level elevation readout moved with level owner into secondary/element context and removed from footer                                                              |
| UX-STAT-005 | Active tool readout           | `StatusBar.tsx`                        | Keep                     | Footer can show current command/prompt                                                                                                                             |
| UX-STAT-006 | Snap mode toggles             | `StatusBar.tsx`                        | Fixed                    | Footer owns compact global snap toggles; floating plan snap toolbar is removed from canvas                                                                         |
| UX-STAT-007 | Grid toggle                   | `StatusBar.tsx`                        | Fixed                    | Footer owns compact global grid toggle while view-state controls remain in secondary                                                                               |
...
| UX-STAT-013 | Activity entry                | `status-bar-activity-entry`            | Fixed                    | Activity stream ownership remains in footer entry; header collaboration strip is presence/comments only                                                            |
...
| UX-STAT-019 | Jobs/progress status          | `JobsPanel.tsx`                        | Partial                  | Footer entry is implemented, but seeded API currently returns `404 /api/jobs`; running/failed counts cannot be proven until jobs endpoint availability is restored |
...
| ----------- | ----------------------------------- | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| UX-RISK-001 | Line count mistaken for quality     | A 1000-line tracker can still miss key behavior               | Fixed with automated quality gates for source audit rows, screenshot-state coverage, and command registry reachability             |
| UX-RISK-002 | Legacy code mistaken for live UI    | Some components may be unused or conditionally mounted        | Fixed with route inventory guards and Playwright reachability checks for mounted workspace, public, family-editor, and icon routes |
| UX-RISK-003 | Hidden mode state missed            | Some UI appears only during active commands or selections     | Fixed with active command Playwright coverage proving modifiers/options stay in ribbon and status stays in footer                  |
...
| UX-RISK-005 | Dialog trigger regressions          | Moving controls can orphan dialogs                            | Fixed with modal command ownership coverage requiring every dialog command to declare canonical triggers plus Cmd+K fallback       |
| UX-RISK-006 | Ribbon over-pruning                 | Hiding invalid tools can accidentally remove real workflows   | Fixed with pruned-ribbon audit proving hidden invalid commands keep Cmd+K reachability and bridge/unavailable reasons              |
| UX-RISK-007 | Secondary sidebar overload          | Moving all view controls left can create a new junk drawer    | Fixed with secondary section scope metadata, advanced disclosure sections, and ownership tests against junk-drawer regressions     |
...
| UX-LIVE-003 | Header overload persisted in every seeded mode.                                                                    | Header                            | Fixed  | Header semantic ownership tests prove tabs/Cmd+K/share stay in header while authoring commands are absent.               |
| UX-LIVE-004 | Primary sidebar contained editable levels, types, families, browser legend, and view navigation together.          | Primary/secondary/resources       | Fixed  | Primary sidebar ownership tests prove only project selector, search, view navigation, and user menu remain.              |
| UX-LIVE-005 | Collapsed primary sidebar showed no useful left rail content.                                                      | Primary/header                    | Fixed  | Collapse, zero-width drag, narrow auto-hide, and header restore are covered by unit, Playwright, and live captures.      |
| UX-LIVE-006 | Generic Architecture ribbon stayed visible in sheet, schedule, agent, concept, 3D, and plan states.                | Ribbon                            | Fixed  | Main view-type screenshots and ribbon pruning audits prove per-view ribbons with Cmd+K bridges for hidden commands.      |
| UX-LIVE-007 | Plan canvas retained floating palette, command dock, snap toolbar, reveal hidden, scale, and north/readout chrome. | Ribbon/secondary/footer/canvas    | Fixed  | Canvas ownership tests and screenshots prove persistent tool palette/temp visibility chrome is absent from canvas.       |
| UX-LIVE-008 | 3D no-selection state showed sun, scene, graphics, layers, projection, and section box in right rail.              | Secondary sidebar                 | Fixed  | 3D no-selection tests and live secondary-disclosure screenshots keep view-wide controls in the secondary sidebar.        |
| UX-LIVE-009 | Selected 3D roof state showed useful element inspector plus 3D view controls in the same right rail.               | Element sidebar/secondary sidebar | Fixed  | Element-sidebar selection regressions prove selected properties stay separate from secondary view-wide controls.         |
| UX-LIVE-010 | Sheet mode showed real sheet canvas, manifest, review toolbar, and advisor errors under generic chrome.            | Sheet secondary/ribbon/footer     | Fixed  | Sheet mode screenshot coverage proves sheet secondary, sheet canvas, sheet ribbon commands, and footer advisor.          |
| UX-LIVE-011 | Schedule mode showed real schedules with row/place/duplicate actions under generic chrome.                         | Schedule secondary/ribbon         | Fixed  | Schedule mode screenshots and live pruning capture prove schedule definition lives left and row/place actions in ribbon. |
| UX-LIVE-012 | Agent findings appeared in agent canvas and also in right rail review content.                                     | Footer/advisor workflow           | Fixed  | Agent mode screenshots and advisor footer/dialog tests prove global advisor entry is footer-owned.                       |
| UX-LIVE-013 | Active wall command showed tool modifier/options bar and status-bar command text.                                  | Ribbon modifier/footer            | Fixed  | Active-command screenshots prove modifiers/options stay in ribbon/options region and footer keeps command prompt.        |
