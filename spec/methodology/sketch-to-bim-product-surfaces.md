...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sketch IR               | `sketch_bim.py semantic-checklist`, `accept --ir`, methodology SKB-21 brief creation                                     | API v3 descriptor/route `sketch.ir.validate` at `POST /api/v3/sketch/ir/validate`; CLI `sketch ir validate --ir --capabilities --out` aliasing `initiation-check`; schemas in `packages/cli/lib/sketch-initiation.mjs`; durable fixtures in `spec/data/sketch-to-bim-golden-seeds.json`                                                                                                                       | `executable`. API route validates the stable IR contract; CLI writes the full coverage/checklist/acceptance packet.                                                        | Keep backend validation aligned with the richer CLI packet builder as the IR schema evolves.                                                                                                                                                    |
...
| Advisor / evidence loop | `advisor`, `advisor-parity`, `constructability-report`, `browser-evidence`, `issue-ledger`, `agent-loop-packet`, `material-check` | API v3 descriptors/routes `qa.advisor` at `POST /api/models/{model_id}/qa/advisor` and `qa.constructability` at `GET /api/models/{model_id}/constructability-report`; CLI `qa advisor --output json --severity`; CLI `initiation-run` writes snapshot, validate, evidence-package, advisor warning/info, optional screenshots, visual gate; backend routes `/snapshot`, `/validate`, `/evidence-package`, `/command-log` | `executable` for Advisor/constructability and command-log. Evidence package is backend route-backed; `sketch.evidence.collect` remains `CLI-only`; browser evidence and `agent-loop-packet` are `skill-local` adapter surfaces. | Promote `agent-loop-packet` into a product `sketch.evidence.agent-loop` descriptor once the backend hosts source bundle/recipe provenance. Keep browser automation as capture/equivalence support, not a core MCP dependency. |
...
- Evidence collection still mixes product routes with CLI orchestration and
  ...
  advisor status.
- `agent-loop-packet` gives agents a deterministic bridge from Advisor and
  constructability findings to source authoring commands, command-log
  transactions, recipe/bundle line hits, phase ownership, and next actions.
  This is currently skill-local adapter metadata; productizing it means hosting
  the same provenance packet behind an API/MCP descriptor.
