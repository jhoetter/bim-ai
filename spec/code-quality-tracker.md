...
| `app/bim_ai/routes_api.py`                                  | 3,763      | API route aggregator after extracted query/resolve/QA and presentation routes.                    |
| `packages/web/src/viewport/meshBuilders.ts`                 | 3,728      | Mesh builder collection after extracted family/detail, balcony, and structural builder modules.   |
...
- 2026-05-20: extracted external model-call audit, source ingestion,
...
- 2026-05-20: a follow-up PlanCanvas extraction slice moved select-tool click
  picking, EQ toggle dispatch, linked-model filtering, and multi-select intent
  handling into `plan/planCanvasSelectClick.ts`, reducing `PlanCanvas.tsx` to
  `5,412` local lines with focused PlanCanvas, EQ dimension, select-linked
  tests, and web typecheck passing.
- 2026-05-20: a follow-up PlanCanvas extraction slice moved query, tag, door,
  and window click handling into `plan/planCanvasClickHandlers.ts`, reducing
  `PlanCanvas.tsx` to `5,392` local lines with focused PlanCanvas, manual-tag,
  DXF, select-linked tests, and web typecheck passing.
- 2026-05-20: a Workspace extraction slice moved floor-plan, ceiling-plan,
  saved-3D-view, section-tool, sheet, and schedule creation actions into
  `workspace/useWorkspaceCreateViews.ts`, reducing `Workspace.tsx` to `5,248`
  local lines with focused Workspace, saved-view, sheet tests, and web
  typecheck passing.
- 2026-05-20: a PlanCanvas extraction slice moved room-rectangle, grid,
...
- 2026-05-20: a Workspace extraction slice moved sheet, schedule, section, and
...
- 2026-05-20: a Viewport extraction slice moved saved-3D overlay list
  ...
  passing.
- 2026-05-20: a Viewport extraction slice moved wall context menu, slice-grip
...
- 2026-05-20: an InspectorContent extraction slice moved angular, radial,
  ...
  passing.
- 2026-05-20: an InspectorContent extraction slice moved wall and floor
...
- 2026-05-20: a backend element-model extraction moved MEP route/equipment/
  ...
  and ruff passing.
- 2026-05-20: a viewport mesh-builder extraction moved balcony rendering and
  structural column/beam/connection rendering into `meshBuilders.balcony.ts`
  and `meshBuilders.structural.ts`, reducing `meshBuilders.ts` to `3,728`
  local lines with full web typecheck, focused viewport ESLint, and focused
  mesh tests passing.
...
- 2026-05-20: a Viewport extraction slice moved post-mount scene
  synchronization effects into `viewport/useViewportSceneEffects.ts`, reducing
  `Viewport.tsx` to `3,801` local lines with focused viewport tests and web
  typecheck passing.
- 2026-05-20: a core extraction slice moved late model contracts, command
  contracts, presentation/export contracts, and collaboration-adjacent aliases
  into `core/src/modelContracts.ts`, reducing `packages/core/src/index.ts` to
  `3,386` local lines with core and web typechecks passing.
- 2026-05-20: a family editor extraction slice moved symbolic-line and sweep
  properties panels into `familyEditor/FamilyEditorPropertiesPanels.tsx`,
  reducing `FamilyEditorWorkbench.tsx` to `3,813` local lines with focused
  family editor tests and web typecheck passing.
