...
- 2026-05-20: a viewport mesh-builder extraction moved balcony rendering and
  ...
  and `meshBuilders.structural.ts`, reducing `meshBuilders.ts` to `3,724`
  local lines with full web typecheck, focused viewport ESLint, and focused
  mesh tests passing.
- 2026-05-20: a CLI extraction moved the MEP command family into
  `packages/cli/lib/mep-commands.mjs`, reducing `packages/cli/cli.mjs` to
  `3,722` local lines with `node --check` and CLI MCP parity tests passing.
