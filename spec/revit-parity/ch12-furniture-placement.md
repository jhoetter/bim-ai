...
**bim-ai status:** 🟡 Partial — Column elements can be rotated via: (1) a drag-handle grip in the plan canvas (`columnGripProvider.ts` — "Drag to rotate column" hint), which fires `updateElementProperty { key: 'rotationDeg', value }` on drag completion; (2) a "Rotation (°)" `<input type="number">` (`data-testid="inspector-column-rotation"`, step 15°) in the column inspector. Missing: a general-purpose two-click Rotate-about-point command applicable to non-column elements (floors, rooms, walls, etc.); no angular snap at standard angles (0°/45°/90°/etc.); no numeric "rotate by typed angle" dialog.
