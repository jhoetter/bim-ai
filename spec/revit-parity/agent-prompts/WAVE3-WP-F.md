...
Add to `packages/web/src/schedules/scheduleExport.test.ts` (new file): 5. CSV string has correct header row matching preset field labels 6. Values containing commas are double-quoted in CSV output 7. Empty schedule rows → header only (no crash)

Add to toolbar tests (wherever ScheduleDefinitionToolbar is tested): 8. "Export CSV" button is present (data-testid="schedule-export-csv") 9. Level filter dropdown shows "All Levels" + project levels 10. Selecting a level filters rows to that level's elements
