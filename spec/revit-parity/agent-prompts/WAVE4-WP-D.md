...
Add to `packages/web/src/viewport/excavationMesh.test.ts` (new file): 4. `buildExcavationMesh()` returns a Group with 2 children (walls + floor) 5. Floor mesh y-position = `−depthMm / 1000` metres 6. Zero-area boundary → returns empty Group without crash
