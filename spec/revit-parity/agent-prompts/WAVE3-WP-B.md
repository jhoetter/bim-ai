...
Add to `packages/web/src/workspace/project/ProjectBrowser.test.tsx` (or WorkspaceLeftRail test): 4. "Place on Sheet…" menu item appears in context menu for a saved_view row 5. Clicking a sheet in the picker dispatches `update_sheet` with a `viewRef` starting with
`'saved_view:'`
