...
**bim-ai status:** ✅ Available — `ManageLinksDialog.tsx` lists `link_model` rows, `link_dxf` underlays, and typed IFC/PDF/image `link_external` rows. Rows expose source metadata, loaded/unloaded status, reload status/message, reload/unload/remove, source-path changes, visibility, spatial position lock, alignment/position controls, and relevant opacity controls; DXF rows also expose color mode and layer visibility with source reparse on linked reload.
