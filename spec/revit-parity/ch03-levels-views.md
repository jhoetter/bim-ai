...
**bim-ai status:** 🟡 Partial — `ProjectBrowser.tsx` now shows `section_cut` and `elevation_view` rows with inline rename support (double-click → input, blur/Enter commits via `updateElementProperty { key: 'name' }`). Elevation markers are rendered as triangular symbols in the plan canvas (`symbology.ts` / `elevationViewPlanThree`). Double-clicking an elevation marker in the plan canvas fires `activateElevationView(id)` and opens the corresponding elevation tab (`PlanCanvas.tsx` `onDblClick` handler). A "N/S/E/W" button in the Project Browser's Elevations section header fires four `createElevationView` commands (North/South/East/West Elevation) in one click. Missing: true automatic generation on project creation (without user action); multi-view elevation host grouping.
