...
| F-047 | Temporary Hide / Isolate (sunglasses)                | Walls          | 🟡            | 👓 Isolate + 👁 Hide buttons in PlanCanvas (both category-level); TemporaryVisibilityChip in Workspace resets; missing per-element isolate/hide and unified sunglasses pop-up menu |
...
| F-096 | Area and Volume Computations dialog                  | Rooms & Areas | 🟡            | "Volume Computed At" and "Room Area Computation Basis" fields on `project_settings` element; editable in right-rail inspector and via "⚙ Computations…" button in area-boundary OptionsBar (inline panel). Missing: backend derivation engine does not yet consume these settings |
...
| F-103 | Move tool (MV) — two-point with snap                 | Troubleshoot   | 🟡            | Wall grip drag + Δx/Δy inspector input; `placed_asset` elements support Δx/Δy inspector move (MoveAssetDelta); missing two-point on-canvas Move for floor/room/door/window elements |
...
6. **Temporary Hide/Isolate** (F-047, F-101–F-102) — F-047 and F-101 are 🟡: both a "👓 Isolate" and "👁 Hide" button now appear in PlanCanvas when an element is selected (both operating at category level); `TemporaryVisibilityChip` in Workspace resets. Missing: per-element (non-category) isolate/hide and the unified sunglasses pop-up menu. F-102 is 🟡 — permanent view-specific hide via VVDialog + inspector "Hide Category in View" button. F-014 (Reveal Hidden) is ✅ — 💡 toggle + magenta chip + magenta per-element overlay + right-click "Unhide in View: <category>" context menu.
