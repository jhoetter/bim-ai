...
**bim-ai status:** 🟡 Partial — `TemporaryVisibilityChip.tsx` is mounted in `Workspace.tsx` and shows "Isolate: <categories>" or "Hide: <categories>" with a one-click reset. `PlanCanvas.tsx` now renders two buttons when an element is selected: "👓 Isolate" (`data-testid="temp-visibility-toggle"`) and "👁 Hide" (`data-testid="temp-hide-toggle"`) — both operating at category level. Missing: per-element (non-category) isolate/hide; no "Isolate Element" / "Hide Element" sub-menu equivalent.
