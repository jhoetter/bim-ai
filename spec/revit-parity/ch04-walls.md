...
**bim-ai status:** 🟡 Partial — `TemporaryVisibilityChip.tsx` is mounted in `Workspace.tsx` and shows "Isolate: <categories>" or "Hide: <categories>" with a one-click reset. `PlanCanvas.tsx` renders a "👓 Isolate / 👓 Reset" button (`data-testid="temp-visibility-toggle"`) when an element is selected, calling `setTemporaryVisibility({ mode: 'isolate', categories: [el.kind] })`. Missing: Revit's full sunglasses menu (Isolate Category, Isolate Element, Hide Category, Hide Element — bim-ai only supports Isolate Category); no "Hide" mode trigger from the button.
