...
**bim-ai status:** 🟡 Partial — A "Graphic Display Options" toggle button (`data-testid="viewport-gdo-toggle"`) in the 3D viewport bottom bar opens a panel with: Visual Style (Shaded/Wireframe), Background (White/Light Grey/Dark — applied to the Three.js renderer clear color), and Edge display (Normal/None). The existing shaded/wireframe button remains. Missing: silhouette edge width, depth cue (near/far fade), photographic exposure, shadows, ambient occlusion, and background image options.
