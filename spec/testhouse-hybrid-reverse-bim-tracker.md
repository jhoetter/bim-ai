...
| TH-X-F006 | Open | Alpha, Beta, Gamma | Product/schema | The TH-UI-004 evidence pill currently uses a name-heuristic stub. The persistent backing requires a first-class `source_view_evidence` element kind that joins to `section_cut` / `elevation_view` / detail (callout `plan_view`) by `viewElementId`, with fields `sourceDocumentId`, `page`, `region`, `comparisonType`, `screenshotPath`, `overlayPath`, `status` (one of `missing_source_link`, `source_linked`, `screenshot_captured`, `overlay_compared`, `findings_open`, `accepted`), `findingIds[]`, `notes`. | Implement in iteration 2 across `packages/core/src/index.ts`, `app/bim_ai/elements.py`, `app/bim_ai/commands.py` (`UpsertSourceViewEvidenceCmd`), `app/bim_ai/engine_dispatch_documentation.py`, and the project-browser pill renderer (read from joined element instead of heuristic). The MCP descriptor `reverse_bim.source_view_evidence_upsert` is already registered. |
...
| TH-X-F008 | Open | Beta, Gamma (Alpha unaffected) | Methodology / classifier defect | Compound scanned PDFs (single PDF holding plans + elevations + sections + details) are classified document-wide. `Grundrisse, Ansichten, Schnitt (1).pdf` → `floor_plan` only; `Kannenofen.pdf` → `unknown` (worst case — 5/6 work packages got `missing_inputs`). Page-level visual classification is missing. | Add `source.classify_pages` (or extend `source.classify_documents`) to emit per-page roles for any PDF where the document-level classifier is `unknown`, where page-count >> 1, or where document role is one of `{floor_plan, elevation, section, detail, site_plan, mixed}`. Use the rendered PNGs (native text is unavailable for scans). Until this lands, single-PDF scan sources are second-class. Tracked across [[TH-B-F003]] and [[TH-G-F003]]. |
| TH-X-F009 | Open | Alpha, Beta, Gamma | Methodology / reader response routing | The folder-output reader normalizer counts each response file against exactly one `workPackageId`. A single "global" reader that emits facts spanning multiple packages (the gamma rescue pattern in iteration 1) only satisfies one package's acceptance gate. | Pick one: (a) document a "split before normalize" convention in `claude-skills/hybrid-reverse-bim/SKILL.md` and add a helper `source.split_reader_response_by_work_package`; or (b) extend `source.normalize_ai_visual_trace_reader_responses` to honour an `additionalWorkPackageIds` array on the response envelope. Recommend (b). |
| TH-X-F010 | Open | Alpha, Beta, Gamma | Methodology / preflight DPI | Iteration 1 used the default 160-180 DPI render; both alpha and beta reader passes returned a "dimension legibility" conflict and had to fall back on inferred wall thicknesses (era-typical 365/240/115 mm with confidence 0.4-0.55). Gamma rotated-scan pages compound this. | Preflight should default to 240+ DPI for any source classified as `floor_plan` / `section` / `elevation` / `detail` / `area_calculation`, and to 300+ DPI when reader pass-01 raises a `dimension_legibility` conflict. Add a `source.rerender_for_legibility` helper that targets specific (documentId, page) pairs. |
...
1. **Ship `source.classify_pages`** (or equivalent page-level visual
   classification) for compound PDFs. Without it, beta and gamma cannot
   reach a properly-routed reader pass. ([[TH-X-F008]])

2. **Ship `source_view_evidence` schema** end-to-end (core types, Pydantic,
   command, engine dispatch, project-browser pill backing). ([[TH-X-F006]])

3. **Extend the response normalizer** to honour `additionalWorkPackageIds`
   so a single "global" or "rescue" reader can satisfy multiple work
   packages. ([[TH-X-F009]])

4. **Default preflight render DPI to 240 for plan/section/elevation/detail
   roles** and add a `source.rerender_for_legibility` helper that targets
   specific `(documentId, page)` pairs when readers flag dimension
   legibility. ([[TH-X-F010]])
