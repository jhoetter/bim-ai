...
| Cmd+K                 | Groups results by context badge, shows disabled/bridge state for registered commands, supports `>` tool / `@` view / `:` settings prefixes, scopes recent non-universal commands by active view/mode, and includes active-view visibility, snapshot, presentation, schedule, and universal rail routing. |
