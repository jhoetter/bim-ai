...
| Floating palette      | Shows active-canvas tool definitions and checks capability availability plus tool preconditions; pure 3D exposes Select only while 3D edits live in selected-element/right-rail/Cmd+K commands; legacy perspective-only plan-tool filtering has been removed. |
