...
| ID | Work item | Current status | Done criteria |
| --- | --- | --- | --- |
| W5-001 | Run fresh Leo source preflight from folder. | Partial | Fresh package at `tmp/reverse-bim/leo-hybrid-fresh-v4` was generated from `/Users/jhoetter/Desktop/Testhäuser/Testhaus Leo`: 16 source documents, 68 rendered pages, 6 work packages, 12 chunked AI-reader requests, 20 reader-pass assignments including independent second passes for 4 critical packages, and 20 self-contained reader prompts. The floor-plan package now contains only Exposé pages 14-16 plus `EG.pdf`, `DG.pdf`, and `Grundrisse, Schnitt.pdf`; photo/narrative Exposé pages are no longer misrouted as plans. Manifest rows include content-stable source ids. |
| W5-002 | Repair Leo source understanding until handoff-ready or explicitly blocked. | Partial | All 20 reader responses are present and valid response files; 19 are Codex subagent responses and one current-condition chunk used the optional OpenAI adapter. The package correctly remains `source_understanding_blocked`: levels now reconcile to KG/EG/DG and are complete, but target scope, coordinate frames, room access/adjacency, area-to-room binding, site/terrain precision, roof/dormer precision, material assembly dispositions, and 2 consensus gaps remain. Modeling must not start until those source repairs/dispositions are closed. |
...
reader-response consolidation/repair evidence.
```
