...
| GPF-005 | Package source pages for multimodal AI/subagent reading. | `source.ai_reading_packet`, `source.ai_visual_trace_packet`, `source.ai_visual_trace_work_order`, `source.ai_visual_trace_agent_requests`, and `source.ai_visual_trace_reader_pass_manifest`; large work packages are split into chunked reader requests, assigned to first/independent reader passes, and merged by work package before validation. | Partial | Page-level reader routing and actual provider/subagent dispatch remain; orchestration/checklist is provider-neutral and deterministic. |
