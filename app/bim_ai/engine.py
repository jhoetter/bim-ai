    ...
    match cmd:
        ...
        case UpdateFrameCmd():
            ...
            if not isinstance(frame, FrameElem):
                raise ValueError(
                    f"update_frame: element '{cmd.id}' is not a frame"
                )
            patch = {}
        ...
        case DeleteFrameCmd():
            ...
            if not isinstance(frame, FrameElem):
                raise ValueError(
                    f"delete_frame: element '{cmd.id}' is not a frame"
                )
        ...
        case ReorderFrameCmd():
            ...
            if not isinstance(frame, FrameElem):
                raise ValueError(
                    f"reorder_frame: element '{cmd.id}' is not a frame"
                )
            # Update the target frame's sort_order
            els[cmd.id] = frame.model_copy(update={"sort_order": cmd.new_sort_order})
            # Re-normalise all frames on the same canvas to sequential integers
        ...
        case UpdateSavedViewCmd():
            ...
            if not isinstance(sv, SavedViewElem):
                raise ValueError(
                    f"update_saved_view: element '{cmd.id}' is not a saved_view"
                )
            patch = {}
            if cmd.name is not None:
                patch["name"] = cmd.name
        ...
        case DeleteSavedViewCmd():
            ...
            if not isinstance(sv, SavedViewElem):
                raise ValueError(
                    f"delete_saved_view: element '{cmd.id}' is not a saved_view"
                )
            del els[cmd.id]
