...
def _authoritative_replay_v0_preflight(doc: Document, cmds_raw: list[dict[str, Any]]) -> str | None:
    ...
    for cmd in cmds_raw:
        ...
        if t == "createLevel":
            ...
            fid = cmd.get("hostFloorId")
            if not isinstance(fid, str) or not fid.strip():
                return "merge_reference_unresolved"
            fs = fid.strip()
            if fs not in known_floors:
                return "merge_reference_unresolved"
            eid = _authoritative_replay_v0_declared_id(cmd)
            if eid is not None:
                if eid in doc.elements or eid in declared:
...
def try_apply_kernel_ifc_authoritative_replay_v0(
    ...
    ``createSlabOpening`` / ``insertDoorOnWall`` / ``insertWindowOnWall`` payloads from
    ``build_kernel_ifc_authoritative_replay_sketch_v0``. Runs
