...
_RULE_DISCIPLINE: dict[str, str] = {
    ...
    "toposolid_pierce_check": "structure",
...
_RULE_BLOCKING_CLASS: dict[str, str] = {
    ...
    "toposolid_pierce_check": "geometry",
    "monitored_source_drift": "evidence",
...
def _ear_clip_triangulate(
    ...
    # Force CCW orientation so the convex test below is uniform.
    pts = list(poly)
    ...
    while len(indices) > 3 and safety < 4 * n:
        ...
        for k in range(m):
            ...
            if valid:
                ...
                break
        if not ear_found:
            # Degenerate polygon — give up rather than loop forever.
            return []
    if len(indices) == 3:
...
def _clip_convex_against_convex(
    subject: list[tuple[float, float]],
    clip: list[tuple[float, float]],
) -> list[tuple[float, float]]:
    """Sutherland–Hodgman: clip the (convex) subject against the (convex) clip."""

    if not subject or not clip:
        return []
    # Ensure clip is CCW so the half-plane test below is consistent.
...
def _polygon_overlap_area_mm2(
    poly_a: list[tuple[float, float]],
    poly_b: list[tuple[float, float]],
) -> float:
    """Polygon-polygon intersection area in mm² for arbitrary simple polygons.

    Triangulates each polygon (ear clipping handles concave shapes), then sums
    the area of every triangle-vs-triangle clip via Sutherland–Hodgman. O(n²·m²)
    but n, m are small (floor footprints are typically ≤16 vertices).
    """

    if len(poly_a) < 3 or len(poly_b) < 3:
        return 0.0
    ax0, ay0, ax1, ay1 = _polygon_bbox(poly_a)
    bx0, by0, bx1, by1 = _polygon_bbox(poly_b)
    if ax1 < bx0 or bx1 < ax0 or ay1 < by0 or by1 < ay0:
        return 0.0
    tris_a = _ear_clip_triangulate(poly_a)
    tris_b = _ear_clip_triangulate(poly_b)
    ...
    for ta in tris_a:
        ...
        for tb in tris_b:
            ...
            clipped = _clip_convex_against_convex(ta_list, tb_list)
            if len(clipped) >= 3:
...
def evaluate(elements: dict[str, Element]) -> list[Violation]:
    ...
    viols.extend(_toposolid_pierce_advisory_violations(elements))
...
def _toposolid_pierce_advisory_violations(
    elements: dict[str, Element],
) -> list[Violation]:
    toposolids = [e for e in elements.values() if isinstance(e, ToposolidElem)]
    if not toposolids:
        return []
    floors = [e for e in elements.values() if isinstance(e, FloorElem)]
    if not floors:
        return []
    slab_opening_host_ids = {
        e.host_floor_id
        for e in elements.values()
    ...
    for topo in toposolids:
        topo_poly = Poly(tuple((pt.x_mm, pt.y_mm) for pt in topo.boundary_mm))
        for floor in floors:
            if floor.id in slab_opening_host_ids:
                continue
            if len(floor.boundary_mm) < 3:
                continue
            floor_poly = Poly(tuple((pt.x_mm, pt.y_mm) for pt in floor.boundary_mm))
            if sat_overlap(topo_poly, floor_poly):
                fname = getattr(floor, "name", floor.id) or floor.id
                out.append(
                    Violation(
                        rule_id="toposolid_pierce_check",
                        severity="warning",
                        message=(
                            f"Floor '{fname}' overlaps toposolid '{topo.id}' without a slab_opening — "
                            "consider adding a SlabOpening to cut through the terrain."
                        ),
                        element_ids=[floor.id, topo.id],
                        blocking=False,
                    )
                )
    return out
