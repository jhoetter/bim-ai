...
def test_place_room_at_point_creates_room_inside_closed_wall_loop() -> None:
    lvl = LevelElem(kind="level", id="lvl-1", name="EG", elevationMm=0)
    walls = (
        WallElem(
            kind="wall",
    ...
    assert ok
    room = new_doc.elements["room-1"]
    assert isinstance(room, RoomElem)
    assert room.name == "Office"
    assert len(room.outline_mm) == 4
